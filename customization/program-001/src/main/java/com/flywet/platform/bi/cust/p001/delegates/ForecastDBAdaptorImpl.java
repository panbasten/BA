...
public class ForecastDBAdaptorImpl extends BIAbstractDbAdaptor implements
    ...
    @Override
    public String getDescriptionFromExtendPredict(String title) throws KettleDatabaseException {
        String sql = "SELECT "
                + quote(CustomDatabaseRepositoryBase.FIELD_EXTEND_PREDICT_DESCRIPTION)
                + " FROM "
                + quoteTable(CustomDatabaseRepositoryBase.TABLE_C_EXTEND_PREDICT)
                ...
                + quote(CustomDatabaseRepositoryBase.FIELD_EXTEND_PREDICT_TITLE)
                + " = ?";
        Object[] row = getOneRow(sql, new Object[]{title});
        if (row != null) {
            return (String) row[0];
        }
        return null;
    ...
    @Override
    public Long getIDFromExtendPredict(String title)
            throws KettleDatabaseException {
        String sql = "SELECT "

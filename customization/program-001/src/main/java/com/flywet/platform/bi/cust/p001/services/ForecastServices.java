...
public class ForecastServices extends AbstractRepositoryServices implements
	...
	// 数据上传
	private static final String PROP_UPLOAD_SERVER_SERVER_NAME = "custom.portal.upload.server.serverName";
	private static final String PROP_UPLOAD_SERVER_SERVER_PORT = "custom.portal.upload.server.serverPort";
	private static final String PROP_UPLOAD_SERVER_SERVER_USER_NAME = "custom.portal.upload.server.userName";
	...
	public String monthPredictUpdate(String targetId,
		...
		try {
			...
			attrsMap.addVariable(
					"currentMonthFiles",
					getBrowse(PROP_MONTH_PREDICT_FILE_ROOT_PATH,
	...
	public String monthPredictDataRun(String targetId,
		...
		try {
			...
			TransExecuteWapper wapper = new TransExecuteWapper(
					"sstMonthPredict", tm);
			wapper.putParam("cmd",
					PropertyUtils.getProperty(PROP_FORECAST_EVALUATION_RUN));
	...
	public String extendPredictDataRun(String targetId,
		...
		try {
			...
			TransExecuteWapper wapper = new TransExecuteWapper(
					"sstMonthPredict", tm);
			wapper.putParam("cmd",
					PropertyUtils.getProperty(PROP_PROCESS_FORECAST_RUN));
			TransExecuteQueue.instance().offer(wapper);
	...
	private String getMonthName(String monthCode) {
		if (monthCode != null && monthCode.length() == 6) {
			try {
				...
				return monthCode.substring(0, 4) + "年" + mStr;
			} catch (Exception ex) {
	...
	public String extendSetting(String targetId, HashMap<String, Object> context)
		...
		try {
			...
			String content = Const.NVL(
					prog.getDescriptionFromExtendPredict(currentText), "");
		...
		try {
			...
			if (Utils.isJSEmpty(currentText)) {
				...
				currentText = currentText
						+ YUN_DESC[Long.valueOf(xun).intValue()];
			}
	...
	public String buzNorms(String targetId, HashMap<String, Object> context)
		...
		try {
			...
			attrsMap.addVariable(
					"normFiles",
					getBrowse(PROP_BUZ_NORM_FILE_ROOT_PATH,
							PROP_BUZ_NORM_FILE_CATEGORY, ""));
	...
	public String dataShow(String targetId, HashMap<String, Object> context)
		...
		try {
			...
			attrsMap.addVariable(
					"files",
					getBrowse(PROP_DATA_SHOW_FILE_ROOT_PATH,
			...
			if (rows != null) {
				for (Object[] row : rows) {
					...
					ExtendPredictEvaVo vo = ExtendPredictEvaVo.instance()
							...
							.setS2((Long) row[3]).setS3((Long) row[4])
							.setS4((Long) row[5]).setS5((Long) row[6])
							.setS6((Long) row[7]).setS7((Long) row[8])
							.setS8((Long) row[9]).setS9((Long) row[10]);
					grid.putObject(vo);
				}
			}
	...
	public String monthPredictEvaluateSettingUpdate(String targetId,
		...
		try {
			...
			for (String code : CITY_CODES) {
				detailMap.put(
						code,
						ArrayUtils.concat(new String[] { code },
								vals[cityIdx].split(" ")));
				cityIdx++;
			}
			...
			if (cityL != null) {
				for (int i = 0; i < cityL.size(); i++) {
					...
					rmd.addValue(
							...
							ValueMetaInterface.TYPE_STRING, cityL.get(i));
					rmd.addValue(
	...
	private boolean uploadFile(String rootPath, String fileName,
		...
		String serverName = PropertyUtils
				.getProperty(PROP_UPLOAD_SERVER_SERVER_NAME);
		String serverPort = PropertyUtils
				.getProperty(PROP_UPLOAD_SERVER_SERVER_PORT);
		String userName = PropertyUtils
				.getProperty(PROP_UPLOAD_SERVER_SERVER_USER_NAME);
		String password = Encr.decryptPasswordOptionallyEncrypted(PropertyUtils
				.getProperty(PROP_UPLOAD_SERVER_SERVER_PASSWORD));
		String remoteDirectory = PropertyUtils
				.getProperty(PROP_UPLOAD_SERVER_SERVER_REMOTE_DIRECTORY);

		boolean activeConnection = false;
		int timeout = 100;
		String controlEncoding = "ISO-8859-1";
		...
		try {
			...
			// move to spool dir ...
			if (!Const.isEmpty(remoteDirectory)) {
	...
	public String sstMonthPredictRun(String targetId,
		...
		try {
			...
			TransExecuteWapper wapper = new TransExecuteWapper(
					"sstMonthPredict", tm);
			wapper.putParam("cmd",
					PropertyUtils.getProperty(PROP_MONTH_FORECAST_RUN));
			TransExecuteQueue.instance().offer(wapper);
	...
	public String sstQuarterPredictRun(String targetId,
		...
		try {
			...
			wapper.putParam("cmd",
					PropertyUtils.getProperty(PROP_SEASON_FORECAST_RUN));
			TransExecuteQueue.instance().offer(wapper);

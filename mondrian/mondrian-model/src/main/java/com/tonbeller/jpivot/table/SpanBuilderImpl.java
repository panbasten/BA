...
/**
 * renders a row or column heading. Creates a DOM Element from a Member,
 * Hierarchy, Dimension or Level.
 * 
 * @author av
 */
public class SpanBuilderImpl extends PartBuilderSupport implements SpanBuilder {
	...
	private static final Logger logger = Logger
			.getLogger(SpanBuilderImpl.class);

	/**
	 * creates an instance
	 * 
	 * @param memberName
	 *            either "row-heading" or "column-heading"
	 * @param headingName
	 *            element name for the heading of a row- or column-heading (i.e.
	 *            "heading-heading").
	 */
	...
	class RenderSwitch extends VisitorSupportStrict implements SpanVisitor {
		...
		private void renderMember(Displayable d, String label) {
			...
			if (d instanceof PropertyHolder) {
				...
				Property style = PropertyUtils.getInlineProperty(props,
						PropertyUtils.STYLE_PROPERTY);
				if (style != null) {
					String value = style.getValue().trim();
					if (value.length() > 0)
						elem.setAttribute(PropertyUtils.STYLE_PROPERTY, value
								.toLowerCase());
				}
			}
	...
	class RenderSwitchJo extends VisitorSupportStrict implements SpanVisitor {
		...
		public RenderSwitchJo(SBContext sbctx) {
			this.sbctx = sbctx;
		}

		void renderHeading(Displayable d) {
			elemName = headingName;

			elem = new JSONObject();
			JSONObject caption = table.insert(CAPTION, elem);
			String label = d.getLabel();
			caption.put(CAPTION, label);
			sbctx.setCaptionJo(caption, label);
		}
		...
		private void renderMember(Displayable d, String label) {
			...
			if (d instanceof PropertyHolder) {
				...
				if (style != null) {
					String value = style.getValue().trim();
					if (value.length() > 0)
						elem.put(PropertyUtils.STYLE_PROPERTY, value
								.toLowerCase());
				}
			}
		...
		public Object[] getJo() {
			Object[] jo = new Object[] { elemName, elem };
			return jo;
		}

		public void setElem(String elemName, JSONObject elem) {
	...
	public Object[] buildJo(SBContext sbctx, Span span, boolean even) {
		...
		RenderSwitchJo renderSwitchJo = new RenderSwitchJo(sbctx);

		Visitable v = span.getObject();

		v.accept(renderSwitchJo);
		Object[] elem = renderSwitchJo.getJo();
		for (Iterator it = table.clickableIterator(); it.hasNext();)
			((ClickableMember) it.next()).decorate(sbctx, span.getObject());
		return elem;
	}

 ...
 * @author av
 */
public class RowAxisBuilderImpl extends AxisBuilderSupport implements
		RowAxisBuilder, RowAxisConfig {
	int positionHeader;
	SpanCalc headerSpans;
	private static final Logger logger = Logger
			.getLogger(RowAxisBuilderImpl.class);
	...
	public void initialize(Axis axis) {
		...
		case HIERARCHY_LEVEL_HEADER:
			...
			SpanCalc sc1 = spanCalc
					.createPositionHeader(new IgnorePropertiesHierarchyHeaderFactory());
			SpanCalc sc2 = spanCalc
					.createPositionHeader(new LevelHeaderFactory());
			headerSpans = SpanCalc.appendBelow(sc1, sc2);
			break;
		case LEVEL_HEADER:
			logger.info("LEVEL_HEADER");
			headerSpans = spanCalc
					.createPositionHeader(new LevelHeaderFactory());
			break;
		case HIERARCHY_HEADER:
			logger.info("HIERARCHY_HEADER");
			headerSpans = spanCalc
					.createPositionHeader(new HierarchyHeaderFactory());
			break;
		default:
			headerSpans = null;
			break;

...
public class PopUp implements Renderable {
	...
	private class RendererJo extends Switch {
		private JSONObject parent;
		private int level;

		RendererJo(JSONObject parent, int level) {
			this.parent = parent;
			this.level = level;
		}
		...
		void handleGroupItem(GroupItem item) {
			...
			setItemAttrs(elem, item);
			RendererJo r = new RendererJo(elem, level + 1);
			r.handleList(item.getChildren());

			parent.put("popup-group", elem);
		}

		void handleMenuItem(MenuItem item) {
			JSONObject elem = new JSONObject();
			setItemAttrs(elem, item);
			elem.put("href", item.getHref());
			parent.put("popup-item", elem);
		}

		private void setItemAttrs(JSONObject elem, Item item) {
			elem.put("level", "" + level);
			if (item.getLabel() != null)
				elem.put("label", item.getLabel());
			if (item.getImage() != null)
				elem.put("image", item.getImage());
		}
	}
	...
	public JSONObject renderJo(JSONObject factory) {
		if (isEmpty(items))
			return null;
		// Element popup = factory.createElement("popup-menu");
		...
		popup.put("id", id);
		if (label != null)
			popup.put("label", label);
		if (image != null)
			popup.put("image", image);
		RendererJo r = new RendererJo(popup, 0);
		r.handleList(items);
		return popup;
	}

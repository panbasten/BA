...
import com.yonyou.bq8.di.component.components.flow.FlowChartData;
import com.yonyou.bq8.di.component.components.flow.FlowChartMeta;
import com.yonyou.bq8.di.component.components.flow.FlowElementSet;
import com.yonyou.bq8.di.component.components.flow.FlowHop;
import com.yonyou.bq8.di.component.components.flow.FlowStep;
import com.yonyou.bq8.di.component.components.flow.step.PictureStep;
...
@Path("/transjob")
public class DITransJobResource {
	private final Logger logger = Logger.getLogger(DITransJobResource.class);
	...
	public String saveTrans(@CookieParam("repository") String repository,
			@PathParam("id") String id, String body) throws DIException {
		try {
			...
			throw new DIException("保存转换[" + id + "]出现错误。", ex);
		}
	}
	 ...
	 * @throws DIException
	 */
	private StepMeta newStep(FlowStep flowStep) throws DIException {
		...
		try {
			...
			throw new DIException("创建步骤[" + pluginName + "]出现错误。", ex);
		}
	}
	...
	public String openTransStepEditor(
			...
			@QueryParam("targetId") String targetId) throws DIException {
		try {
			...
			throw new DIException("打开Step的设置页面出现错误。", ex);
		}
	...
	public String createTransPage() throws DIException {
		try {
			...
			BQVariableResolver attrsMap = BQVariableResolver.instance();
			attrsMap.addVariable("editorId", DIBaseResource.ID_EDITOR_TRANS);

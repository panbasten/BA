 ...
 *
 */
@Service("di.resource.fileOperation")
@Path("/fsop")
public class DIFileOperationResource {
    ...
    public String uploadFiles(@Context HttpServletRequest request) throws Exception {
		...
		for (FileItem item : items) {
            ...
            try {
	            String destFileStr = BQFileUtils.dirAppend(workDir, fullFile.getName());
	            FileObject destFileObj = filesysService.composeVfsObject(category, destFileStr, rootId);
	...
	public String createFolder(String body) throws Exception {
		...
		try {
			ParameterContext paramContext = DIWebUtils.fillParameterContext(body);
			...
			FileObject fileObj = filesysService.composeVfsObject(category, workPath+ "/" + dirName, rootId);
	...
	/**
	 * 删除指定的文件项
	 * @param dataStr 文件标识信息，包括path，根目录id，类别等
	 * @return
	 * @throws Exception
	 */
	@DELETE
	...
	public String delete(@QueryParam("data")String dataStr) throws Exception {
		...
		try {
			...
			if (fileObj.getType().equals(FileType.FOLDER) && fileObj.getChildren().length != 0) {
	...
	public void download(@QueryParam("data")String dataStr,
		...
		try {
			...
			String category = dataObj.get("category").toString();
			//拼装文件信息
			FileObject fileObj = filesysService.composeVfsObject(category, workPath, rootId);
			
			is = fileObj.getContent().getInputStream();
			response.setContentType("application/octet-stream");
			response.setHeader("Content-Disposition", "attachment;filename=\"" + fileObj.getName().getBaseName() + "\"");
	...
	public String rename(String body) throws Exception {
		ActionMessage am = new ActionMessage();
		try {
			ParameterContext paramContext = DIWebUtils.fillParameterContext(URLDecoder.decode(body, "utf-8"));
			...
			FileObject oldFileObj = filesysService.composeVfsObject(category, workPath, rootId);
			File newFile = new File(new File(workPath).getParentFile(),newName);
			FileObject newFileObj = filesysService.composeVfsObject(category, newFile.getPath(), rootId);
	...
	/**
	 * 粘贴文件操作
	 * @param currentCase 当前的场景
	 * @param operateCase 被选中用来粘贴文件时的操作场景信息
	 * @return
	 * @throws Exception
	 */
	@GET
	@Path("/paste")
	@Produces(MediaType.APPLICATION_JSON)
	public String paste(@QueryParam("currentCase")String currentCase, @QueryParam("operateCase")String operateCase) throws Exception {
		ActionMessage am = new ActionMessage();
		try {
			JSONObject currentCaseObj = JSONUtils.convertStringToJSONObject(URLDecoder.decode(currentCase, "utf-8"));
			
			long currentRootId = Long.parseLong(currentCaseObj.get("rootId").toString());
			String currentWorkPath = currentCaseObj.get("path").toString();
			String currentCategory = currentCaseObj.get("category").toString();
			
			JSONObject operateCaseObj = JSONUtils.convertStringToJSONObject(URLDecoder.decode(operateCase, "utf-8"));
			long operateRootId = Long.parseLong(operateCaseObj.get("rootId").toString());
			...
			FileObject oldFileObj = filesysService.composeVfsObject(operateCategory, operateWorkPath, operateRootId);
			FileObject newFileObj = filesysService.composeVfsObject(currentCategory, BQFileUtils.directoryComplete(currentWorkPath) + oldFileObj.getName().getBaseName(), currentRootId);

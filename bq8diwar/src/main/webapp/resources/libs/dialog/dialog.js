YonYou.widget.Dialog = function(cfg) {
...
YonYou.extend(YonYou.widget.Dialog, YonYou.widget.BaseWidget);

YonYou.widget.Dialog.prototype.ajax = function() {
	if(this.cfg.url){
		var _self = this;
		YonYou.ab({
			type : "get",
...
YonYou.widget.Dialog.prototype.init = function() {
	this.jq = $(this.jqId);
	
	if(this.cfg.parentId){
		this.parent = $(YonYou.escapeClientId(this.cfg.parentId));
	}else{
	...
	if(this.cfg.footerButtons){
		...
		YonYou.autocw(this.cfg.footerButtons,this.footer);
...
YonYou.widget.Dialog.prototype.createIcon = function(anchorClass,iconClass){
	var a = $("<a href='#'></a>");
...
YonYou.widget.Dialog.prototype.enableModality = function() {
...
YonYou.widget.Dialog.prototype.disableModality = function(){
    $(document.body).children(this.jqId + '_modal').remove();
    $(document).unbind(this.blockEvents).unbind('keydown.dialog');
}

YonYou.widget.Dialog.prototype.syncWindowResize = function() {
...
YonYou.widget.Dialog.prototype.show = function() {
    if(this.visible) {
       return;
    }
...
YonYou.widget.Dialog.prototype._show = function() {
...
YonYou.widget.Dialog.prototype.postShow = function() {   
...
YonYou.widget.Dialog.prototype.hide = function() {   
    if(!this.visible) {
       return;
    }
...
YonYou.widget.Dialog.prototype.focusFirstInput = function() {
    this.jq.find(':not(:submit):not(:button):input:visible:enabled:first').focus();
}

YonYou.widget.Dialog.prototype.bindEvents = function() {   
    var _self = this;
...
YonYou.widget.Dialog.prototype.setupDraggable = function() {    
    this.jq.draggable({
...
YonYou.widget.Dialog.prototype.setupResizable = function() {
    var _self = this;
...
YonYou.widget.Dialog.prototype.initPosition = function() {
    //reset
...
YonYou.widget.Dialog.prototype.onHide = function(event, ui) {
    if(this.cfg.onHide) {
...
YonYou.widget.Dialog.prototype.moveToTop = function() {
    this.jq.css('z-index', ++YonYou.zindex);
}

/**
 * 改变子元素尺寸
 */
YonYou.widget.Dialog.prototype.resizeSub = function() {
...
YonYou.widget.Dialog.prototype.toggleMaximize = function() {
...
YonYou.widget.Dialog.prototype.saveState = function() {
    this.state = {
        width: this.jq.width()
...
YonYou.widget.Dialog.prototype.restoreState = function(includeOffset) {
...
YonYou.widget.Dialog.prototype.loadContents = function() {
    var options = {
        source: this.id,
        process: this.id,
        update: this.id
    ...
    options.onsuccess = function(responseXML) {
        ...
        for(var i=0; i < updates.length; i++) {
            ...
            if(id == _self.id){
                ...
                YonYou.ajax.AjaxUtils.updateElement.call(this, id, content);
            }
        }

        YonYou.ajax.AjaxUtils.handleResponse.call(this, xmlDoc);
    ...
    YonYou.ajax.AjaxRequest(options);
}

/**
 * PrimeFaces ConfirmDialog Widget
 */
YonYou.widget.ConfirmDialog = function(cfg) {
	var _self = this;
	cfg = $.extend({
		footerButtons:null,
		showHeader:true,
	...
	cfg.id=(cfg.id)?cfg.id:"widget_"+(YonYou.windex++);
    cfg.draggable = false;
    cfg.resizable = false;
    cfg.modal = true;
    cfg.realRemove = true;
	    	...
	    	cfg.footerButtons = [{
	    		...
	    		events : {
	        		click:function(event){
	    				YonYou.invokeFunction(cfg.confirmFunc,event,true);
	    				_self.hide();
	    			}
	        	}
	    	},{
	    		...
	    		events : {
		    		click:function(event){
						YonYou.invokeFunction(cfg.confirmFunc,event,false);
						_self.hide();
					}
	        	}
	    	}];
    ...
    YonYou.widget.Dialog.call(this, cfg);
}

YonYou.widget.ConfirmDialog.prototype = YonYou.widget.Dialog.prototype;

YonYou.widget.ConfirmDialog.prototype.changeTitle = function(title){
	this.jq.find("div.ui-dialog-titlebar span").html(title);
}

YonYou.widget.ConfirmDialog.prototype.changeContent = function(img,content){
...
YonYou.widget.ConfirmDialog.prototype.createContentDom =function(cfg){
...
YonYou.widget.PromptDialog=function(cfg){
	var _self = this;
	cfg.id=(cfg.id)?cfg.id:"widget_"+(YonYou.windex++);
    cfg.draggable = false;
    cfg.resizable = false;
    ...
    cfg.footerButtons = [{
		...
		events : {
    		click:function(event){
				YonYou.invokeFunction(cfg.confirmFunc,event,"");
				_self.hide();
			}
    	}
	},{
...
YonYou.widget.PromptDialog.prototype = YonYou.widget.Dialog.prototype;
(function($){
		...
		function wrapMenu(menu){
			menu.addClass('ui-menu').find('>div').each(function(){
				...
				if (item.hasClass('ui-menu-sep')){
					...
					var itemOpts = $.extend({}, YonYou.parseOptions(this,['name','iconCls','href']), {
	...
	$.fn.menu.parseOptions = function(target){
		return $.extend({}, YonYou.parseOptions(target, ['left','top']));
	};
...
YonYou.widget.Menu=function(cfg){
	this.cfg = cfg;
	this.id = cfg.id;
	if(!this.id){
		this.id = "menu_" + (YonYou.windex++);
	}
	...
	function addSubItem(parent, item, _self){
		...
		YonYou.addAttrs($item, item, ["id","name","title","onClick"]);
		parent.append($item);
	}
};
YonYou.extend(YonYou.widget.Menu, YonYou.widget.BaseWidget);


YonYou.widget.Menu.prototype.show=function(e){
	this.jq.menu("show",{
		left: e.pageX,
		top: e.pageY
	});

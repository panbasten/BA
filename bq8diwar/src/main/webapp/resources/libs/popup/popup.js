 ...
 * @date 2012-10-10
 */
YonYou.widget.Popup=function(cfg){
	this.cfg = cfg;
	this.id = this.cfg.id;
	this.jqId = YonYou.escapeClientId(this.id);
	this.jq = $(this.jqId);
	this.target = $(YonYou.escapeClientId(this.cfg.targetId));
	this.init();
};

YonYou.extend(YonYou.widget.Popup, YonYou.widget.BaseWidget);

YonYou.widget.Popup.prototype={
	init:function(){
		var _self=this;
		if(this.jq.length==0){

 ...
 * Clipboard Widget
 */
YonYou.widget.FlowChartClipboard = new function(){
	var steps = [];
	var hops = [];
 ...
 * Resources Widget
 */
YonYou.widget.FlowChartResources = new function(){
	// 资源ID的Index
	var resourceIndex = 0;
...
YonYou.widget.FlowChartUtils = {
	...
	drawText : function(config, ctx, text, x, y, style) {
		if (text) {
			...
			var nf = YonYou.widget.FlowChartUtils.scaleLength(config.scale, 10);
	...
	drawRect : function(ctx,x,y,w,h,style,type){
		...
		ctx.beginPath();
		YonYou.widget.FlowChartUtils.drawLine.factory(ctx,type,x,y,x+w,y);
		YonYou.widget.FlowChartUtils.drawLine.factory(ctx,type,x+w,y,x+w,y+h);
		YonYou.widget.FlowChartUtils.drawLine.factory(ctx,type,x+w,y+h,x,y+h);
		YonYou.widget.FlowChartUtils.drawLine.factory(ctx,type,x,y+h,x,y);
		ctx.stroke();
	},
	...
	drawRemarkRect : function(targetSize,sourceSize,elsSize,ctx) {
		...
		// 画整体元素区域
		YonYou.widget.FlowChartUtils.drawRect(ctx,x,y,w,h,"#999999","dotted");
	...
	drawRopeRect : function(config, ctx) {
		...
		if(r2 && r2 != null){
			...
			if(config.outerControlType=="partMagnify"){
				YonYou.widget.FlowChartUtils.drawText(config,ctx,"+",x,y);
			}
		}
	},
	...
	drawSelectedRect : function(config, ctx, dx, dy, dWidth, dHeight,changable){
		var nc = YonYou.widget.FlowChartUtils.scalePoint(config, {
			x : dx,
			y : dy
		});
		var w = YonYou.widget.FlowChartUtils.scaleLength(config.scale, dWidth);
		var h = YonYou.widget.FlowChartUtils.scaleLength(config.scale, dHeight);
		if(changable){
			ctx.strokeStyle = "#B6B6B6";
			ctx.beginPath();
			YonYou.widget.FlowChartUtils.drawLine.dashTo_(ctx,
	...
	drawArrow : {
		...
		factory : function(x,y,arc,direction,canvasObj,arrowType){
			...
			if(arrowType == "circularity"){
				YonYou.widget.FlowChartUtils.drawArrow.circularity(x,y,arc,direction,canvasObj);
			}
			
			// type2:简单箭头
			else if(arrowType == "simple"){
				YonYou.widget.FlowChartUtils.drawArrow.simple(x,y,arc,direction,canvasObj);
			}
			
			// type3:实心菱形箭头
			else if(arrowType == "solid-diamond"){
				YonYou.widget.FlowChartUtils.drawArrow.solidDiamond(x,y,arc,direction,canvasObj);
			}
			
			// type4:空心菱形箭头
			else if(arrowType == "hollow-diamond"){
				YonYou.widget.FlowChartUtils.drawArrow.hollowDiamond(x,y,arc,direction,canvasObj);
			}
			
			// 通用箭头
			else {
				YonYou.widget.FlowChartUtils.drawArrow.normal(x,y,arc,direction,canvasObj);
			}
		},
		...
		circularity : function(x,y,arc,direction,canvasObj){
			...
			ctx.beginPath();
			var r = YonYou.widget.FlowChartUtils.scaleLength(canvasObj.config.scale, 3);
		...
		simple : function(x,y,arc,direction,canvasObj){
			...
			var side = YonYou.widget.FlowChartUtils.scaleLength(canvasObj.config.scale, 6);
	...
	drawLine : {
		...
		factory : function(ctx, type,x,y,lastX,lastY){
			...
			if(type == "dotted"){
				YonYou.widget.FlowChartUtils.drawLine.dashTo_(ctx, lastX, lastY, x, y, 6, 2);
			}else{
				ctx.moveTo(x,y);
		...
		dashTo_ : function(ctx, x, y, x2, y2, onLength, offLength){
			...
			var segLength = YonYou.widget.FlowChartUtils.getLineLength(dx, dy);
	...
	getMouseCoords : function(e){
		...
		if(jQuery.browser.msie) {
			position.x = position.x - YonYou.widget.FlowChartUtils.getLeft(target);
			position.y = position.y - YonYou.widget.FlowChartUtils.getTop(target);
		} else {
	...
	getMidPoint : function(config,dx,dy,dWidth,dHeight) {
    	var nel = YonYou.widget.FlowChartUtils.scalePoint(config,{
    		x : dx,
    		y : dy
    	});
    	return {
    		x : nel.x + YonYou.widget.FlowChartUtils.scaleLength(config.scale,dWidth) / 2,
        	y : nel.y + YonYou.widget.FlowChartUtils.scaleLength(config.scale,dHeight) / 2
    	};
    }
};
...
(function($) {
    ...
    function FigureModel() {
    	...
    	this.getElValue = function() {
    		...
    		return YonYou.toJSONString(value);
    	};
    	...
    	this.draw = function(o, _offset, canvasObj) {
    		...
    		var ctx = canvasObj.ctx;
    		var sd = YonYou.widget.FlowChartUtils.scalePoint( canvasObj.config, {
    			x : this.dx,
    			y : this.dy
    		});

    		var sdWidth = YonYou.widget.FlowChartUtils.scaleLength(canvasObj.config.scale,this.dWidth);
    		var sdHeight = YonYou.widget.FlowChartUtils.scaleLength(canvasObj.config.scale,this.dHeight);
            ...
            // 写文字
    		YonYou.widget.FlowChartUtils.drawText(canvasObj.config,canvasObj.ctx,this.bText, sd.x + (sdWidth / 2),
    				sd.y + sdHeight + YonYou.widget.FlowChartUtils.scaleLength(canvasObj.config.scale,10) + 4,
    				this.bTextStyle);
    		
    		YonYou.widget.FlowChartUtils.drawText( canvasObj.config, canvasObj.ctx, this.cText, sd.x + (sdWidth / 2),
					sd.y + (sdHeight / 2), this.cTextStyle);

    		ctx.restore();
            
    		if(this.state != "show"){
    			YonYou.widget.FlowChartUtils.drawSelectedRect(canvasObj.config,canvasObj.ctx,this.dx,this.dy,this.dWidth,this.dHeight,this.changable);
    		}
    	...
    	this.drawRectangular = function(canvasObj,sd,sdWidth,sdHeight){
    		...
    		var spr = YonYou.widget.FlowChartUtils.scaleLength(canvasObj.config.scale,this.pr);
    	...
    	this.drawCircle = function(canvasObj,sd,sdWidth,sdHeight){
    		...
    		var r = YonYou.widget.FlowChartUtils.scaleLength(canvasObj.config.scale,this.r);
    	...
    	this.drawConcentric = function(canvasObj,sd,sdWidth,sdHeight){
    		...
    		var r = YonYou.widget.FlowChartUtils.scaleLength(canvasObj.config.scale,this.r);
    		var r2 = YonYou.widget.FlowChartUtils.scaleLength(canvasObj.config.scale,this.r2);
    ...
    function PictureModel() {
    	...
    	this.draw = function(o, _offset, canvasObj) {
    		...
    		if(this.imgId && this.imgId != ""){
    			...
    			imgObj = YonYou.widget.FlowChartResources.addAndFindResource(this.imgSrc);
    		}
    		...
    		YonYou.widget.FlowChartUtils.drawText(canvasObj.config, ctx, this.bText, sd.x + (sdWidth / 2), sd.y + sdHeight
    				+ YonYou.widget.FlowChartUtils.scaleLength(canvasObj.config.scale, 10) + 4, this.bTextStyle);
    		YonYou.widget.FlowChartUtils.drawText(canvasObj.config, ctx, this.tlText, sd.x, sd.y, this.tlTextStyle);
    ...
    function HopModel() {
		...
		this.draw = function(o, _offset, canvasObj, flowObj) {
			...
			if(this.fromElId!=null&&this.fromElId!=""){
				...
				felc = YonYou.widget.FlowChartUtils.getMidPoint(canvasObj.config,fel.el.dx,fel.el.dy,fel.el.dWidth,fel.el.dHeight);
				this.x[0]=felc.x;
				this.y[0]=felc.y;
			}
			// 得到结束对象,更新结束点
			if(this.toElId!=null&&this.toElId!=""){
				...
				telc = YonYou.widget.FlowChartUtils.getMidPoint(canvasObj.config,tel.el.dx,tel.el.dy,tel.el.dWidth,tel.el.dHeight);
				this.x[len-1]=telc.x;
				this.y[len-1]=telc.y;
			}
			var fc=null,tc=null;
			...
			// 大于两个节点
				...
				nc = YonYou.widget.FlowChartUtils.scalePoint(canvasObj.config, {
					x : this.x[1],
					y : this.y[1]
				});
			}
			
			fc = this.getLineShortedCoords(canvasObj.config.scale,YonYou.widget.FlowChartUtils.scaleLength(canvasObj.config.scale, fel.el.dWidth),YonYou.widget.FlowChartUtils.scaleLength(canvasObj.config.scale, fel.el.dHeight),{
				x : felc.x,
				y : felc.y,
				x2 : nc.x,
				y2 : nc.y
			...
			if(this.toElId!=null && this.toElId!=""){
				...
				tc = this.getLineShortedCoords(canvasObj.config.scale,YonYou.widget.FlowChartUtils.scaleLength(canvasObj.config.scale, tel.el.dWidth),YonYou.widget.FlowChartUtils.scaleLength(canvasObj.config.scale, tel.el.dHeight),{
					x : telc.x,
					y : telc.y,
					x2 : nc.x,
					y2 : nc.y
			...
			if(len<=2 && this.fromElId == this.toElId){
				var r = YonYou.widget.FlowChartUtils.scaleLength(canvasObj.config.scale, 20);
				...
				if(this.arrowStartType && this.arrowStartType!=""){
					YonYou.widget.FlowChartUtils.drawArrow.factory( (this.x[0]), (this.y[0]), -1, false, canvasObj, this.arrowStartType);
				}
				if(this.arrowEndType && this.arrowEndType!=""){
					YonYou.widget.FlowChartUtils.drawArrow.factory( (this.x[0]-ot), (this.y[0]-r+ot), 1, true, canvasObj, this.arrowEndType);
				}
			}else{
				...
				for(var i=1;i<len-1;i++){
					...
					YonYou.widget.FlowChartUtils.drawLine.factory(ctx, this.lineType,nc.x,nc.y,lastX,lastY);
				...
				if(tc!=null){
					YonYou.widget.FlowChartUtils.drawLine.factory(ctx, this.lineType,tc.x,tc.y,lastX,lastY);
					...
					if(this.arrowStartType && this.arrowStartType!=""){
						...
						if(len==2){
							YonYou.widget.FlowChartUtils.drawArrow.factory(fc.x, fc.y, (fc.y-tc.y)/(fc.x-tc.x), (fc.x>=tc.x), canvasObj, this.arrowStartType);
						}else{
							var nc2 = YonYou.widget.FlowChartUtils.scalePoint(canvasObj.config, {
								x : this.x[1],
								y : this.y[1]
							});
							YonYou.widget.FlowChartUtils.drawArrow.factory(fc.x, fc.y, (fc.y-nc2.y)/(fc.x-nc2.x), (fc.x>=nc2.x), canvasObj, this.arrowStartType);
						}
					}
					
					if(this.arrowEndType && this.arrowEndType!=""){
						YonYou.widget.FlowChartUtils.drawArrow.factory(tc.x, tc.y, (tc.y-nc.y)/(tc.x-nc.x), (tc.x>=nc.x), canvasObj, this.arrowEndType);
					}
					...
					YonYou.widget.FlowChartUtils.drawLine.factory(ctx, this.lineType,this.x[len-1],this.y[len-1],lastX,lastY);
				}
			...
			if(len<=2 && this.fromElId == this.toElId){
				...
				YonYou.widget.FlowChartUtils.drawText(canvasObj.config, canvasObj.ctx, this.text, (nh.x+nh2.x)/2, (nh.y+nh2.y)/2 - 2*r, this.textStyle);
			}else{
				...
				if(fn!=0&&fn!=(len-1)){
					nh = YonYou.widget.FlowChartUtils.scalePoint(canvasObj.config, {
						x : this.x[fn],
						y : this.y[fn]
					});
				}else{
				...
				if(tn!=0&&tn!=(len-1)){
					nh2 = YonYou.widget.FlowChartUtils.scalePoint(canvasObj.config, {
						x : this.x[tn],
						y : this.y[tn]
					});
				}else{
				...
				YonYou.widget.FlowChartUtils.drawText(canvasObj.config, canvasObj.ctx, this.text, (nh.x+nh2.x)/2, (nh.y+nh2.y)/2, this.textStyle);
			}
		...
		this.getAddJoinBlockPoint = function(canvasObj,flowObj, c){
			...
			if(this.fromElId==this.toElId && len<=2){
				r = YonYou.widget.FlowChartUtils.scaleLength(canvasObj.config.scale, 20);
				d = YonYou.widget.FlowChartUtils.getDistancePointToCircle(c,{
					x : x[0],
					y : y[0]-r
				},r);
				if(d<5){
				...
				for(var j=1;j<len;j++){
					if((j-1)==0){
						...
						np = YonYou.widget.FlowChartUtils.scalePoint(canvasObj.config, {
							x : x[j-1],
							y : y[j-1]
						});
					}
					...
					if(j==(len-1)){
						...
						np2 = YonYou.widget.FlowChartUtils.scalePoint(canvasObj.config, {
							x : x[j],
							y : y[j]
						});
					}
					if( 
						...
						d = YonYou.widget.FlowChartUtils.getDistancePointToLine(c,{
							x : np.x,
							y : np.y,
							x2 : np2.x,
							y2 : np2.y
		...
		this.getLineShortedCoords = function(scale,dWidth,dHeight,line){
			...
			var buffer = YonYou.widget.FlowChartUtils.scaleLength(scale, 6);
			var x,y;
			var X,Y;
			// 竖线
			if(Math.abs(dx)<5){
		...
		this.getJoinBlockByCoords = function(canvasObj, c){
			...
			for(var j=1;j<len-1;j++){
				ns = YonYou.widget.FlowChartUtils.scalePoint(canvasObj.config, {
					x : this.x[j],
					y : this.y[j]
				});
		...
		this.drawStartAndEndText = function(x,y,arc,direction,canvasObj,text){
			...
			var bufX=0,bufY=0;
			var buf = YonYou.widget.FlowChartUtils.scaleLength(canvasObj.config.scale, 10);
			...
			if(direction){
				YonYou.widget.FlowChartUtils.drawText(canvasObj.config, canvasObj.ctx, text, x-bufX, y-bufY, this.textStyle);
			}else{
				YonYou.widget.FlowChartUtils.drawText(canvasObj.config, canvasObj.ctx, text, x+bufX, y+bufY, this.textStyle);
			}
		};
    ...
    function FlowObject() {
		...
		this.getElsValue = function() {
			var defaultAttributes = YonYou.toJSONString(this.config.defaultAttributes);
			...
			if (this.config.extendData) {
				otherStr = otherStr + ',"extendData":' +  YonYou.toJSONString(this.config.extendData);
			}
		...
		this.dealCreateHop = function(mouseType){
			...
			// 鼠标移动，生成连接线
				...
				// 添加Hop连接点
					...
					var om = YonYou.widget.FlowChartUtils.arcScalePoint(this.canvasObj.config, {
						x : mouseUp.x,
						y : mouseUp.y
					});
					var stateHopX = this.config.stateHop.x;
		...
		this.dealMove = function(c){
			// 转换成正常比例下的鼠标
			var p = YonYou.widget.FlowChartUtils.getCloseGridPoint(c,this.canvasObj.config);
			var om = YonYou.widget.FlowChartUtils.arcScalePoint(this.canvasObj.config, {
				x : p.x,
				y : p.y
			});
			...
			if (this.config.canvasEls.steps.length > 0) {
				for ( var i = 0; i < this.config.canvasEls.steps.length; i++) {
					...
					if(v && v != undefined){
						...
						YonYou.widget.FlowChartUtils.drawRect(this.canvasObj.octx,x,y,w,w,"#dc003f");
					}
				}
			}
		};
		...
		this.setElsByRope = function(rope){
			if(rope!=null){
				...
				for(var i=0;i<steps.length;i++){
					...
					nc = YonYou.widget.FlowChartUtils.scalePoint(this.canvasObj.config, {
						x : v.dx,
						y : v.dy
					});
					ndWidth = YonYou.widget.FlowChartUtils.scaleLength(this.canvasObj.config.scale, v.dWidth);
					ndHeight = YonYou.widget.FlowChartUtils.scaleLength(this.canvasObj.config.scale, v.dHeight);
					// 表示在圈选框中
		...
		this.getChangeBlock = function(c){
			...
			for(var i=els.length-1;i>=0;i--){
				...
				if(els[i] && els[i]!=undefined){
					...
					no = YonYou.widget.FlowChartUtils.scalePoint(this.canvasObj.config, {
						x : o.dx,
						y : o.dy
					});
					...
					no1 = {
						x : no.x+YonYou.widget.FlowChartUtils.scaleLength(this.canvasObj.config.scale, o.dWidth),
						y : no.y+YonYou.widget.FlowChartUtils.scaleLength(this.canvasObj.config.scale, o.dHeight)
					};
	    ...
	    this.getPosition = function(o, type){
	    	if(type=="step"){
	    		if (o){
	    			var no = YonYou.widget.FlowChartUtils.scalePoint(this.canvasObj.config, {
						x : o.dx,
						y : o.dy
					});
	    			
	    			no.x2 = no.x + YonYou.widget.FlowChartUtils.scaleLength(this.canvasObj.config.scale, o.dWidth);
	    			no.y2 = no.y + YonYou.widget.FlowChartUtils.scaleLength(this.canvasObj.config.scale, o.dHeight);
	    ...
	    this.appendEl = function(el,type){
			...
			if(type == "step"){
				if(el.cloneEl){
					...
					var newp = YonYou.widget.FlowChartUtils.arcScalePoint(this.canvasObj.config,{
		...
		this.copyEls = function(){
			...
			YonYou.widget.FlowChartClipboard.copyToClipboard({
				steps : steps,
				hops : hops
			});
		};
		...
		this.pasteEls = function(){
			var el = YonYou.widget.FlowChartClipboard.getFromClipboard();
			var steps = el.steps;
			var hops = [];
    ...
    function FlowChart() {
    	...
    	this.constructCanvas = function(){
			this.canvasDiv = jQuery(YonYou.escapeClientId(this.clientId));
			this.canvas = this.canvasDiv.find(YonYou.escapeClientId(this.clientId+"_canvas")).get(0);
			if(this.canvas){
			}else{
			...
			if(this.oclientId){
				...
				this.ocanvasDiv = jQuery(YonYou.escapeClientId(this.oclientId));
		...
		this.constructCanvasDiv = function(){
			...
			this.canvasDiv.mousedown(function(e){
				...
				if (e.which != 1) {return;}
				// 取得按下鼠标的坐标
				var mouseCoords = YonYou.widget.FlowChartUtils.getMouseCoords(e);
			...
			if(this.hasOverview){
				...
				this.ocanvasDiv.mousedown(function(e){
					...
					if (e.which != 1) {return;}
					// 取得按下鼠标的坐标
					var mouseCoords = YonYou.widget.FlowChartUtils.getMouseCoords2(e);
		...
		this.dealPartMagnify = function(mouseType){
			...
			if(mouseType == "mouseUp"){
				...
				var ns = YonYou.widget.FlowChartUtils.arcScalePoint(this.config, {
					x : mouseDown.x,
					y : mouseDown.y
				});
				var ne = YonYou.widget.FlowChartUtils.arcScalePoint(this.config, {
					x : mouseUp.x,
					y : mouseUp.y
				});
		...
		this.redraw = function(){
			...
			// 重画圈选框
			YonYou.widget.FlowChartUtils.drawRopeRect(this.config,this.ctx);
			...
			if(this.config.showGrid){
				...
				for(var i=tx0;i<=this.config.canvasConfig.width;i=i+distance){
					...
					for(var j=ty0;j<=this.config.canvasConfig.height;j=j+distance){
						YonYou.widget.FlowChartUtils.drawCross({
							x : i,
							y : j
						},this.ctx);
						c.push({
		...
		this.redrawOverview = function(){
			...
			// 重画覆盖框
			var oconfig = YonYou.widget.FlowChartUtils.drawRemarkRect(this.oconfig.canvasConfig,this.config.canvasConfig,els,this.octx);
...
YonYou.widget.FlowChart = function(cfg) {
    this.cfg = cfg;
    this.id = this.cfg.id;
...
YonYou.extend(YonYou.widget.FlowChart,YonYou.widget.BaseWidget);

YonYou.widget.FlowChart.prototype.changeSize = function(w, h){
...
YonYou.widget.FlowChart.prototype.flush = function(exdata){
	 this.oraData = exdata;

YonYou.widget.FusionCharts = function(cfg){
	this.cfg = $.extend({autoOpen:true},cfg);
	this.id = this.cfg.id;
	this.jq = $(YonYou.escapeClientId(this.id));	
...
YonYou.extend(YonYou.widget.FusionCharts, YonYou.widget.BaseWidget);

YonYou.widget.FusionCharts.prototype = {
	...
	render: function(elementId){
		if(elementId){
			this.jq = (typeof elementId == 'string') ? $(YonYou.escapeClientId(elementId)) : $(elementId);
		}
...
YonYou.widget.FusionChartsUtil.cleanupSWFs = function() {
...
// Fixes bug in fp9
YonYou.widget.FusionChartsUtil.prepUnload = function() {
	__flash_unloadHandler = function(){};
	__flash_savedUnloadHandler = function(){};
	if (typeof window.onunload == 'function') {
		var oldUnload = window.onunload;
		window.onunload = function() {
			YonYou.widget.FusionChartsUtil.cleanupSWFs();
			oldUnload();
		}
	} else {
		window.onunload = YonYou.widget.FusionChartsUtil.cleanupSWFs;
	}
};
...
if (typeof window.onbeforeunload == 'function') {
	...
	window.onbeforeunload = function() {
		YonYou.widget.FusionChartsUtil.prepUnload();
		oldBeforeUnload();
	}
} else {
	window.onbeforeunload = YonYou.widget.FusionChartsUtil.prepUnload;
}
...
YonYou.widget.FusionChartsUtil.getChartObject = function(id)
{
...
*/
YonYou.widget.FusionChartsUtil.updateChartXML = function(chartId, strXML){
	//Get reference to chart object				
	var chartObj = YonYou.widget.FusionChartsUtil.getChartObject(chartId);		
	//Set dataURL to null

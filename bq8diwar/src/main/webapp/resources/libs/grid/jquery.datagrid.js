	...
	function wrapGrid(target, rownumbers) {
		...
		function getColumns() {
			...
			$(target).children("thead").each(
				function() {
					var opt = YonYou.parseOptions(this, [ {
						frozen : "boolean"
					} ]);
					$(this).find("tr").each(
					function() {
...
YonYou.widget.Grid = function(cfg) {
	this.cfg = cfg;
	this.id = this.cfg.id;
...
YonYou.extend(YonYou.widget.Grid, YonYou.widget.BaseWidget);

 ...
 * yanghyc
 */
YonYou.widget.BrowsePanel=function(cfg){
	this.cfg = cfg;
	this.id = this.cfg.id;
...
YonYou.extend(YonYou.widget.BrowsePanel, YonYou.widget.BaseWidget);
YonYou.widget.BrowsePanel.prototype={
	init:function(){
		if(!this.container){
		...
		for(var i=0;i<items.length;i++){
			...
			itemDom.bind("click",item,function(event){
				...
				if(event.data && event.data.events && event.data.events["click"]){
					YonYou.invokeFunction(event.data.events["click"],event,event.data);
				}
			});
			...
			itemDom.bind("mouseover",item,function(event){
				...
				if(event.data && event.data.events && event.data.events["mouseover"]){
					YonYou.invokeFunction(event.data.events["mouseover"],event,event.data);
				}
			});
			...
			itemDom.bind("mouseout",item,function(event){
				...
				if(event.data && event.data.events && event.data.events["mouseout"]){
					YonYou.invokeFunction(event.data.events["mouseout"],event,event.data);
				}
			});
			
			// other event
			YonYou.attachBehaviors(itemDom,YonYou.assembleBehaviors(item.attrs.events,["click","mouseover","mouseout"]),item);

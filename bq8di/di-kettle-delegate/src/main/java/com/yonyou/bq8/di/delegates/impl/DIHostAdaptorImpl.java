...
@DIDelegate(type="db")
public class DIHostAdaptorImpl extends AbstractDbAdaptor implements DIHostAdaptor{
	...
	private final String QUERY_HOST = "SELECT "
		...
		+ ","
		+ KettleDatabaseRepositoryBase.FIELD_HOST_CODE
		+ ","
	...
	public Object[] getHostById(long id) throws DIKettleException {
		try {
			String sql = QUERY_HOST + 
				" WHERE " + KettleDatabaseRepositoryBase.FIELD_HOST_ID_HOST + " = " + id;
	...
	@Override
	public List<Object[]> getHostsByType(String type) throws DIKettleException {
		try {
			String sql = QUERY_HOST + 
				" WHERE " + KettleDatabaseRepositoryBase.FIELD_HOST_TYPE + " = '" + type + "'";
			return getRows(sql);
		} catch (KettleException e) {
			throw new DIKettleException(e);
		}
	...
	public void addHost(DIHost host) throws DIKettleException {
		try {
			...
			params.add(String.valueOf(this.getNextBatchId(KettleDatabaseRepositoryBase.TABLE_R_HOST,KettleDatabaseRepositoryBase.FIELD_HOST_ID_HOST)));

...
@DIDelegate(type="db")
public class DIFunctionTypeDbAdaptorImpl extends AbstractDbAdaptor implements DIFunctionTypeAdaptor {
	private final Logger logger = Logger.getLogger(DIFunctionTypeDbAdaptorImpl.class);
	
	/*@Override
	public List<Object[]> getFunctionLevelOne() throws DIKettleException {
		try {
			String sql = "SELECT "
		...
		try {
			String sql = "SELECT "
			 ...
			 for (RowMetaAndData rmd : rmds) {
				 ...
				 ft.setId(rmd.getInteger(KettleDatabaseRepositoryBase.FIELD_FUNC_TYPE_ID_FUNC_TYPE));
				 ft.setCode(rmd.getString(KettleDatabaseRepositoryBase.FIELD_FUNC_TYPE_CODE, null));
				 ft.setDesc(rmd.getString(KettleDatabaseRepositoryBase.FIELD_FUNC_TYPE_DESCRIPTION, null));
				 ft.setParentId(rmd.getInteger(KettleDatabaseRepositoryBase.FIELD_FUNC_TYPE_ID_FUNC_TYPE_PARENT));
				 ft.setHelpText(rmd.getString(KettleDatabaseRepositoryBase.FIELD_FUNC_TYPE_HELPTEXT, null));
				 ft.setIndex(Integer.parseInt(rmd.getString(KettleDatabaseRepositoryBase.FIELD_FUNC_TYPE_TYPE_INDEX, "0")));
				 
				 String extSql = "SELECT " + KettleDatabaseRepositoryBase.FIELD_FUNC_TYPE_EXTATTR_FID
				 				...
				 				+ " FROM " + KettleDatabaseRepositoryBase.TABLE_R_FUNC_TYPE_EXTATTR
				 				+ " WHERE " + KettleDatabaseRepositoryBase.FIELD_FUNC_TYPE_EXTATTR_FID 
				 				+ " = " + ft.getId();
				 List<RowMetaAndData> extRmds = getRowsWithMeta(extSql);
				 if (!Utils.isEmpty(extRmds)) {
					 for (RowMetaAndData extRmd : extRmds) {
						 NameValuePair pair = new NameValuePair();
						 pair.setName(extRmd.getString(KettleDatabaseRepositoryBase.FIELD_FUNC_TYPE_EXTATTR_ATTR_NAME, null));
						 pair.setValue(extRmd.getString(KettleDatabaseRepositoryBase.FIELD_FUNC_TYPE_EXTATTR_ATTR_VALUE, null));
						 ft.addExtAttr(pair);
					 }
				 }
				 fts.add(ft);

package com.yonyou.bq8.di.component.resolvers.selectMenu;
...
public class BQSelectMenuResolver extends BaseComponentResolver implements
	...
	public void renderSub(Node node, HTMLWriter html, List<String> script,
		...
		try {
			...
			if (onchange != null || interaction != null) {
				onchange = (interaction != null) ? "YonYou.interaction.action(this.value,"
			...
			throw new DIPageException("SelectMenu解析出现错误。");
		}
	}

	private void getOptions(Node node, HTMLWriter html, List<String> script,
			BQVariableResolver attrs, String val) {
		...
		for (int i = 0; i < nodeList.getLength(); i++) {
			...
			if (HTML.COMPONENT_TYPE_OPTION.equalsIgnoreCase(subNodeName)) {
				...
				if (items != null && items.size() > 0) {
					BQVariableResolver optionsAttrs = new BQVariableResolver();
					String value, label;

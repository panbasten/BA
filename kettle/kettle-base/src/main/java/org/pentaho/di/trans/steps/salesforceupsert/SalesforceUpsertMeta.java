...
public class SalesforceUpsertMeta extends BaseStepMeta implements
		StepMetaInterface {
	private static Class<?> PKG = SalesforceUpsertMeta.class; // for i18n
																// purposes,
																// needed by
																// Translator2!!
																// $NON-NLS-1$
	...
	public String getXML() {
		...
		retval.append("    "
				+ XMLHandler.addTagValue("upsertfield", UpsertField));
		retval.append("    " + XMLHandler.addTagValue("batchSize", batchSize));
		retval.append("    "
				+ XMLHandler.addTagValue("salesforceIDFieldName",
		...
		for (int i = 0; i < updateLookup.length; i++) {
			...
			retval
					.append("        ").append(XMLHandler.addTagValue("field", updateStream[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval.append("        ").append(
					XMLHandler.addTagValue("useExternalId", useExternalId[i]
							.booleanValue()));
			retval.append("      </field>").append(Const.CR); //$NON-NLS-1$
		}
	...
	private void readData(Node stepnode) throws KettleXMLException {
		try {
			...
			salesforceIDFieldName = XMLHandler.getTagValue(stepnode,
					"salesforceIDFieldName");
			...
			for (int i = 0; i < nrFields; i++) {
				...
				if (updateStream[i] == null)
					updateStream[i] = updateLookup[i]; // default: the same
														// name!
				String updateValue = XMLHandler.getTagValue(fnode,
						"useExternalId"); //$NON-NLS-1$
				if (updateValue == null) {
					// default FALSE
			...
			rollbackAllChangesOnError = "Y".equalsIgnoreCase(XMLHandler
					.getTagValue(stepnode, "rollbackAllChangesOnError"));
		} catch (Exception e) {
	...
	public void getFields(RowMetaInterface r, String name,
		...
		String realfieldname = space
				.environmentSubstitute(getSalesforceIDFieldName());
		if (!Const.isEmpty(realfieldname)) {
			ValueMetaInterface v = new ValueMeta(realfieldname,
	...
	public void readRep(Repository rep, ObjectId id_step,
		...
		try {
			...
			salesforceIDFieldName = rep.getStepAttributeString(id_step,
					"salesforceIDFieldName");
			...
			for (int i = 0; i < nrFields; i++) {
				...
				updateStream[i] = rep.getStepAttributeString(id_step, i,
						"field_attribut"); //$NON-NLS-1$
				useExternalId[i] = Boolean.valueOf(rep.getStepAttributeBoolean(
						id_step, i, "field_useExternalId", false));
			}
			useCompression = rep.getStepAttributeBoolean(id_step,
					"useCompression");
			timeout = rep.getStepAttributeString(id_step, "timeout");
			rollbackAllChangesOnError = rep.getStepAttributeBoolean(id_step,
					"rollbackAllChangesOnError");
		} catch (Exception e) {
			throw new KettleException(BaseMessages.getString(PKG,
					"SalesforceUpsertMeta.Exception.ErrorReadingRepository"), e);
		}
	}
	...
	public void saveRep(Repository rep, ObjectId id_transformation,
		...
		try {
			rep.saveStepAttribute(id_transformation, id_step, "targeturl",
					targeturl);
			rep.saveStepAttribute(id_transformation, id_step, "batchSize",
					batchSize);
			rep.saveStepAttribute(id_transformation, id_step, "module", module);
			rep.saveStepAttribute(id_transformation, id_step, "upsertfield",
					UpsertField);
			...
			rep.saveStepAttribute(id_transformation, id_step,
					"salesforceIDFieldName", salesforceIDFieldName);

			for (int i = 0; i < updateLookup.length; i++) {
				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_name", updateLookup[i]); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_attribut", updateStream[i]); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_useExternalId", useExternalId[i].booleanValue());
			...
			rep.saveStepAttribute(id_transformation, id_step, "timeout",
					timeout);
			rep.saveStepAttribute(id_transformation, id_step,
					"rollbackAllChangesOnError", rollbackAllChangesOnError);
		} catch (Exception e) {
			throw new KettleException(BaseMessages.getString(PKG,
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (input.length > 0)
			cr = new CheckResult(
					...
					BaseMessages.getString(PKG,
							"SalesforceUpsertMeta.CheckResult.NoInputExpected"),
					stepMeta);
		else
			cr = new CheckResult(CheckResult.TYPE_RESULT_OK,
					BaseMessages.getString(PKG,
							"SalesforceUpsertMeta.CheckResult.NoInput"),
					stepMeta);
		remarks.add(cr);
		...
		if (Const.isEmpty(targeturl))
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, BaseMessages
					.getString(PKG, "SalesforceUpsertMeta.CheckResult.NoURL"),
					stepMeta);
		else
			cr = new CheckResult(CheckResult.TYPE_RESULT_OK, BaseMessages
					.getString(PKG, "SalesforceUpsertMeta.CheckResult.URLOk"),
					stepMeta);
		remarks.add(cr);
		...
		if (Const.isEmpty(username))
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, BaseMessages
					.getString(PKG,
							"SalesforceUpsertMeta.CheckResult.NoUsername"),
					stepMeta);
		else
			cr = new CheckResult(CheckResult.TYPE_RESULT_OK, BaseMessages
					.getString(PKG,
							"SalesforceUpsertMeta.CheckResult.UsernameOk"),
					stepMeta);
		remarks.add(cr);
		...
		if (Const.isEmpty(module))
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG,
							"SalesforceUpsertMeta.CheckResult.NoModule"),
					stepMeta);
		else
			cr = new CheckResult(CheckResult.TYPE_RESULT_OK,
					BaseMessages.getString(PKG,
							"SalesforceUpsertMeta.CheckResult.ModuleOk"),
					stepMeta);
		remarks.add(cr);
		...
		if (updateLookup.length == 0)
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG,
							"SalesforceUpsertMeta.CheckResult.NoFields"),
					stepMeta);
		else
			cr = new CheckResult(CheckResult.TYPE_RESULT_OK,
					BaseMessages.getString(PKG,
							"SalesforceUpsertMeta.CheckResult.FieldsOk"),
					stepMeta);
		remarks.add(cr);
	...
	public StepInterface getStep(StepMeta stepMeta,
			...
			Trans trans) {
		return new SalesforceUpsert(stepMeta, stepDataInterface, cnr,
				transMeta, trans);
	}

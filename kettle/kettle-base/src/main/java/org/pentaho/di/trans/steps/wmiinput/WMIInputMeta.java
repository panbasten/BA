...
public class WMIInputMeta extends BaseStepMeta implements StepMetaInterface {
	private static Class<?> PKG = WMIInputMeta.class; // for i18n purposes,
														// needed by
														// Translator2!!
														// $NON-NLS-1$
	...
	public void getFields(RowMetaInterface row, String origin,
		...
		try {
			...
			throw new KettleStepException(BaseMessages.getString(PKG,
					"WMIInputMeta.GetFields.Error", wmq), e);
		} finally {
	...
	public void readRep(Repository rep, ObjectId id_step,
		...
		try {
			...
			password = Encr.decryptPasswordOptionallyEncrypted(rep
					.getStepAttributeString(id_step, "password"));
	...
	public void saveRep(Repository rep, ObjectId id_transformation,
		...
		try {
			rep.saveStepAttribute(id_transformation, id_step, "wmiQuery",
					wmiQuery);
			rep
					.saveStepAttribute(id_transformation, id_step, "limit",
							rowLimit);
			rep.saveStepAttribute(id_transformation, id_step,
					"variables_active", variableReplacementActive);
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (input.length > 0) {
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR,
					"Step is not expecting info from input steps.", stepMeta);
			remarks.add(cr);
		} else {

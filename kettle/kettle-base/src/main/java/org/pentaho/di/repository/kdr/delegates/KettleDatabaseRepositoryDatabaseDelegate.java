...
public class KettleDatabaseRepositoryDatabaseDelegate extends KettleDatabaseRepositoryBaseDelegate {
    ...
    public synchronized Collection<RowMetaAndData> getDatabases() throws KettleException
    {
    	List<RowMetaAndData> dbs = new ArrayList<RowMetaAndData>();
    	String sql = "SELECT * FROM "+quoteTable(KettleDatabaseRepository.TABLE_R_DATABASE);
    	List<Object[]> rows = repository.connectionDelegate.getRows(sql);
    	for (Object[] row : rows) 
    	{
    		RowMetaAndData rowWithMeta = new RowMetaAndData(repository.connectionDelegate.getReturnRowMeta(), row);
    			dbs.add(rowWithMeta);

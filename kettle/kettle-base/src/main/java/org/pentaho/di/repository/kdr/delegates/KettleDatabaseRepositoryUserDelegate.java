...
public class KettleDatabaseRepositoryUserDelegate extends KettleDatabaseRepositoryBaseDelegate {
	...
	public IUser loadUserInfo(ObjectId id_user) throws KettleException {
		try {
			RowMetaAndData r = getUser(id_user);
			if (r!=null) {
				IUser userInfo = new UserInfo();
				userInfo.setObjectId(new LongObjectId(r.getInteger("ID_USER", 0)));
				...
				throw new KettleDatabaseException(BaseMessages.getString(PKG, "UserInfo.Error.UserNotFound", id_user));
			}
		} catch(KettleDatabaseException dbe) {
			throw new KettleException(BaseMessages.getString(PKG, "UserInfo.Error.UserNotLoaded", id_user, ""), dbe);
		}
	}

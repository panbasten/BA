...
public class SalesforceInputMeta extends BaseStepMeta implements
	...
	private static Class<?> PKG = SalesforceInputMeta.class; // for i18n
																// purposes,
																// needed by
																// Translator2!!
																// $NON-NLS-1$
	...
	public String getXML() {
		...
		retval.append("    "
				+ XMLHandler.addTagValue("specifyQuery", specifyQuery));
		...
		retval.append("    "
				+ XMLHandler.addTagValue("include_deletion_date",
						includeDeletionDate));

		retval.append("    "
				+ XMLHandler.addTagValue("deletion_date_field",
						deletionDateField));
		retval.append("    "
		...
		retval.append("    "
				+ XMLHandler.addTagValue("records_filter",
						SalesforceConnectionUtils
								.getRecordsFilterCode(recordsFilter)));
		retval.append("    "
				+ XMLHandler.addTagValue("useCompression", useCompression));
	...
	private void readData(Node stepnode) throws KettleXMLException {
		try {
			...
			specifyQuery = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "specifyQuery"));
			...
			includeDeletionDate = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "include_deletion_date"));
			rowNumberField = XMLHandler.getTagValue(stepnode, "rownum_field");
			deletionDateField = XMLHandler.getTagValue(stepnode,
					"deletion_date_field");
			...
			recordsFilter = SalesforceConnectionUtils
					.getRecordsFilterByCode(Const.NVL(XMLHandler.getTagValue(
							stepnode, "records_filter"), ""));
			useCompression = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "useCompression"));
			queryAll = "Y".equalsIgnoreCase(XMLHandler.getTagValue(stepnode,
					"queryAll"));
	...
	public void getFields(RowMetaInterface r, String name,
		...
		if (includeTimestamp && !Const.isEmpty(timestampField)) {
			ValueMetaInterface v = new ValueMeta(space
					.environmentSubstitute(timestampField), ValueMeta.TYPE_DATE);
			v.setOrigin(name);
			r.addValueMeta(v);
		}
		...
		if (includeDeletionDate && !Const.isEmpty(deletionDateField)) {
			ValueMetaInterface v = new ValueMeta(space
					.environmentSubstitute(deletionDateField),
					ValueMeta.TYPE_DATE);
			v.setOrigin(name);
			r.addValueMeta(v);
		}
	...
	public void readRep(Repository rep, ObjectId id_step,
		...
		try {
			...
			includeRowNumber = rep.getStepAttributeBoolean(id_step,
					"include_rownum");
			includeDeletionDate = rep.getStepAttributeBoolean(id_step,
					"include_deletion_date");
			rowNumberField = rep
					.getStepAttributeString(id_step, "rownum_field");
			deletionDateField = rep.getStepAttributeString(id_step,
					"deletion_date_field");
			...
			recordsFilter = SalesforceConnectionUtils
					.getRecordsFilterByCode(Const.NVL(rep
							.getStepAttributeString(id_step, "records_filter"),
							""));
			useCompression = rep.getStepAttributeBoolean(id_step,
					"useCompression");
			...
			for (int i = 0; i < nrFields; i++) {
				...
				field.setField(rep.getStepAttributeString(id_step, i,
						"field_attribut"));
				field.setIdLookup(rep.getStepAttributeBoolean(id_step, i,
						"field_idlookup"));
				...
				field.setPrecision((int) rep.getStepAttributeInteger(id_step,
						i, "field_precision"));
				field
						.setTrimType(SalesforceInputField.getTrimTypeByCode(rep
			...
			throw new KettleException(BaseMessages.getString(PKG,
					"SalesforceInputMeta.Exception.ErrorReadingRepository"), e);
		}
	}
	...
	public void saveRep(Repository rep, ObjectId id_transformation,
		...
		try {
			...
			rep.saveStepAttribute(id_transformation, id_step, "specifyQuery",
					specifyQuery);
			...
			rep.saveStepAttribute(id_transformation, id_step, "include_rownum",
					includeRowNumber);
			rep.saveStepAttribute(id_transformation, id_step,
					"include_deletion_date", includeDeletionDate);
			...
			rep.saveStepAttribute(id_transformation, id_step, "rownum_field",
					rowNumberField);
			rep.saveStepAttribute(id_transformation, id_step,
					"deletion_date_field", deletionDateField);
			...
			rep.saveStepAttribute(id_transformation, id_step, "read_from",
					readFrom);
			rep
					.saveStepAttribute(id_transformation, id_step, "read_to",
							readTo);
			rep.saveStepAttribute(id_transformation, id_step, "records_filter",
					SalesforceConnectionUtils
							.getRecordsFilterCode(recordsFilter));
			rep.saveStepAttribute(id_transformation, id_step, "useCompression",
					useCompression);
			rep.saveStepAttribute(id_transformation, id_step, "queryAll",
					queryAll);
			...
			for (int i = 0; i < inputFields.length; i++) {
				...
				// H.kawaguchi Bug Fix 17-01-2009
				rep.saveStepAttribute(id_transformation, id_step, i,
						"field_idlookup", field.isIdLookup());
				rep.saveStepAttribute(id_transformation, id_step, i,
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (input.length > 0)
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, BaseMessages
					.getString(PKG,
							"SalesforceInputMeta.CheckResult.NoInputExpected"),
					stepMeta);
		else
			cr = new CheckResult(CheckResult.TYPE_RESULT_OK, BaseMessages
					.getString(PKG, "SalesforceInputMeta.CheckResult.NoInput"),
					stepMeta);
		remarks.add(cr);
		...
		if (Const.isEmpty(targeturl))
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, BaseMessages
					.getString(PKG, "SalesforceInputMeta.CheckResult.NoURL"),
					stepMeta);
		else
			cr = new CheckResult(CheckResult.TYPE_RESULT_OK, BaseMessages
					.getString(PKG, "SalesforceInputMeta.CheckResult.URLOk"),
					stepMeta);
		remarks.add(cr);
		...
		if (Const.isEmpty(username))
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, BaseMessages
					.getString(PKG,
							"SalesforceInputMeta.CheckResult.NoUsername"),
					stepMeta);
		else
			cr = new CheckResult(CheckResult.TYPE_RESULT_OK, BaseMessages
					.getString(PKG,
							"SalesforceInputMeta.CheckResult.UsernameOk"),
					stepMeta);
		remarks.add(cr);
		...
		if (Const.isEmpty(module))
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG,
							"SalesforceInputMeta.CheckResult.NoModule"),
					stepMeta);
		else
			cr = new CheckResult(CheckResult.TYPE_RESULT_OK,
					BaseMessages.getString(PKG,
							"SalesforceInputMeta.CheckResult.ModuleOk"),
					stepMeta);
		remarks.add(cr);
		...
		if (inputFields.length == 0)
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG,
							"SalesforceInputMeta.CheckResult.NoFields"),
					stepMeta);
		else
			cr = new CheckResult(CheckResult.TYPE_RESULT_OK,
					BaseMessages.getString(PKG,
							"SalesforceInputMeta.CheckResult.FieldsOk"),
					stepMeta);
		remarks.add(cr);
		...
		if (includeTargetURL && Const.isEmpty(targetURLField)) {
			cr = new CheckResult(
					...
					BaseMessages.getString(PKG,
							"SalesforceInputMeta.CheckResult.NoTargetURLField"),
					stepMeta);
			remarks.add(cr);
		}
		if (includeSQL && Const.isEmpty(sqlField)) {
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, BaseMessages
					.getString(PKG,
							"SalesforceInputMeta.CheckResult.NoSQLField"),
					stepMeta);
			remarks.add(cr);
		}
		if (includeModule && Const.isEmpty(moduleField)) {
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, BaseMessages
					.getString(PKG,
							"SalesforceInputMeta.CheckResult.NoModuleField"),
					stepMeta);
			remarks.add(cr);
		}
		if (includeTimestamp && Const.isEmpty(timestampField)) {
			cr = new CheckResult(
					CheckResult.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG,
							"SalesforceInputMeta.CheckResult.NoTimestampField"),
					stepMeta);
			remarks.add(cr);
		}
		if (includeRowNumber && Const.isEmpty(rowNumberField)) {
			cr = new CheckResult(
					CheckResult.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG,
							"SalesforceInputMeta.CheckResult.NoRowNumberField"),
					stepMeta);
			remarks.add(cr);
		}
		if (includeDeletionDate && Const.isEmpty(deletionDateField)) {
			cr = new CheckResult(
					...
					BaseMessages
							.getString(PKG,
									"SalesforceInputMeta.CheckResult.NoDeletionDateField"),
					stepMeta);
			remarks.add(cr);
		}
	}

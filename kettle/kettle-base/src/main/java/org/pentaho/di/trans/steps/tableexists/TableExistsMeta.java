...
public class TableExistsMeta extends BaseStepMeta implements StepMetaInterface {
	private static Class<?> PKG = TableExistsMeta.class; // for i18n purposes,
															// needed by
															// Translator2!!
															// $NON-NLS-1$
	...
	public void getFields(RowMetaInterface inputRowMeta, String name,
		...
		if (!Const.isEmpty(resultfieldname)) {
			ValueMetaInterface v = new ValueMeta(space
					.environmentSubstitute(resultfieldname),
	...
	public String getXML() {
		...
		retval
				.append("    " + XMLHandler.addTagValue("connection", database == null ? "" : database.getName())); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		retval
				.append("    " + XMLHandler.addTagValue("tablenamefield", tablenamefield)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
	...
	private void readData(Node stepnode,
		...
		try {
			...
			throw new KettleXMLException(BaseMessages.getString(PKG,
					"TableExistsMeta.Exception.UnableToReadStepInfo"), e); //$NON-NLS-1$
		}
	}
	...
	public void readRep(Repository rep, ObjectId id_step,
		...
		try {
			...
			throw new KettleException(
					BaseMessages
							.getString(PKG,
									"TableExistsMeta.Exception.UnexpectedErrorReadingStepInfo"), e); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, ObjectId id_transformation,
		...
		try {
			rep.saveDatabaseMetaStepAttribute(id_transformation, id_step,
					"id_connection", database);
			rep.saveStepAttribute(id_transformation, id_step,
					"tablenamefield", tablenamefield); //$NON-NLS-1$
			rep.saveStepAttribute(id_transformation, id_step, "schemaname",
					schemaname);
			...
			if (database != null)
				rep.insertStepDatabase(id_transformation, id_step, database
						.getObjectId());
		} catch (Exception e) {
			throw new KettleException(
					BaseMessages.getString(PKG,
							"TableExistsMeta.Exception.UnableToSaveStepInfo") + id_step, e); //$NON-NLS-1$
		}
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (database == null) {
			error_message = BaseMessages.getString(PKG,
					"TableExistsMeta.CheckResult.InvalidConnection"); //$NON-NLS-1$
		...
		if (Const.isEmpty(resultfieldname)) {
			error_message = BaseMessages.getString(PKG,
					"TableExistsMeta.CheckResult.ResultFieldMissing"); //$NON-NLS-1$
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, error_message,
					stepMeta);
			remarks.add(cr);
		} else {
			error_message = BaseMessages.getString(PKG,
					"TableExistsMeta.CheckResult.ResultFieldOK"); //$NON-NLS-1$
		...
		if (Const.isEmpty(tablenamefield)) {
			error_message = BaseMessages.getString(PKG,
					"TableExistsMeta.CheckResult.TableFieldMissing"); //$NON-NLS-1$
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, error_message,
					stepMeta);
			remarks.add(cr);
		} else {
			error_message = BaseMessages.getString(PKG,
					"TableExistsMeta.CheckResult.TableFieldOK"); //$NON-NLS-1$
			cr = new CheckResult(CheckResult.TYPE_RESULT_OK, error_message,
					stepMeta);
			remarks.add(cr);
		}
		...
		if (input.length > 0) {
			cr = new CheckResult(
					...
					BaseMessages
							.getString(PKG,
									"TableExistsMeta.CheckResult.ReceivingInfoFromOtherSteps"), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		} else {
			cr = new CheckResult(
					CheckResult.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG,
							"TableExistsMeta.CheckResult.NoInpuReceived"), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		}
	...
	public StepInterface getStep(StepMeta stepMeta,
			...
			Trans trans) {
		return new TableExists(stepMeta, stepDataInterface, cnr, transMeta,
				trans);
	}

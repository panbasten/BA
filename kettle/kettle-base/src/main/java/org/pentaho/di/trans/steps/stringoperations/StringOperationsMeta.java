 ...
 * @since 02 April 2009
 */
public class StringOperationsMeta extends BaseStepMeta implements
		StepMetaInterface {

	private static Class<?> PKG = StringOperationsMeta.class; // for i18n
																// purposes,
																// needed by
																// Translator2!!
																// $NON-NLS-1$
	...
	public final static String lowerUpperDesc[] = {
			...
			BaseMessages
					.getString(PKG, "StringOperationsMeta.LowerUpper.Lower"),
			BaseMessages
					.getString(PKG, "StringOperationsMeta.LowerUpper.Upper") };
	...
	// digits
	public final static String digitsCode[] = { "none", "digits_only",
			"remove_digits" };
	...
	public final static String maskXMLCode[] = { "none", "escapexml", "cdata",
			"unescapexml", "escapesql", "escapehtml", "unescapehtml" };
	...
	public final static String removeSpecialCharactersCode[] = { "none", "cr",
			"lf", "crlf", "tab", "espace" };
		...
		try {
			...
			for (int i = 0; i < nrkeys; i++) {
				...
				fieldInStream[i] = Const.NVL(XMLHandler.getTagValue(fnode,
						"in_stream_name"), ""); //$NON-NLS-1$
				fieldOutStream[i] = Const.NVL(XMLHandler.getTagValue(fnode,
						"out_stream_name"), ""); //$NON-NLS-1$

				trimType[i] = getTrimTypeByCode(Const.NVL(XMLHandler
						.getTagValue(fnode, "trim_type"), ""));
				lowerUpper[i] = getLowerUpperByCode(Const.NVL(XMLHandler
						.getTagValue(fnode, "lower_upper"), ""));
				padding_type[i] = getPaddingByCode(Const.NVL(XMLHandler
						.getTagValue(fnode, "padding_type"), ""));
				padChar[i] = Const.NVL(XMLHandler
						.getTagValue(fnode, "pad_char"), ""); //$NON-NLS-1$
				padLen[i] = Const.NVL(
						XMLHandler.getTagValue(fnode, "pad_len"), ""); //$NON-NLS-1$
				initCap[i] = getInitCapByCode(Const.NVL(XMLHandler.getTagValue(
						fnode, "init_cap"), ""));
				maskXML[i] = getMaskXMLByCode(Const.NVL(XMLHandler.getTagValue(
						fnode, "mask_xml"), ""));
				digits[i] = getDigitsByCode(Const.NVL(XMLHandler.getTagValue(
						fnode, "digits"), ""));
				remove_special_characters[i] = getRemoveSpecialCharactersByCode(Const
						.NVL(XMLHandler.getTagValue(fnode,
			...
			throw new KettleXMLException(
					BaseMessages
							.getString(PKG,
									"StringOperationsMeta.Exception.UnableToReadStepInfoFromXML"), e); //$NON-NLS-1$
		}
	}
	...
	public String getXML() {
		...
		for (int i = 0; i < fieldInStream.length; i++) {
			...
			retval
					.append("        ").append(XMLHandler.addTagValue("in_stream_name", fieldInStream[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval.append("        ").append(
					XMLHandler
							.addTagValue("out_stream_name", fieldOutStream[i]));

			retval.append("        ").append(
					XMLHandler.addTagValue("trim_type",
							getTrimTypeCode(trimType[i])));
			retval.append("        ").append(
					XMLHandler.addTagValue("lower_upper",
							getLowerUpperCode(lowerUpper[i])));
			retval.append("        ").append(
					XMLHandler.addTagValue("padding_type",
							getPaddingCode(padding_type[i])));
			retval.append("        ").append(
					XMLHandler.addTagValue("pad_char", padChar[i]));
			retval.append("        ").append(
					XMLHandler.addTagValue("pad_len", padLen[i]));
			retval.append("        ").append(
					XMLHandler.addTagValue("init_cap",
							getInitCapCode(initCap[i])));
			retval.append("        ").append(
					XMLHandler.addTagValue("mask_xml",
							getMaskXMLCode(maskXML[i])));
			retval.append("        ").append(
					XMLHandler.addTagValue("digits", getDigitsCode(digits[i])));
			retval
					.append("        ")
					.append(
							XMLHandler
	...
	public void readRep(Repository rep, ObjectId id_step,
		...
		try {
			...
			allocate(nrkeys);
			for (int i = 0; i < nrkeys; i++) {
				fieldInStream[i] = Const.NVL(rep.getStepAttributeString(
						id_step, i, "in_stream_name"), ""); //$NON-NLS-1$
				fieldOutStream[i] = Const.NVL(rep.getStepAttributeString(
						id_step, i, "out_stream_name"), "");

				trimType[i] = getTrimTypeByCode(Const.NVL(rep
						.getStepAttributeString(id_step, i, "trim_type"), ""));
				lowerUpper[i] = getLowerUpperByCode(Const.NVL(rep
						.getStepAttributeString(id_step, i, "lower_upper"), ""));
				padding_type[i] = getPaddingByCode(Const
						.NVL(rep.getStepAttributeString(id_step, i,
								"padding_type"), ""));
				padChar[i] = Const.NVL(rep.getStepAttributeString(id_step, i,
						"pad_char"), "");
				padLen[i] = Const.NVL(rep.getStepAttributeString(id_step, i,
						"pad_len"), "");
				initCap[i] = getInitCapByCode(Const.NVL(rep
						.getStepAttributeString(id_step, i, "init_cap"), ""));
				maskXML[i] = getMaskXMLByCode(Const.NVL(rep
						.getStepAttributeString(id_step, i, "mask_xml"), ""));
				digits[i] = getDigitsByCode(Const.NVL(rep
						.getStepAttributeString(id_step, i, "digits"), ""));
				remove_special_characters[i] = getRemoveSpecialCharactersByCode(Const
						.NVL(rep.getStepAttributeString(id_step, i,
								"remove_special_characters"), ""));
			...
			throw new KettleException(
					BaseMessages
							.getString(PKG,
									"StringOperationsMeta.Exception.UnexpectedErrorInReadingStepInfo"), e); //$NON-NLS-1$
		}
	}
		...
		try {
			...
			for (int i = 0; i < fieldInStream.length; i++) {
				rep.saveStepAttribute(id_transformation, id_step, i,
						"in_stream_name", fieldInStream[i]); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i,
						"out_stream_name", fieldOutStream[i]);
				rep.saveStepAttribute(id_transformation, id_step, i,
						"trim_type", getTrimTypeCode(trimType[i]));
				rep.saveStepAttribute(id_transformation, id_step, i,
						"lower_upper", getLowerUpperCode(lowerUpper[i]));
				rep.saveStepAttribute(id_transformation, id_step, i,
						"padding_type", getPaddingCode(padding_type[i]));
				rep.saveStepAttribute(id_transformation, id_step, i,
						"pad_char", padChar[i]);
				rep.saveStepAttribute(id_transformation, id_step, i, "pad_len",
						padLen[i]);
				rep.saveStepAttribute(id_transformation, id_step, i,
						"init_cap", getInitCapCode(initCap[i]));
				rep.saveStepAttribute(id_transformation, id_step, i,
						"mask_xml", getMaskXMLCode(maskXML[i]));
				rep.saveStepAttribute(id_transformation, id_step, i, "digits",
			...
			throw new KettleException(
					BaseMessages
							.getString(PKG,
									"StringOperationsMeta.Exception.UnableToSaveStepInfo") + id_step, e); //$NON-NLS-1$
		}
	}
		...
		for (int i = 0; i < fieldOutStream.length; i++) {
			...
			if (!Const.isEmpty(outputField)) {
				...
				if (paddingType == PADDING_LEFT || paddingType == PADDING_RIGHT) {
					int padLen = Const.toInt(space
							.environmentSubstitute(getPadLen()[i]), 0);
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (prev == null) {
			...
			error_message += BaseMessages.getString(PKG,
					"StringOperationsMeta.CheckResult.NoInputReceived") + Const.CR; //$NON-NLS-1$
				...
				if (v == null) {
					if (first) {
						...
						error_message += BaseMessages
								.getString(PKG,
										"StringOperationsMeta.CheckResult.MissingInStreamFields") + Const.CR; //$NON-NLS-1$
					}
					error_found = true;
			...
			if (error_found) {
				...
				cr = new CheckResult(
						...
						BaseMessages
								.getString(PKG,
										"StringOperationsMeta.CheckResult.FoundInStreamFields"), stepinfo); //$NON-NLS-1$
			}
			remarks.add(cr);
				...
				if (v != null) {
					if (v.getType() != ValueMeta.TYPE_STRING) {
						if (first) {
							...
							error_message += BaseMessages
									.getString(PKG,
											"StringOperationsMeta.CheckResult.OperationOnNonStringFields") + Const.CR; //$NON-NLS-1$
						}
						error_found = true;
						error_message += "\t\t" + field + Const.CR; //$NON-NLS-1$
					}
			...
			if (error_found) {
				...
				cr = new CheckResult(
						...
						BaseMessages
								.getString(PKG,
										"StringOperationsMeta.CheckResult.AllOperationsOnStringFields"), stepinfo); //$NON-NLS-1$
			}
			remarks.add(cr);
					...
					if (Const.isEmpty(fieldInStream[idx])) {
						cr = new CheckResult(
								...
								BaseMessages
										.getString(
												PKG,
												"StringOperationsMeta.CheckResult.InStreamFieldMissing", new Integer(idx + 1).toString()), stepinfo); //$NON-NLS-1$
						remarks.add(cr);
				...
				for (int jdx = 0; jdx < fieldInStream.length; jdx++) {
					if (fieldInStream[idx].equals(fieldInStream[jdx])
						...
						error_message = BaseMessages
								.getString(
										PKG,
										"StringOperationsMeta.CheckResult.FieldInputError", fieldInStream[idx]); //$NON-NLS-1$
						cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR,
								error_message, stepinfo);
						remarks.add(cr);
					}
	...
	public StepInterface getStep(StepMeta stepMeta,
			...
			Trans trans) {
		return new StringOperations(stepMeta, stepDataInterface, cnr,
				transMeta, trans);
	}

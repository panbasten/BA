...
public class XsltMeta extends BaseStepMeta implements StepMetaInterface {
	private static Class<?> PKG = XsltMeta.class; // for i18n purposes, needed
													// by Translator2!!
													// $NON-NLS-1$

	public static final String[] outputProperties = new String[] { "method",
			"version", "encoding", "standalone", "indent",
	...
	private void readData(Node stepnode) throws KettleXMLException {
		try {
			...
			xslFileFieldUse = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "xslfilefielduse"));
			String isAFile = XMLHandler
					.getTagValue(stepnode, "xslfieldisafile");
			...
			Node parametersOutputProps = XMLHandler.getSubNode(stepnode,
					"outputproperties"); //$NON-NLS-1$
			int nroutputprops = XMLHandler.countNodes(parametersOutputProps,
					"outputproperty");
			allocate(nrparams, nroutputprops);

			for (int i = 0; i < nrparams; i++) {
				Node anode = XMLHandler.getSubNodeByNr(parametersNode,
			...
			for (int i = 0; i < nroutputprops; i++) {
				Node anode = XMLHandler.getSubNodeByNr(parametersOutputProps,
						"outputproperty", i); //$NON-NLS-1$
			...
			throw new KettleXMLException(BaseMessages.getString(PKG,
					"XsltMeta.Exception.UnableToLoadStepInfoFromXML"), e); //$NON-NLS-1$
		}
	}
	...
	public void getFields(RowMetaInterface inputRowMeta, String name,
		...
		ValueMetaInterface v = new ValueMeta(space
				.environmentSubstitute(getResultfieldname()),
	...
	public String getXML() {
		...
		retval
				.append("    " + XMLHandler.addTagValue("xslfilename", xslFilename)); //$NON-NLS-1$ //$NON-NLS-2$
		...
		retval.append("    "
				+ XMLHandler.addTagValue("xslfilefield", xslFileField));
		retval.append("    "
				+ XMLHandler.addTagValue("xslfilefielduse", xslFileFieldUse));
		retval.append("    "
				+ XMLHandler.addTagValue("xslfieldisafile", xslFieldIsAFile));

		retval
				.append("    "
						+ XMLHandler.addTagValue("xslfactory", xslFactory));
		retval.append("    <parameters>").append(Const.CR); //$NON-NLS-1$

		for (int i = 0; i < parameterName.length; i++) {
			retval.append("      <parameter>").append(Const.CR); //$NON-NLS-1$
			retval
					.append("        ").append(XMLHandler.addTagValue("field", parameterField[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval
					.append("        ").append(XMLHandler.addTagValue("name", parameterName[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval.append("      </parameter>").append(Const.CR); //$NON-NLS-1$
		}
		...
		for (int i = 0; i < outputPropertyName.length; i++) {
			...
			retval
					.append("        ").append(XMLHandler.addTagValue("name", outputPropertyName[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval
					.append("        ").append(XMLHandler.addTagValue("value", outputPropertyValue[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval.append("      </outputproperty>").append(Const.CR); //$NON-NLS-1$
		}
	...
	public void readRep(Repository rep, ObjectId id_step,
		...
		try {
			...
			xslFileFieldUse = rep.getStepAttributeBoolean(id_step,
					"xslfilefielduse");
			String isAfile = rep.getStepAttributeString(id_step,
					"xslfieldisafile");
			if (xslFileFieldUse && Const.isEmpty(isAfile)) {
				xslFieldIsAFile = true;
			} else {
			...
			int nroutputprops = rep.countNrStepAttributes(id_step,
					"output_property_name"); //$NON-NLS-1$
			allocate(nrparams, nroutputprops);

			for (int i = 0; i < nrparams; i++) {
				parameterField[i] = rep.getStepAttributeString(id_step, i,
						"param_field"); //$NON-NLS-1$
				parameterName[i] = rep.getStepAttributeString(id_step, i,
						"param_name"); //$NON-NLS-1$
			}
			for (int i = 0; i < nroutputprops; i++) {
				outputPropertyName[i] = rep.getStepAttributeString(id_step, i,
						"output_property_name"); //$NON-NLS-1$
				outputPropertyValue[i] = rep.getStepAttributeString(id_step, i,
						"output_property_value"); //$NON-NLS-1$
			}
		} catch (Exception e) {
			throw new KettleException(BaseMessages.getString(PKG,
					"XsltMeta.Exception.UnexpectedErrorInReadingStepInfo"), e); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, ObjectId id_transformation,
		...
		try {
			rep.saveStepAttribute(id_transformation, id_step,
					"xslfilename", xslFilename); //$NON-NLS-1$
			rep.saveStepAttribute(id_transformation, id_step,
					"fieldname", fieldName); //$NON-NLS-1$
			rep.saveStepAttribute(id_transformation, id_step,
					"resultfieldname", resultFieldname); //$NON-NLS-1$

			rep.saveStepAttribute(id_transformation, id_step, "xslfilefield",
					xslFileField);

			rep.saveStepAttribute(id_transformation, id_step,
					"xslfilefielduse", xslFileFieldUse);
			rep.saveStepAttribute(id_transformation, id_step,
					"xslfieldisafile", xslFieldIsAFile);

			rep.saveStepAttribute(id_transformation, id_step, "xslfactory",
					xslFactory);

			for (int i = 0; i < parameterName.length; i++) {
				rep.saveStepAttribute(id_transformation, id_step, i,
						"param_field", parameterField[i]); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i,
						"param_name", parameterName[i]); //$NON-NLS-1$
			}
			for (int i = 0; i < outputPropertyName.length; i++) {
				rep.saveStepAttribute(id_transformation, id_step, i,
						"output_property_name", outputPropertyName[i]); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i,
						"output_property_value", outputPropertyValue[i]); //$NON-NLS-1$
			}
		} catch (Exception e) {
			throw new KettleException(BaseMessages.getString(PKG,
					"XsltMeta.Exception.UnableToSaveStepInfo") + id_step, e); //$NON-NLS-1$
		}
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (prev != null && prev.size() > 0) {
			cr = new CheckResult(
					...
					BaseMessages
							.getString(
									PKG,
									"XsltMeta.CheckResult.ConnectedStepOK", String.valueOf(prev.size())), stepMeta); //$NON-NLS-1$ //$NON-NLS-2$
			remarks.add(cr);
		} else {
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG,
							"XsltMeta.CheckResult.NoInputReceived"), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		}
		...
		if (input.length > 0) {
			...
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG,
							"XsltMeta.CheckResult.ExpectedInputError"),
					stepMeta);
			remarks.add(cr);
		}
		...
		if (getResultfieldname() == null) {
			...
			cr = new CheckResult(
					...
					BaseMessages.getString(PKG,
							"XsltMeta.CheckResult.ErrorResultFieldNameMissing"), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		...
		if (xslFileFieldUse) {
			if (getXSLFileField() == null) {
				...
				cr = new CheckResult(
						...
						BaseMessages
								.getString(PKG,
										"XsltMeta.CheckResult.ErrorResultXSLFieldNameMissing"), stepMeta); //$NON-NLS-1$
				remarks.add(cr);
			} else {
				...
				cr = new CheckResult(
						...
						BaseMessages
								.getString(PKG,
										"XsltMeta.CheckResult.ErrorResultXSLFieldNameOK"), stepMeta); //$NON-NLS-1$
				remarks.add(cr);
			}
		} else {
			if (xslFilename == null) {
				...
				cr = new CheckResult(
						CheckResult.TYPE_RESULT_ERROR,
						BaseMessages.getString(PKG,
								"XsltMeta.CheckResult.ErrorXSLFileNameMissing"), stepMeta); //$NON-NLS-1$
				remarks.add(cr);

			} else {
				// Check if it's exist and it's a file
				String RealFilename = transMeta
						.environmentSubstitute(xslFilename);
				...
				if (f.exists()) {
					if (f.isFile()) {
						cr = new CheckResult(
								...
								BaseMessages.getString(PKG,
										"XsltMeta.CheckResult.FileExists",
										RealFilename), stepMeta);
						remarks.add(cr);
					} else {
						cr = new CheckResult(
								CheckResultInterface.TYPE_RESULT_ERROR,
								BaseMessages.getString(PKG,
										"XsltMeta.CheckResult.ExistsButNoFile",
										RealFilename), stepMeta);
						remarks.add(cr);
					}
				} else {
					cr = new CheckResult(
							CheckResultInterface.TYPE_RESULT_ERROR,
							BaseMessages.getString(PKG,
									"XsltMeta.CheckResult.FileNotExists",
									RealFilename), stepMeta);
					remarks.add(cr);
				}
			}

...
public class UpdateMeta extends BaseStepMeta implements StepMetaInterface {
	private static Class<?> PKG = UpdateMeta.class; // for i18n purposes, needed
													// by Translator2!!
													// $NON-NLS-1$
	...
	public void setDefault() {
		...
		for (int i = 0; i < nrvalues; i++) {
			updateLookup[i] = BaseMessages.getString(PKG,
					"UpdateMeta.ColumnName.ReturnField") + i; //$NON-NLS-1$
			updateStream[i] = BaseMessages.getString(PKG,
					"UpdateMeta.ColumnName.NewName") + i; //$NON-NLS-1$
		}
	}
	...
	public String getXML() {
		...
		retval
				.append("    " + XMLHandler.addTagValue("connection", databaseMeta == null ? "" : databaseMeta.getName())); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		retval.append("    "
				+ XMLHandler.addTagValue("skip_lookup", skipLookup));
		...
		retval
				.append("    " + XMLHandler.addTagValue("error_ignored", errorIgnored)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("    " + XMLHandler.addTagValue("ignore_flag_field", ignoreFlagField)); //$NON-NLS-1$ //$NON-NLS-2$
		...
		for (int i = 0; i < keyStream.length; i++) {
			...
			retval
					.append("        " + XMLHandler.addTagValue("condition", keyCondition[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval
		...
		for (int i = 0; i < updateLookup.length; i++) {
			...
			retval
					.append("        " + XMLHandler.addTagValue("name", updateLookup[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval
					.append("        " + XMLHandler.addTagValue("rename", updateStream[i])); //$NON-NLS-1$ //$NON-NLS-2$
			retval.append("        </value>" + Const.CR); //$NON-NLS-1$
		}
	...
	public void readRep(Repository rep, ObjectId id_step,
		...
		try {
			...
			errorIgnored = rep
					.getStepAttributeBoolean(id_step, "error_ignored"); //$NON-NLS-1$
			ignoreFlagField = rep.getStepAttributeString(id_step,
					"ignore_flag_field"); //$NON-NLS-1$
			...
			for (int i = 0; i < nrvalues; i++) {
				updateLookup[i] = rep.getStepAttributeString(id_step, i,
						"value_name"); //$NON-NLS-1$
				updateStream[i] = rep.getStepAttributeString(id_step, i,
						"value_rename"); //$NON-NLS-1$
			}
		} catch (Exception e) {
			throw new KettleException(
					BaseMessages
							.getString(PKG,
									"UpdateMeta.Exception.UnexpectedErrorReadingStepInfoFromRepository"), e); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, ObjectId id_transformation,
		...
		try {
			...
			rep.saveStepAttribute(id_transformation, id_step, "skip_lookup",
					skipLookup);
			...
			rep.saveStepAttribute(id_transformation, id_step,
					"error_ignored", errorIgnored); //$NON-NLS-1$
			rep.saveStepAttribute(id_transformation, id_step,
					"ignore_flag_field", ignoreFlagField); //$NON-NLS-1$

			for (int i = 0; i < keyStream.length; i++) {
				rep.saveStepAttribute(id_transformation, id_step, i,
						"key_name", keyStream[i]); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i,
						"key_field", keyLookup[i]); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i,
						"key_condition", keyCondition[i]); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i,
						"key_name2", keyStream2[i]); //$NON-NLS-1$
			}

			for (int i = 0; i < updateLookup.length; i++) {
				rep.saveStepAttribute(id_transformation, id_step, i,
						"value_name", updateLookup[i]); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i,
						"value_rename", updateStream[i]); //$NON-NLS-1$
			}
			...
			throw new KettleException(
					BaseMessages
							.getString(PKG,
									"UpdateMeta.Exception.UnableToSaveStepInfoToRepository") + id_step, e); //$NON-NLS-1$
		}
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (databaseMeta != null) {
			...
			try {
				...
				if (!Const.isEmpty(tableName)) {
					cr = new CheckResult(
							...
							BaseMessages.getString(PKG,
									"UpdateMeta.CheckResult.TableNameOK"), stepinfo); //$NON-NLS-1$
					remarks.add(cr);
					...
					if (r != null) {
						cr = new CheckResult(
								CheckResultInterface.TYPE_RESULT_OK,
								BaseMessages.getString(PKG,
										"UpdateMeta.CheckResult.TableExists"), stepinfo); //$NON-NLS-1$
						remarks.add(cr);
							...
							if (v == null) {
								if (first) {
									...
									error_message += BaseMessages
											.getString(PKG,
													"UpdateMeta.CheckResult.MissingCompareFieldsInTargetTable") + Const.CR; //$NON-NLS-1$
								}
								error_found = true;
						...
						if (error_found) {
							...
							cr = new CheckResult(
									...
									BaseMessages
											.getString(PKG,
													"UpdateMeta.CheckResult.AllLookupFieldsFound"), stepinfo); //$NON-NLS-1$
						}
						remarks.add(cr);
						...
						for (int i = 0; i < updateLookup.length; i++) {
							...
							if (v == null) {
								if (first) {
									...
									error_message += BaseMessages
											.getString(PKG,
													"UpdateMeta.CheckResult.MissingFieldsToUpdateInTargetTable") + Const.CR; //$NON-NLS-1$
								}
								error_found = true;
								error_message += "\t\t" + lufield + Const.CR; //$NON-NLS-1$
							}
						...
						if (error_found) {
							...
							cr = new CheckResult(
									...
									BaseMessages
											.getString(PKG,
													"UpdateMeta.CheckResult.AllFieldsToUpdateFoundInTargetTable"), stepinfo); //$NON-NLS-1$
						}
						remarks.add(cr);
					} else {
						error_message = BaseMessages.getString(PKG,
								"UpdateMeta.CheckResult.CouldNotReadTableInfo"); //$NON-NLS-1$
						cr = new CheckResult(
								CheckResultInterface.TYPE_RESULT_ERROR,
								error_message, stepinfo);
						remarks.add(cr);
				...
				if (prev != null && prev.size() > 0) {
					cr = new CheckResult(
							...
							BaseMessages
									.getString(
											PKG,
											"UpdateMeta.CheckResult.StepReceivingDatas", prev.size() + ""), stepinfo); //$NON-NLS-1$ //$NON-NLS-2$
					remarks.add(cr);
					...
					for (int i = 0; i < keyStream.length; i++) {
						...
						if (v == null) {
							if (first) {
								...
								error_message += BaseMessages
										.getString(PKG,
												"UpdateMeta.CheckResult.MissingFieldsInInput") + Const.CR; //$NON-NLS-1$
							}
							error_found = true;
						...
						if (keyStream2[i] != null && keyStream2[i].length() > 0) {
							...
							if (v == null) {
								if (first) {
									...
									error_message += BaseMessages
											.getString(PKG,
													"UpdateMeta.CheckResult.MissingFieldsInInput2") + Const.CR; //$NON-NLS-1$
								}
								error_found = true;
								error_message += "\t\t" + keyStream[i] + Const.CR; //$NON-NLS-1$
							}
					...
					if (error_found) {
						...
						cr = new CheckResult(
								...
								BaseMessages
										.getString(PKG,
												"UpdateMeta.CheckResult.AllFieldsFoundInInput"), stepinfo); //$NON-NLS-1$
					}
					remarks.add(cr);
					...
					for (int i = 0; i < updateStream.length; i++) {
						...
						if (v == null) {
							if (first) {
								...
								error_message += BaseMessages
										.getString(PKG,
												"UpdateMeta.CheckResult.MissingInputStreamFields") + Const.CR; //$NON-NLS-1$
							}
							error_found = true;
							error_message += "\t\t" + lufield + Const.CR; //$NON-NLS-1$
						}
					...
					if (error_found) {
						...
						cr = new CheckResult(
								...
								BaseMessages
										.getString(PKG,
												"UpdateMeta.CheckResult.AllFieldsFoundInInput2"), stepinfo); //$NON-NLS-1$
					}
					remarks.add(cr);
				} else {
					error_message = BaseMessages.getString(PKG,
							"UpdateMeta.CheckResult.MissingFieldsInInput3") + Const.CR; //$NON-NLS-1$
					cr = new CheckResult(
							CheckResultInterface.TYPE_RESULT_ERROR,
							error_message, stepinfo);
					remarks.add(cr);
				...
				error_message = BaseMessages.getString(PKG,
						"UpdateMeta.CheckResult.DatabaseErrorOccurred") + e.getMessage(); //$NON-NLS-1$
			...
			error_message = BaseMessages.getString(PKG,
					"UpdateMeta.CheckResult.InvalidConnection"); //$NON-NLS-1$
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR,
					error_message, stepinfo);
			remarks.add(cr);
		}
		...
		if (input.length > 0) {
			cr = new CheckResult(
					...
					BaseMessages
							.getString(PKG,
									"UpdateMeta.CheckResult.StepReceivingInfoFromOtherSteps"), stepinfo); //$NON-NLS-1$
			remarks.add(cr);
		} else {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG,
							"UpdateMeta.CheckResult.NoInputError"), stepinfo); //$NON-NLS-1$
			remarks.add(cr);
		}
	}
				...
				if (!Const.isEmpty(tableName)) {
					...
					try {
						...
						if (getIgnoreFlagField() != null
							...
							prev.addValueMeta(new ValueMeta(
									getIgnoreFlagField(),
									ValueMetaInterface.TYPE_BOOLEAN));
						}

						String cr_table = db.getDDL(schemaTable, prev, null,
								false, null, true);
						...
						if (keyLookup != null && keyLookup.length > 0) {
							...
							retval.setError(BaseMessages.getString(PKG,
									"UpdateMeta.CheckResult.MissingKeyFields")); //$NON-NLS-1$
						}
						...
						if (idx_fields != null
							...
							cr_index = db.getCreateIndexStatement(schemaName,
									tableName, indexname, idx_fields, false,
									false, false, true);
						}
						...
						retval
								.setError(BaseMessages.getString(PKG,
										"UpdateMeta.ReturnValue.ErrorOccurred") + e.getMessage()); //$NON-NLS-1$
					}
				} else {
					retval
							.setError(BaseMessages
									.getString(PKG,
											"UpdateMeta.ReturnValue.NoTableDefinedOnConnection")); //$NON-NLS-1$
				}
			} else {
				retval.setError(BaseMessages.getString(PKG,
						"UpdateMeta.ReturnValue.NotReceivingAnyFields")); //$NON-NLS-1$
			}
		} else {
			retval.setError(BaseMessages.getString(PKG,
					"UpdateMeta.ReturnValue.NoConnectionDefined")); //$NON-NLS-1$
		}

...
public class UniqueRowsMeta extends BaseStepMeta implements StepMetaInterface {
	private static Class<?> PKG = UniqueRowsMeta.class; // for i18n purposes,
														// needed by
														// Translator2!!
														// $NON-NLS-1$
	...
	private void readData(Node stepnode) throws KettleXMLException {
		try {
			...
			throw new KettleXMLException(BaseMessages.getString(PKG,
					"UniqueRowsMeta.Exception.UnableToLoadStepInfoFromXML"), e); //$NON-NLS-1$
		}
	}
	...
	public void readRep(Repository rep, ObjectId id_step,
		...
		try {
			...
			throw new KettleException(
					BaseMessages
							.getString(PKG,
									"UniqueRowsMeta.Exception.UnexpectedErrorReadingStepInfo"), e); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, ObjectId id_transformation,
		...
		try {
			...
			throw new KettleException(BaseMessages.getString(PKG,
					"UniqueRowsMeta.Exception.UnableToSaveStepInfo"), e); //$NON-NLS-1$
		}
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (input.length > 0) {
			cr = new CheckResult(
					...
					BaseMessages
							.getString(PKG,
									"UniqueRowsMeta.CheckResult.StepReceivingInfoFromOtherSteps"), stepinfo); //$NON-NLS-1$
			remarks.add(cr);
		} else {
			cr = new CheckResult(
					...
					BaseMessages
							.getString(PKG,
									"UniqueRowsMeta.CheckResult.NoInputReceivedFromOtherSteps"), stepinfo); //$NON-NLS-1$
			remarks.add(cr);
		}
	}

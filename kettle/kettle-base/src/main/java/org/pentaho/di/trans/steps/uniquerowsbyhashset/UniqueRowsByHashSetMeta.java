...
public class UniqueRowsByHashSetMeta extends BaseStepMeta implements
		StepMetaInterface {
	private static Class<?> PKG = UniqueRowsByHashSetMeta.class; // for i18n
																	// purposes,
																	// needed by
																	// Translator2!!
																	// $NON-NLS-1$
	...
	/**
	 * Whether to compare strictly by hash value or to store the row values for
	...
	public Object clone() {
		UniqueRowsByHashSetMeta retval = (UniqueRowsByHashSetMeta) super
				.clone();
	...
	private void readData(Node stepnode) throws KettleXMLException {
		try {
			...
			rejectDuplicateRow = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "reject_duplicate_row"));
			errorDescription = XMLHandler.getTagValue(stepnode,
					"error_description"); //$NON-NLS-1$
			...
			throw new KettleXMLException(
					BaseMessages
							.getString(PKG,
									"UniqueRowsByHashSetMeta.Exception.UnableToLoadStepInfoFromXML"), e); //$NON-NLS-1$
		}
	}
	...
	public String getXML() {
		...
		retval
				.append("      " + XMLHandler.addTagValue("store_values", storeValues)); //$NON-NLS-1$ //$NON-NLS-2$
		retval.append("      "
				+ XMLHandler.addTagValue("reject_duplicate_row",
						rejectDuplicateRow));
		retval
				.append("      "
						+ XMLHandler.addTagValue("error_description",
								errorDescription));
	...
	public void readRep(Repository rep, ObjectId id_step,
		...
		try {
			...
			rejectDuplicateRow = rep.getStepAttributeBoolean(id_step,
					"reject_duplicate_row");
			errorDescription = rep.getStepAttributeString(id_step,
					"error_description"); //$NON-NLS-1$
			...
			throw new KettleException(
					BaseMessages
							.getString(PKG,
									"UniqueRowsByHashSetMeta.Exception.UnexpectedErrorReadingStepInfo"), e); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, ObjectId id_transformation,
		...
		try {
			rep.saveStepAttribute(id_transformation, id_step,
					"reject_duplicate_row", rejectDuplicateRow);
			rep.saveStepAttribute(id_transformation, id_step,
			...
			throw new KettleException(
					BaseMessages
							.getString(PKG,
									"UniqueRowsByHashSetMeta.Exception.UnableToSaveStepInfo"), e); //$NON-NLS-1$
		}
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (input.length > 0) {
			cr = new CheckResult(
					...
					BaseMessages
							.getString(PKG,
									"UniqueRowsByHashSetMeta.CheckResult.StepReceivingInfoFromOtherSteps"), stepinfo); //$NON-NLS-1$
			remarks.add(cr);
		} else {
			cr = new CheckResult(
					...
					BaseMessages
							.getString(PKG,
									"UniqueRowsByHashSetMeta.CheckResult.NoInputReceivedFromOtherSteps"), stepinfo); //$NON-NLS-1$
			remarks.add(cr);
		}
	}
	...
	public StepInterface getStep(StepMeta stepMeta,
			...
			Trans trans) {
		return new UniqueRowsByHashSet(stepMeta, stepDataInterface, cnr,
				transMeta, trans);
	}

...
/**
 * This Transformation Step allows a user to execute a rule set against an
 * individual rule or a collection of rules.
 * 
 * Additional columns can be added to the output from the rules and these (of
 * course) can be used for routing if desired.
 * 
 * @author cboyden
 * 
 */

public class RulesExecutorMeta extends BaseStepMeta implements
		StepMetaInterface {
	...
	public StepInterface getStep(StepMeta stepMeta,
		...
		return new RulesExecutor(stepMeta, stepDataInterface, copyNr,
				transMeta, trans);
	}
	...
	@Override
	public void loadXML(Node stepnode, List<DatabaseMeta> _databases,
			Map<String, Counter> counters) throws KettleXMLException {
		try {
			Node fields = XMLHandler.getSubNode(stepnode,
					StorageKeys.NODE_FIELDS.toString());
			int nrfields = XMLHandler.countNodes(fields,
					StorageKeys.SUBNODE_FIELD.toString());

			ValueMetaInterface vm = null;
			for (int i = 0; i < nrfields; i++) {
				vm = new ValueMeta();
				Node fnode = XMLHandler.getSubNodeByNr(fields,
						StorageKeys.SUBNODE_FIELD.toString(), i);

				vm.setName(XMLHandler.getTagValue(fnode,
						StorageKeys.COLUMN_NAME.toString()));
				vm.setType(ValueMeta.getType(XMLHandler.getTagValue(fnode,
						StorageKeys.COLUMN_TYPE.toString())));

				getRuleResultColumns().add(vm);
			}

			setRuleFile(XMLHandler.getTagValue(stepnode, StorageKeys.RULE_FILE
					.toString()));
			setRuleDefinition(XMLHandler.getTagValue(stepnode,
					StorageKeys.RULE_DEFINITION.toString()));
		} catch (Exception e) {
			throw new KettleXMLException(BaseMessages.getString(PKG,
					"RulesMeta.Error.LoadFromXML"), e); //$NON-NLS-1$
		}
	}
	...
	public String getXML() {
		...
		for (int i = 0; i < ruleResultColumns.size(); i++) {
			retval
					.append("      <" + StorageKeys.SUBNODE_FIELD + ">").append(Const.CR); //$NON-NLS-1$ //$NON-NLS-2$
			retval
		...
		retval
				.append("    </" + StorageKeys.NODE_FIELDS + ">").append(Const.CR); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("    ").append(XMLHandler.addTagValue(StorageKeys.RULE_FILE.toString(), getRuleFile())); //$NON-NLS-1$
		retval
				.append("    ").append(XMLHandler.addTagValue(StorageKeys.RULE_DEFINITION.toString(), getRuleDefinition())); //$NON-NLS-1$
	...
	public void readRep(Repository rep, ObjectId idStep,
			List<DatabaseMeta> _databases, Map<String, Counter> counters)
			throws KettleException {

		int nrfields = rep.countNrStepAttributes(idStep,
				StorageKeys.COLUMN_NAME.toString());
		...
		for (int i = 0; i < nrfields; i++) {
			...
			vm.setName(rep.getStepAttributeString(idStep, i,
					StorageKeys.COLUMN_NAME.toString()));
			vm.setType(ValueMeta.getType(rep.getStepAttributeString(idStep, i,
					StorageKeys.COLUMN_TYPE.toString())));

			getRuleResultColumns().add(vm);
		}

		setRuleFile(rep.getStepAttributeString(idStep, StorageKeys.RULE_FILE
				.toString()));
		setRuleDefinition(rep.getStepAttributeString(idStep,
				StorageKeys.RULE_DEFINITION.toString()));
	}
	...
	public void saveRep(Repository rep, ObjectId idTransformation,
		...
		for (int i = 0; i < ruleResultColumns.size(); i++) {
			rep.saveStepAttribute(idTransformation, idStep, i,
					StorageKeys.COLUMN_NAME.toString(), ruleResultColumns
							.get(i).getName());
			rep.saveStepAttribute(idTransformation, idStep, i,
					StorageKeys.COLUMN_TYPE.toString(), ruleResultColumns
							.get(i).getTypeDesc());
		}

		rep.saveStepAttribute(idTransformation, idStep, StorageKeys.RULE_FILE
				.toString(), getRuleFile());
		rep.saveStepAttribute(idTransformation, idStep,
				StorageKeys.RULE_DEFINITION.toString(), getRuleDefinition());
	}

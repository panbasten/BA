...
/**
 * Executes a xsd validator on the values in the input stream. New fields were
 * calculated values can then be put on the output stream.
 * 
 * @author Samatar
 * @since 14-08-2007
 * 
...
public class XsdValidator extends BaseStep implements StepInterface {
	...
	public XsdValidator(StepMeta stepMeta, StepDataInterface stepDataInterface,
	...
	public boolean processRow(StepMetaInterface smi, StepDataInterface sdi)
		...
		if (first) {
			...
			if (meta.getXMLStream() != null) {
				...
				data.xmlindex = getInputRowMeta().indexOfValue(
						meta.getXMLStream());
				// Let's check the Field
				if (data.xmlindex < 0) {
					// The field is unreachable !
					logError(BaseMessages.getString(PKG,
							"XsdValidator.Log.ErrorFindingField") + "[" + meta.getXMLStream() + "]"); //$NON-NLS-1$ //$NON-NLS-2$
					throw new KettleStepException(
							BaseMessages
									.getString(
											PKG,
											"XsdValidator.Exception.CouldnotFindField", meta.getXMLStream())); //$NON-NLS-1$ //$NON-NLS-2$
				}
				...
				if (meta.getResultfieldname() == null) {
					...
					logError(BaseMessages.getString(PKG,
							"XsdValidator.Log.ErrorResultFieldMissing")); //$NON-NLS-1$ //$NON-NLS-2$
					throw new KettleStepException(BaseMessages.getString(PKG,
							"XsdValidator.Exception.ErrorResultFieldMissing")); //$NON-NLS-1$ //$NON-NLS-2$
				}
				...
				if (meta.getXSDSource().equals(meta.SPECIFY_FILENAME)) {
					if (meta.getXSDFilename() == null) {
						logError(BaseMessages.getString(PKG,
								"XsdValidator.Log.ErrorXSDFileMissing")); //$NON-NLS-1$ //$NON-NLS-2$
						throw new KettleStepException(BaseMessages.getString(
								PKG,
								"XsdValidator.Exception.ErrorXSDFileMissing")); //$NON-NLS-1$ //$NON-NLS-2$
					} else {
						// Is XSD file exists ?
						...
						try {
							xsdfile = KettleVFS
									.getFileObject(environmentSubstitute(meta
											.getXSDFilename()), getTransMeta());
							if (!xsdfile.exists()) {
								logError(BaseMessages
										.getString(PKG,
												"XsdValidator.Log.Error.XSDFileNotExists"));
								throw new KettleStepException(
										BaseMessages
												.getString(PKG,
							...
							logError(BaseMessages.getString(PKG,
									"XsdValidator.Log.Error.GettingXSDFile"));
							throw new KettleStepException(
									BaseMessages
											.getString(PKG,
				...
				if (meta.getXSDSource().equals(meta.SPECIFY_FIELDNAME)) {
					if (meta.getXSDDefinedField() == null) {
						...
						throw new KettleStepException(BaseMessages.getString(
								PKG, "XsdValidator.Exception.XSDFieldMissing"));
					} else {
						// Let's check if the XSD field exist
						// Try to get XML Field index
						data.xsdindex = getInputRowMeta().indexOfValue(
						...
						if (data.xsdindex < 0) {
							...
							logError(BaseMessages
									.getString(
											PKG,
											"XsdValidator.Log.ErrorFindingXSDField", meta.getXSDDefinedField())); //$NON-NLS-1$ //$NON-NLS-2$
							throw new KettleStepException(
									BaseMessages
											.getString(
													PKG,
				...
				logError(BaseMessages.getString(PKG,
						"XsdValidator.Log.Error.XmlStreamFieldMissing")); //$NON-NLS-1$ //$NON-NLS-2$
				throw new KettleStepException(BaseMessages.getString(PKG,
						"XsdValidator.Exception.XmlStreamFieldMissing")); //$NON-NLS-1$ //$NON-NLS-2$
			}
		}
		...
		try {
			...
			String XMLFieldvalue = getInputRowMeta().getString(row,
					data.xmlindex);
			...
			try {
				...
				SchemaFactory factoryXSDValidator = SchemaFactory
						.newInstance(W3C_XML_SCHEMA_NS_URI);

				xsdfile = KettleVFS.getFileObject(xsdfilename, getTransMeta());

				// Get XML stream
				Source sourceXML = new StreamSource(new StringReader(
						XMLFieldvalue));
				...
				if (meta.getXMLSourceFile()) {
					...
					if (!xmlfileValidator.exists()) {
						logError(BaseMessages
								.getString(
										PKG,
										"XsdValidator.Log.Error.XMLfileMissing", XMLFieldvalue)); //$NON-NLS-1$ //$NON-NLS-2$
						throw new KettleStepException(
								BaseMessages
										.getString(
												PKG,
												"XsdValidator.Exception.XMLfileMissing", XMLFieldvalue)); //$NON-NLS-1$ //$NON-NLS-2$
					}
				...
				if (xsdfile instanceof LocalFile) {
					SchematXSD = factoryXSDValidator.newSchema(new File(
							KettleVFS.getFilename(xsdfile)));
				} else if (xsdfile instanceof HttpFileObject) {
					SchematXSD = factoryXSDValidator.newSchema(new URL(
							KettleVFS.getFilename(xsdfile)));
				} else {
					// we should not get here as anything entered in that does
					// not look like
				...
				if (meta.getXSDSource().equals(meta.NO_NEED)) {
					// ---Some documents specify the schema they expect to be
					// validated against,
			...
			if (meta.getOutputStringField()) {
				// Output type=String
				if (isvalid)
					outputRowData = RowDataUtil.addValueData(row,
				...
				outputRowData = RowDataUtil.addValueData(row, getInputRowMeta()
						.size(), isvalid);
			}

			if (meta.useAddValidationMessage())
				outputRowData2 = RowDataUtil.addValueData(outputRowData,
						getInputRowMeta().size() + 1, validationmsg);
			else
				outputRowData2 = outputRowData;

			if (log.isRowLevel())
				logRowlevel(BaseMessages.getString(PKG,
						"XsdValidator.Log.ReadRow")
						+ " "
						+ getInputRowMeta().getString(row));

			// add new values to the row.
			putRow(data.outputRowMeta, outputRowData2); // copy row to output
														// rowset(s);
			...
			if (sendToErrorRow) {
				...
				putError(getInputRowMeta(), row, 1, errorMessage, null,
						"XSD001");
			} else {
				logError(BaseMessages.getString(PKG,
						"XsdValidator.ErrorProcesing" + " : " + e.getMessage()));
				throw new KettleStepException(BaseMessages.getString(PKG,
						"XsdValidator.ErrorProcesing"), e);
			}
		}

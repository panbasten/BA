...
public class SampleRowsMeta extends BaseStepMeta implements StepMetaInterface {
	private static Class<?> PKG = SampleRowsMeta.class; // for i18n purposes,
														// needed by
														// Translator2!!
														// $NON-NLS-1$
	...
	public void getFields(RowMetaInterface inputRowMeta, String name,
		...
		if (!Const.isEmpty(linenumfield)) {
			...
			ValueMetaInterface v = new ValueMeta(space
					.environmentSubstitute(linenumfield),
	...
	private void readData(Node stepnode,
		...
		try {
			...
			throw new KettleXMLException(BaseMessages.getString(PKG,
					"SampleRowsMeta.Exception.UnableToReadStepInfo"), e);
		}
	}
	...
	public void readRep(Repository rep, ObjectId id_step,
		...
		try {
			...
			throw new KettleException(
					BaseMessages
							.getString(PKG,
									"SampleRowsMeta.Exception.UnexpectedErrorReadingStepInfo"), e); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, ObjectId id_transformation,
		...
		try {
			rep.saveStepAttribute(id_transformation, id_step, "linesrange",
					linesrange);
			rep.saveStepAttribute(id_transformation, id_step, "linenumfield",
					linenumfield);
			...
			throw new KettleException(
					BaseMessages
							.getString(PKG,
									"SampleRowsMeta.Exception.UnexpectedErrorSavingStepInfo"), e); //$NON-NLS-1$
		}
	}
	...
	public String getXML() {
		...
		retval
				.append("    "
						+ XMLHandler.addTagValue("linesrange", linesrange));
		retval.append("    "
				+ XMLHandler.addTagValue("linenumfield", linenumfield));
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (Const.isEmpty(linesrange))
			...
			cr = new CheckResult(CheckResult.TYPE_RESULT_OK, BaseMessages
					.getString(PKG, "SampleRowsMeta.CheckResult.LinesRangeOk"),
					stepinfo);
		remarks.add(cr);
		...
		if (prev == null || prev.size() == 0)
			cr = new CheckResult(
					...
					BaseMessages.getString(PKG,
							"SampleRowsMeta.CheckResult.NotReceivingFields"), stepinfo); //$NON-NLS-1$
		else
			cr = new CheckResult(
					...
					BaseMessages
							.getString(
									PKG,
									"SampleRowsMeta.CheckResult.StepRecevingData", prev.size() + ""), stepinfo); //$NON-NLS-1$ //$NON-NLS-2$
		remarks.add(cr);
		...
		if (input.length > 0)
			cr = new CheckResult(
					CheckResult.TYPE_RESULT_OK,
					BaseMessages.getString(PKG,
							"SampleRowsMeta.CheckResult.StepRecevingData2"), stepinfo); //$NON-NLS-1$
		else
			cr = new CheckResult(
					CheckResult.TYPE_RESULT_ERROR,
					BaseMessages
							.getString(PKG,
									"SampleRowsMeta.CheckResult.NoInputReceivedFromOtherSteps"), stepinfo); //$NON-NLS-1$
		remarks.add(cr);
	}

...
/**
 * Symmetric algorithm Executes a SymmetricCryptoTrans on the values in the
 * input stream. Selected calculated values can then be put on the output
 * stream.
 * 
 * @author Samatar
 * @since 5-apr-2003
 * 
 */
public class SymmetricCryptoTransMeta extends BaseStepMeta implements
		StepMetaInterface {
	private static Class<?> PKG = SymmetricCryptoTransMeta.class; // for i18n
																	// purposes,
																	// needed by
																	// Translator2!!
																	// $NON-NLS-1$
	...
	public final static String operationTypeDesc[] = {
			BaseMessages.getString(PKG,
					"SymmetricCryptoTransMeta.operationType.Encrypt"),
			BaseMessages.getString(PKG,
					"SymmetricCryptoTransMeta.operationType.Decrypt") };
	...
	public Object clone() {
		SymmetricCryptoTransMeta retval = (SymmetricCryptoTransMeta) super
				.clone();
	...
	private void readData(Node stepnode) throws KettleXMLException {
		try {
			operationType = getOperationTypeByCode(Const.NVL(XMLHandler
					.getTagValue(stepnode, "operation_type"), ""));
			...
			setSecretKey(Encr.decryptPasswordOptionallyEncrypted(XMLHandler
					.getTagValue(stepnode, "secretKey")));
			secretKeyInField = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "secretKeyInField"));
			readKeyAsBinary = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "readKeyAsBinary"));
			outputResultAsBinary = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "outputResultAsBinary"));

		} catch (Exception e) {
			throw new KettleXMLException(
					BaseMessages
							.getString(PKG,
									"SymmetricCryptoTransMeta.Exception.UnableToLoadStepInfoFromXML"), e); //$NON-NLS-1$
		}
	}
	...
	public String getXML() {
		...
		retval.append("    "
				+ XMLHandler.addTagValue("operation_type",
						getOperationTypeCode(operationType)));
		retval.append("    " + XMLHandler.addTagValue("algorithm", algorithm));
		retval.append("    " + XMLHandler.addTagValue("schema", schema));
		retval
				.append("    " + XMLHandler.addTagValue("secretKeyField", secretKeyField)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("    " + XMLHandler.addTagValue("messageField", messageField)); //$NON-NLS-1$ //$NON-NLS-2$
		retval
				.append("    " + XMLHandler.addTagValue("resultfieldname", resultfieldname)); //$NON-NLS-1$ //$NON-NLS-2$

		retval.append("    ").append(
				XMLHandler.addTagValue("secretKey", Encr
						.encryptPasswordIfNotUsingVariables(secretKey)));

		retval.append("    "
				+ XMLHandler.addTagValue("secretKeyInField", secretKeyInField));
		retval.append("    "
				+ XMLHandler.addTagValue("readKeyAsBinary", readKeyAsBinary));
		retval.append("    "
				+ XMLHandler.addTagValue("outputResultAsBinary",
	...
	public void readRep(Repository rep, ObjectId id_step,
		...
		try {
			operationType = getOperationTypeByCode(Const.NVL(rep
					.getStepAttributeString(id_step, "operation_type"), ""));
			algorithm = rep.getStepAttributeString(id_step, "algorithm");
			schema = rep.getStepAttributeString(id_step, "schema");
			secretKeyField = rep.getStepAttributeString(id_step,
					"secretKeyField"); //$NON-NLS-1$
			messageField = rep.getStepAttributeString(id_step, "messageField"); //$NON-NLS-1$
			resultfieldname = rep.getStepAttributeString(id_step,
					"resultfieldname"); //$NON-NLS-1$

			secretKey = Encr.decryptPasswordOptionallyEncrypted(rep
					.getStepAttributeString(id_step, "secretKey"));
			secretKeyInField = rep.getStepAttributeBoolean(id_step,
					"secretKeyInField");
			readKeyAsBinary = rep.getStepAttributeBoolean(id_step,
					"readKeyAsBinary");
			outputResultAsBinary = rep.getStepAttributeBoolean(id_step,
					"outputResultAsBinary");
			...
			throw new KettleException(
					BaseMessages
							.getString(PKG,
									"SymmetricCryptoTransMeta.Exception.UnexpectedErrorInReadingStepInfo"), e); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, ObjectId id_transformation,
		...
		try {
			rep.saveStepAttribute(id_transformation, id_step, "operation_type",
					getOperationTypeCode(operationType));
			rep.saveStepAttribute(id_transformation, id_step, "algorithm",
					algorithm);
			rep.saveStepAttribute(id_transformation, id_step, "schema", schema);

			rep.saveStepAttribute(id_transformation, id_step,
					"secretKeyField", secretKeyField); //$NON-NLS-1$
			rep.saveStepAttribute(id_transformation, id_step,
					"messageField", messageField); //$NON-NLS-1$
			rep.saveStepAttribute(id_transformation, id_step,
			...
			rep.saveStepAttribute(id_transformation, id_step, "secretKey", Encr
					.encryptPasswordIfNotUsingVariables(secretKey));
			rep.saveStepAttribute(id_transformation, id_step,
					"secretKeyInField", secretKeyInField);
			rep.saveStepAttribute(id_transformation, id_step,
					"readKeyAsBinary", readKeyAsBinary);
			rep.saveStepAttribute(id_transformation, id_step,
					"outputResultAsBinary", outputResultAsBinary);

		} catch (Exception e) {
			throw new KettleException(
					BaseMessages
							.getString(PKG,
									"SymmetricCryptoTransMeta.Exception.UnableToSaveStepInfo") + id_step, e); //$NON-NLS-1$
		}
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (prev != null && prev.size() > 0) {
			cr = new CheckResult(
					...
					BaseMessages
							.getString(
									PKG,
									"SymmetricCryptoTransMeta.CheckResult.ConnectedStepOK", String.valueOf(prev.size())), stepinfo); //$NON-NLS-1$ //$NON-NLS-2$
		} else {
			cr = new CheckResult(
					CheckResult.TYPE_RESULT_ERROR,
					BaseMessages
							.getString(PKG,
									"SymmetricCryptoTransMeta.CheckResult.NoInputReceived"), stepinfo); //$NON-NLS-1$
		...
		if (getResultfieldname() == null) {
			...
			cr = new CheckResult(
					...
					BaseMessages
							.getString(PKG,
									"SymmetricCryptoTransMeta.CheckResult.ErrorResultFieldNameMissing"), stepinfo); //$NON-NLS-1$
			remarks.add(cr);
	...
	public StepInterface getStep(StepMeta stepMeta,
			...
			Trans trans) {
		return new SymmetricCryptoTrans(stepMeta, stepDataInterface, cnr,
				transMeta, trans);

...
public class KettleDatabaseRepository extends KettleDatabaseRepositoryBase
	...
	private void init() {
		...
		this.databaseDelegate = new KettleDatabaseRepositoryDatabaseDelegate(
				this);
		this.slaveServerDelegate = new KettleDatabaseRepositorySlaveServerDelegate(
				this);
		this.clusterSchemaDelegate = new KettleDatabaseRepositoryClusterSchemaDelegate(
				this);
		this.partitionSchemaDelegate = new KettleDatabaseRepositoryPartitionSchemaDelegate(
				this);
		this.directoryDelegate = new KettleDatabaseRepositoryDirectoryDelegate(
				this);
		this.connectionDelegate = new KettleDatabaseRepositoryConnectionDelegate(
				this, repositoryMeta.getConnection());
		this.userDelegate = new KettleDatabaseRepositoryUserDelegate(this);
		this.conditionDelegate = new KettleDatabaseRepositoryConditionDelegate(
				this);
		...
		this.jobEntryDelegate = new KettleDatabaseRepositoryJobEntryDelegate(
				this);
	 ...
	 *             in case there is a general unexpected error or if we're
	 *             already connected
	 */
	 ...
	 *             already connected
	 */
	public void connect(String username, String password, boolean upgrade)
			throws KettleException {
		// first disconnect if already connected
		connectionDelegate.connect(upgrade, upgrade);
		try {
			IUser userinfo = userDelegate.loadUserInfo(new UserInfo(),
					username, password);
			securityProvider = new KettleDatabaseRepositorySecurityProvider(
					this, repositoryMeta, userinfo);

			// We need to add services in the list in the order of dependencies
			registerRepositoryService(RepositorySecurityProvider.class,
					securityProvider);
			registerRepositoryService(RepositorySecurityManager.class,
					securityProvider);
	...
	public IUser loadUserInfo(String username, String password)
			throws KettleException {
		IUser userinfo = userDelegate.loadUserInfo(new UserInfo(), username,
				password);
		return userinfo;
	}
	...
	public void connect(boolean upgrade) throws KettleException {
		connectionDelegate.connect(upgrade, upgrade);
		securityProvider = new KettleDatabaseRepositorySecurityProvider(this,
				repositoryMeta, null);
		registerRepositoryService(RepositorySecurityProvider.class,
				securityProvider);
		registerRepositoryService(RepositorySecurityManager.class,
	...
	public void setUserInfo(IUser userInfo) {
		securityProvider.setUserInfo(userInfo);
	}

	public void clearUserInfo() {
		securityProvider.setUserInfo(null);
	}
	...
	public TransMeta loadTransformation(String transname,
		...
		securityProvider
				.validateAction(RepositoryOperation.READ_TRANSFORMATION);
		TransMeta transMeta = new TransMeta();
		return transDelegate.loadTransformation(transMeta, transname, repdir,
	...
	public synchronized ObjectId renameTransformation(
			ObjectId id_transformation, RepositoryDirectoryInterface newDir,
			String newName) throws KettleException {
		securityProvider
				.validateAction(RepositoryOperation.MODIFY_TRANSFORMATION);
	 ...
	 * @throws KettleException
	 */
	public JobMeta loadJob(String jobname, RepositoryDirectoryInterface repdir,
			ProgressMonitorListener monitor, String versionName)
			throws KettleException {
	...
	public synchronized ObjectId renameJob(ObjectId id_job,
			RepositoryDirectoryInterface dir, String newname)
			throws KettleException {
		securityProvider
				.validateAction(RepositoryOperation.MODIFY_TRANSFORMATION);
	...
	public boolean exists(String name,
		...
		case JOB:
			securityProvider.validateAction(RepositoryOperation.READ_JOB);
			return jobDelegate.existsJobMeta(name, repositoryDirectory,
					objectType);

		case TRANSFORMATION:
			securityProvider
					.validateAction(RepositoryOperation.READ_TRANSFORMATION);
			return transDelegate.existsTransMeta(name, repositoryDirectory,
					objectType);
	...
	public void save(RepositoryElementInterface repositoryElement,
			String versionComment) throws KettleException {
		save(repositoryElement, versionComment, null);
	}

	public void save(RepositoryElementInterface repositoryElement,
			String versionComment, ProgressMonitorListener monitor)
			throws KettleException {
	...
	public void save(RepositoryElementInterface repositoryElement,
			String versionComment, ProgressMonitorListener monitor,
			ObjectId parentId, boolean used, boolean overwrite)
			throws KettleException {
		...
		try {
			...
			case JOB:
				...
				jobDelegate.saveJob((JobMeta) repositoryElement,
						versionComment, monitor, overwrite);
				break;
			case TRANSFORMATION:
				securityProvider
						.validateAction(RepositoryOperation.MODIFY_TRANSFORMATION);
				transDelegate.saveTransformation((TransMeta) repositoryElement,
						versionComment, monitor, overwrite);
				break;
			case DATABASE:
				securityProvider
						.validateAction(RepositoryOperation.MODIFY_DATABASE);
				databaseDelegate
						.saveDatabaseMeta((DatabaseMeta) repositoryElement);
				break;
			case SLAVE_SERVER:
				securityProvider
						.validateAction(RepositoryOperation.MODIFY_SLAVE_SERVER);
				slaveServerDelegate.saveSlaveServer(
						(SlaveServer) repositoryElement, parentId, used,
						overwrite);
				break;
			case CLUSTER_SCHEMA:
				securityProvider
						.validateAction(RepositoryOperation.MODIFY_CLUSTER_SCHEMA);
				clusterSchemaDelegate.saveClusterSchema(
						(ClusterSchema) repositoryElement, versionComment,
						parentId, used, overwrite);
				break;
			case PARTITION_SCHEMA:
				securityProvider
						.validateAction(RepositoryOperation.MODIFY_PARTITION_SCHEMA);
				partitionSchemaDelegate.savePartitionSchema(
						(PartitionSchema) repositoryElement, parentId, used,
						overwrite);
				break;
			default:
				throw new KettleException(
						"We can't save the element with type ["
								+ repositoryElement.getRepositoryElementType()
								+ "] in the repository");
			}
	...
	public ObjectId saveCondition(Condition condition,
			ObjectId id_condition_parent) throws KettleException {
	 ...
	 * @throws KettleException
	 *             In case something went wrong: database error, insufficient
	 *             permissions, depending objects, etc.
	 */
	...
	public ClusterSchema loadClusterSchema(ObjectId idClusterSchema,
			List<SlaveServer> slaveServers, String versionLabel)
			throws KettleException {
		return clusterSchemaDelegate.loadClusterSchema(idClusterSchema,
				slaveServers);
	}
	...
	public void saveNotePadMeta(NotePadMeta note, ObjectId id_transformation)
			throws KettleException {
	...
	public ObjectId renameRepositoryDirectory(ObjectId id,
			RepositoryDirectoryInterface newParentDir, String newName)
			throws KettleException {
		ObjectId result = null;
		securityProvider.validateAction(RepositoryOperation.RENAME_DIRECTORY);
		result = directoryDelegate.renameRepositoryDirectory(id, newParentDir,
				newName);
		commit();
		return result;
	}
	...
	/**
	 * Create a new directory, possibly by creating several sub-directies of /
	 * at the same time.
	 * 
	...
	public RepositoryDirectoryInterface createRepositoryDirectory(
			RepositoryDirectoryInterface parentDirectory, String directoryPath)
			throws KettleException {
		securityProvider.validateAction(RepositoryOperation.CREATE_DIRECTORY);
		return directoryDelegate.createRepositoryDirectory(parentDirectory,
				directoryPath);
	}
	...
	public synchronized ObjectId getRootDirectoryID() throws KettleException {
		RowMetaAndData result = connectionDelegate
				.getOneRow("SELECT "
	...
	public synchronized int getNrSubDirectories(ObjectId id_directory)
			throws KettleException {
		return directoryDelegate.getNrSubDirectories(id_directory);
	}

	public synchronized ObjectId[] getSubDirectoryIDs(ObjectId id_directory)
			throws KettleException {
	...
	public synchronized ObjectId insertLogEntry(String description)
			throws KettleException {
		...
		connectionDelegate.insertTableRow(
				KettleDatabaseRepository.TABLE_R_REPOSITORY_LOG, table);

		return id;
	}

	public synchronized void insertTransNote(ObjectId id_transformation,
			ObjectId id_note) throws KettleException {
		RowMetaAndData table = new RowMetaAndData();
		...
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_TRANS_NOTE_ID_NOTE,
				ValueMetaInterface.TYPE_INTEGER), id_note);

		connectionDelegate.insertTableRow(
				KettleDatabaseRepository.TABLE_R_TRANS_NOTE, table);
	}

	public synchronized void insertJobNote(ObjectId id_job, ObjectId id_note)
			throws KettleException {
		RowMetaAndData table = new RowMetaAndData();

		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_JOB_NOTE_ID_JOB,
				ValueMetaInterface.TYPE_INTEGER), id_job);
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_JOB_NOTE_ID_NOTE,
				ValueMetaInterface.TYPE_INTEGER), id_note);

		connectionDelegate.insertTableRow(
				KettleDatabaseRepository.TABLE_R_JOB_NOTE, table);
	}

	public synchronized void insertStepDatabase(ObjectId id_transformation,
			ObjectId id_step, ObjectId id_database) throws KettleException {
		...
		if (check.getInteger(0) == null) {
			...
			table
					.addValue(
							...
							id_transformation);
			table.addValue(new ValueMeta(
					KettleDatabaseRepository.FIELD_STEP_DATABASE_ID_STEP,
			...
			connectionDelegate.insertTableRow(
					KettleDatabaseRepository.TABLE_R_STEP_DATABASE, table);
		}
	}

	public synchronized void insertJobEntryDatabase(ObjectId id_job,
			ObjectId id_jobentry, ObjectId id_database) throws KettleException {
		// First check if the relationship is already there.
		// There is no need to store it twice!
		...
		if (check.getInteger(0) == null) {
			...
			table.addValue(new ValueMeta(
					KettleDatabaseRepository.FIELD_JOBENTRY_DATABASE_ID_JOB,
					ValueMetaInterface.TYPE_INTEGER), id_job);
			table
					.addValue(
							new ValueMeta(
			...
			connectionDelegate.insertTableRow(
					KettleDatabaseRepository.TABLE_R_JOBENTRY_DATABASE, table);
		}
	}

	public synchronized ObjectId insertTransformationPartitionSchema(
			ObjectId id_transformation, ObjectId id_partition_schema)
			throws KettleException {
		ObjectId id = connectionDelegate
		...
		connectionDelegate.insertTableRow(
				KettleDatabaseRepository.TABLE_R_TRANS_PARTITION_SCHEMA, table);

		return id;
	}

	public synchronized ObjectId insertClusterSlave(
		...
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_CLUSTER_SLAVE_ID_CLUSTER_SLAVE,
				ValueMetaInterface.TYPE_INTEGER), id);
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_CLUSTER_SLAVE_ID_CLUSTER,
				ValueMetaInterface.TYPE_INTEGER), clusterSchema.getObjectId());
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_CLUSTER_SLAVE_ID_SLAVE,
				ValueMetaInterface.TYPE_INTEGER), slaveServer.getObjectId());

		connectionDelegate.insertTableRow(
				KettleDatabaseRepository.TABLE_R_CLUSTER_SLAVE, table);

		return id;
	}

	public synchronized ObjectId insertTransformationCluster(
			ObjectId id_transformation, ObjectId id_cluster)
			throws KettleException {
		...
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_TRANS_CLUSTER_ID_TRANS_CLUSTER,
				ValueMetaInterface.TYPE_INTEGER), id);
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_TRANS_CLUSTER_ID_TRANSFORMATION,
				ValueMetaInterface.TYPE_INTEGER), id_transformation);
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_TRANS_CLUSTER_ID_CLUSTER,
				ValueMetaInterface.TYPE_INTEGER), id_cluster);

		connectionDelegate.insertTableRow(
				KettleDatabaseRepository.TABLE_R_TRANS_CLUSTER, table);

		return id;
	}

	public synchronized ObjectId insertTransformationSlave(
			ObjectId id_transformation, ObjectId id_slave)
			throws KettleException {
		...
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_TRANS_SLAVE_ID_TRANS_SLAVE,
				ValueMetaInterface.TYPE_INTEGER), id);
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_TRANS_SLAVE_ID_TRANSFORMATION,
				ValueMetaInterface.TYPE_INTEGER), id_transformation);
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_TRANS_SLAVE_ID_SLAVE,
				ValueMetaInterface.TYPE_INTEGER), id_slave);

		connectionDelegate.insertTableRow(
				KettleDatabaseRepository.TABLE_R_TRANS_SLAVE, table);

		return id;
	}

	public synchronized void insertTransStepCondition(
			ObjectId id_transformation, ObjectId id_step, ObjectId id_condition)
			throws KettleException {
		...
		table
				.addValue(
						...
						id_transformation);
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_TRANS_STEP_CONDITION_ID_STEP,
				ValueMetaInterface.TYPE_INTEGER), id_step);
		table
				.addValue(
						new ValueMeta(
	...
	public synchronized String[] getTransformationNames(ObjectId id_directory,
	...
	public List<RepositoryElementMetaInterface> getJobObjects(
			ObjectId id_directory, boolean includeDeleted)
			throws KettleException {
		return getRepositoryObjects(
				quoteTable(KettleDatabaseRepository.TABLE_R_JOB),
				RepositoryObjectType.JOB, id_directory);
	}

	public List<RepositoryElementMetaInterface> getTransformationObjects(
			ObjectId id_directory, boolean includeDeleted)
			throws KettleException {
		List<RepositoryElementMetaInterface> objects = getRepositoryObjects(
				quoteTable(KettleDatabaseRepository.TABLE_R_TRANSFORMATION),
				RepositoryObjectType.TRANSFORMATION, id_directory);
		if (objects.size() > 0) {
			System.out.println(objects.get(0).getRepositoryDirectory()
					.getPath());
		}
		return objects;
	}
	 ...
	 * @throws KettleException
	 */
	private synchronized List<RepositoryElementMetaInterface> getRepositoryObjects(
			String tableName, RepositoryObjectType objectType,
			ObjectId id_directory) throws KettleException {
		return connectionDelegate.getRepositoryObjects(tableName, objectType,
				id_directory);
	}

	public synchronized String[] getJobNames(ObjectId id_directory,
			boolean includeDeleted) throws KettleException {
	...
	public synchronized String[] getDirectoryNames(ObjectId id_directory)
			throws KettleException {
		return connectionDelegate
				.getStrings(
						"SELECT "
		...
		return connectionDelegate
				.getIDs(
						"SELECT "
								...
								+ " WHERE "
								+ quote(KettleDatabaseRepository.FIELD_CONDITION_ID_CONDITION_PARENT)
								+ " = ?", id_condition);
	}
	...
	public ObjectId[] getTransNoteIDs(ObjectId id_transformation)
		...
		return connectionDelegate
				.getIDs(
						"SELECT "
								+ quote(KettleDatabaseRepository.FIELD_TRANS_NOTE_ID_NOTE)
								+ " FROM "
								+ quoteTable(KettleDatabaseRepository.TABLE_R_TRANS_NOTE)
								+ " WHERE "
								+ quote(KettleDatabaseRepository.FIELD_TRANS_NOTE_ID_TRANSFORMATION)
								+ " = ?", id_transformation);
	}
	...
	public ObjectId[] getTransformationConditionIDs(ObjectId id_transformation)
		...
		return connectionDelegate
				.getIDs(
						"SELECT "
								+ quote(KettleDatabaseRepository.FIELD_TRANS_STEP_CONDITION_ID_CONDITION)
								+ " FROM "
								+ quoteTable(KettleDatabaseRepository.TABLE_R_TRANS_STEP_CONDITION)
								+ " WHERE "
	...
	public ObjectId[] getTransformationDatabaseIDs(ObjectId id_transformation)
		...
		return connectionDelegate
				.getIDs(
						"SELECT "
								+ quote(KettleDatabaseRepository.FIELD_STEP_DATABASE_ID_DATABASE)
								+ " FROM "
	...
	public ObjectId[] getJobNoteIDs(ObjectId id_job) throws KettleException {
		return connectionDelegate.getIDs("SELECT "
				+ quote(KettleDatabaseRepository.FIELD_JOB_NOTE_ID_NOTE)
				+ " FROM "
				+ quoteTable(KettleDatabaseRepository.TABLE_R_JOB_NOTE)
				+ " WHERE "
				+ quote(KettleDatabaseRepository.FIELD_JOB_NOTE_ID_JOB)
				+ " = ?", id_job);
	}
		...
		return connectionDelegate
				.getIDs(
						"SELECT "
								...
								+ " WHERE "
								+ quote(KettleDatabaseRepository.FIELD_PARTITION_ID_PARTITION_SCHEMA)
								+ " = ? ", id_partition_schema);
	}

	public ObjectId[] getTransformationPartitionSchemaIDs(
			ObjectId id_transformation) throws KettleException {
		return connectionDelegate
				.getIDs(
						"SELECT "
	...
	public ObjectId[] getTransformationClusterSchemaIDs(
			ObjectId id_transformation) throws KettleException {
	...
	public ObjectId[] getClusterSlaveIDs(ObjectId id_cluster_schema)
		...
		return connectionDelegate
				.getIDs(
						"SELECT "
								+ quote(KettleDatabaseRepository.FIELD_CLUSTER_SLAVE_ID_SLAVE)
								+ " FROM "
								+ quoteTable(KettleDatabaseRepository.TABLE_R_CLUSTER_SLAVE)
								+ " WHERE "
								+ quote(KettleDatabaseRepository.FIELD_CLUSTER_SLAVE_ID_CLUSTER)
								+ " = ? ", id_cluster_schema);
	}

	public synchronized String[] getDatabaseNames(boolean includeDeleted)
			throws KettleException {
	...
	public synchronized String[] getPartitionSchemaNames(boolean includeDeleted)
			throws KettleException {
	...
	public synchronized String[] getSlaveNames(boolean includeDeleted)
			throws KettleException {
	...
	public synchronized String[] getClusterNames(boolean includeDeleted)
			throws KettleException {
	...
	public synchronized String[] getTransformationsUsingDatabase(
		...
		ObjectId[] transIds = connectionDelegate.getIDs("SELECT DISTINCT "
				+ quote(FIELD_STEP_DATABASE_ID_TRANSFORMATION) + " FROM "
				+ quoteTable(KettleDatabaseRepository.TABLE_R_STEP_DATABASE)
				+ " WHERE " + quote(FIELD_STEP_DATABASE_ID_DATABASE) + " = ?",
				id_database);

		return transDelegate.getTransformationsWithIDList(transIds);
	}

	public synchronized String[] getJobsUsingDatabase(ObjectId id_database)
			throws KettleException {
		ObjectId[] jobIds = connectionDelegate
				.getIDs(
						"SELECT DISTINCT "
								+ quote(KettleDatabaseRepository.FIELD_JOBENTRY_DATABASE_ID_JOB)
								+ " FROM "
								+ quoteTable(KettleDatabaseRepository.TABLE_R_JOBENTRY_DATABASE)
								+ " WHERE "
								+ quote(KettleDatabaseRepository.FIELD_JOBENTRY_DATABASE_ID_DATABASE)
								+ " = ? ", id_database);

		return jobDelegate.getJobsWithIDList(jobIds);
	}

	public synchronized String[] getClustersUsingSlave(ObjectId id_slave)
			throws KettleException {
		return connectionDelegate.getStrings("SELECT DISTINCT "
				+ quote(FIELD_CLUSTER_SLAVE_ID_CLUSTER) + " FROM "
				+ quoteTable(TABLE_R_CLUSTER_SLAVE) + " WHERE "
				+ quote(FIELD_CLUSTER_SLAVE_ID_SLAVE) + " = ?", id_slave);
	}

	public synchronized String[] getTransformationsUsingSlave(ObjectId id_slave)
			throws KettleException {
		ObjectId[] transIds = connectionDelegate.getIDs("SELECT DISTINCT "
				+ quote(FIELD_TRANS_SLAVE_ID_TRANSFORMATION) + " FROM "
				+ quoteTable(TABLE_R_TRANS_SLAVE) + " WHERE "
	...
	public synchronized String[] getTransformationsUsingCluster(
			ObjectId id_cluster) throws KettleException {
		ObjectId[] transIds = connectionDelegate.getIDs("SELECT DISTINCT "
				+ quote(FIELD_TRANS_CLUSTER_ID_TRANSFORMATION) + " FROM "
				+ quoteTable(TABLE_R_TRANS_CLUSTER) + " WHERE "
		...
		return connectionDelegate
				.getIDs(
						"SELECT "
								...
								+ " WHERE "
								+ quote(KettleDatabaseRepository.FIELD_JOBENTRY_COPY_ID_JOB)
								+ " = ?", id_job);
	}

	public ObjectId[] getJobEntryCopyIDs(ObjectId id_job, ObjectId id_jobentry)
			throws KettleException {
		return connectionDelegate
				.getIDs(
						"SELECT "
								...
								+ quote(KettleDatabaseRepository.FIELD_JOBENTRY_COPY_ID_JOB)
								+ " = ? AND "
								+ quote(KettleDatabaseRepository.FIELD_JOBENTRY_COPY_ID_JOBENTRY)
								+ " = ? ", id_job, id_jobentry);
	}

	private RowMetaAndData getStepDatabase(ObjectId id_step)
			throws KettleException {
		return connectionDelegate.getOneRow(
				quoteTable(KettleDatabaseRepository.TABLE_R_STEP_DATABASE),
				quote(KettleDatabaseRepository.FIELD_STEP_DATABASE_ID_STEP),
				id_step);
	}

	private RowMetaAndData getJobEntryDatabase(ObjectId id_jobentry)
			throws KettleException {
		return connectionDelegate
				.getOneRow(
						quoteTable(KettleDatabaseRepository.TABLE_R_JOBENTRY_DATABASE),
						quote(KettleDatabaseRepository.FIELD_JOBENTRY_DATABASE_ID_JOBENTRY),
						id_jobentry);
	}
	...
	public synchronized void delSteps(ObjectId id_transformation)
			throws KettleException {
	...
	public synchronized void deleteCondition(ObjectId id_condition)
			throws KettleException {
		boolean ok = true;
	...
	public synchronized void delStepConditions(ObjectId id_transformation)
			throws KettleException {
	 ...
	 */
	public synchronized void delStepDatabases(ObjectId id_transformation)
			throws KettleException {
		connectionDelegate
				.performDelete(
						"DELETE FROM "
	 ...
	 *             in case something unexpected happens.
	 */
	public synchronized void delJobEntryDatabases(ObjectId id_job)
			throws KettleException {
		connectionDelegate
				.performDelete(
						"DELETE FROM "
	...
	public synchronized void delJobEntries(ObjectId id_job)
			throws KettleException {
		connectionDelegate.performDelete("DELETE FROM "
				+ quoteTable(KettleDatabaseRepository.TABLE_R_JOBENTRY)
				+ " WHERE "
	...
	public synchronized void delJobEntryCopies(ObjectId id_job)
			throws KettleException {
		connectionDelegate.performDelete("DELETE FROM "
				+ quoteTable(KettleDatabaseRepository.TABLE_R_JOBENTRY_COPY)
				+ " WHERE "
	...
	public synchronized void delDependencies(ObjectId id_transformation)
			throws KettleException {
		connectionDelegate
				.performDelete(
						"DELETE FROM "
	...
	public synchronized void delStepAttributes(ObjectId id_transformation)
			throws KettleException {
		connectionDelegate
				.performDelete(
						"DELETE FROM "
								...
								+ " WHERE "
								+ quote(KettleDatabaseRepository.FIELD_STEP_ATTRIBUTE_ID_TRANSFORMATION)
								+ " = ? ", id_transformation);
	}

	public synchronized void delTransAttributes(ObjectId id_transformation)
			throws KettleException {
		connectionDelegate
				.performDelete(
						"DELETE FROM "
								...
								+ " WHERE "
								+ quote(KettleDatabaseRepository.FIELD_TRANS_ATTRIBUTE_ID_TRANSFORMATION)
								+ " = ? ", id_transformation);
	}

	public synchronized void delJobAttributes(ObjectId id_job)
			throws KettleException {
		connectionDelegate.performDelete("DELETE FROM "
				+ quoteTable(KettleDatabaseRepository.TABLE_R_JOB_ATTRIBUTE)
				+ " WHERE "
				+ quote(KettleDatabaseRepository.FIELD_JOB_ATTRIBUTE_ID_JOB)
				+ " = ? ", id_job);
	}

	public synchronized void delPartitionSchemas(ObjectId id_transformation)
			throws KettleException {
		connectionDelegate
				.performDelete(
						"DELETE FROM "
	...
	public synchronized void delPartitions(ObjectId id_partition_schema)
			throws KettleException {
		connectionDelegate
				.performDelete(
						"DELETE FROM "
	...
	public synchronized void delClusterSlaves(ObjectId id_cluster)
			throws KettleException {
		connectionDelegate
				.performDelete(
						"DELETE FROM "
	...
	public synchronized void delTransformationClusters(
			ObjectId id_transformation) throws KettleException {
		connectionDelegate
				.performDelete(
						"DELETE FROM "
								+ quoteTable(KettleDatabaseRepository.TABLE_R_TRANS_CLUSTER)
								+ " WHERE "
								+ quote(KettleDatabaseRepository.FIELD_TRANS_CLUSTER_ID_TRANSFORMATION)
								+ " = ? ", id_transformation);
	}

	public synchronized void delTransformationSlaves(ObjectId id_transformation)
			throws KettleException {
		connectionDelegate
				.performDelete(
						"DELETE FROM "
								...
								+ " WHERE "
								+ quote(KettleDatabaseRepository.FIELD_TRANS_SLAVE_ID_TRANSFORMATION)
								+ " = ? ", id_transformation);
	}

	public synchronized void delJobEntryAttributes(ObjectId id_job)
			throws KettleException {
		connectionDelegate
				.performDelete(
						"DELETE FROM "
								+ quoteTable(KettleDatabaseRepository.TABLE_R_JOBENTRY_ATTRIBUTE)
								+ " WHERE "
								+ quote(KettleDatabaseRepository.FIELD_JOBENTRY_ATTRIBUTE_ID_JOB)
								+ " = ? ", id_job);
	}

	public synchronized void delTransHops(ObjectId id_transformation)
			throws KettleException {
		connectionDelegate
				.performDelete(
						"DELETE FROM "
	...
	public synchronized void delTransNotes(ObjectId id_transformation)
			throws KettleException {
		ObjectId ids[] = getTransNoteIDs(id_transformation);

		for (int i = 0; i < ids.length; i++) {
			connectionDelegate.performDelete("DELETE FROM "
					+ quoteTable(KettleDatabaseRepository.TABLE_R_NOTE)
					+ " WHERE "
	...
	public synchronized void delJobNotes(ObjectId id_job)
			throws KettleException {
	...
	public synchronized void delTrans(ObjectId id_transformation)
			throws KettleException {
		securityProvider
				.validateAction(RepositoryOperation.DELETE_TRANSFORMATION);
	...
	public synchronized void delTransStepCondition(ObjectId id_transformation)
			throws KettleException {
		connectionDelegate
				.performDelete(
						"DELETE FROM "
	...
	public synchronized void deleteSlave(ObjectId id_slave)
			throws KettleException {
		securityProvider
				.validateAction(RepositoryOperation.DELETE_SLAVE_SERVER);
			...
			connectionDelegate
					.performDelete(
							"DELETE FROM "
									+ quoteTable(KettleDatabaseRepository.TABLE_R_TRANS_SLAVE)
									+ " WHERE "
									+ quote(KettleDatabaseRepository.FIELD_TRANS_SLAVE_ID_SLAVE)
									+ " = ? ", id_slave);
			commit();
		} else {
	...
	public synchronized void deletePartitionSchema(ObjectId id_partition_schema)
			throws KettleException {
		partitionSchemaDelegate.delPartitionSchema(id_partition_schema);
		commit();
	}

	public synchronized void deleteClusterSchema(ObjectId id_cluster)
			throws KettleException {
		clusterSchemaDelegate.delClusterSchema(id_cluster);
		commit();
	}

	public synchronized void deleteTransformation(ObjectId id_transformation)
			throws KettleException {
	...
	public boolean dropRepositorySchema() throws KettleException {
		...
		for (int i = 0; i < repositoryTableNames.length; i++) {
			try {
				execStatement("DROP TABLE "
						+ quoteTable(repositoryTableNames[i]));
				if (log.isDetailed())
					log.logDetailed("dropped table " + repositoryTableNames[i]);
			} catch (KettleException dbe) {
				if (log.isDetailed())
					log.logDetailed("Unable to drop table: "
							+ repositoryTableNames[i]);
			}
		}
		log.logBasic("Dropped all " + repositoryTableNames.length
				+ " repository tables.");
	...
	public void updateDatabaseTypes() throws KettleException {
		creationHelper.updateDatabaseTypes(new ArrayList<String>(), false,
				false);
	}
	...
	public List<SlaveServer> getSlaveServers() throws KettleException {
		...
		for (int i = 0; i < slaveIDs.length; i++) {
			SlaveServer slaveServer = loadSlaveServer(slaveIDs[i], null); // Load
			// last
			// version
	...
	public List<DatabaseMeta> readDatabases() throws KettleException {
		...
		for (int i = 0; i < ids.length; i++) {
			DatabaseMeta databaseMeta = loadDatabaseMeta(ids[i], null); // reads
			// last
			// versions
	...
	//
	public synchronized int countNrStepAttributes(ObjectId id_step, String code)
			throws KettleException {
		return connectionDelegate.countNrStepAttributes(id_step, code);
	}

	public synchronized int countNrJobEntryAttributes(ObjectId id_jobentry,
	...
	public boolean getJobEntryAttributeBoolean(ObjectId id_jobentry, String code)
			throws KettleException {
		return connectionDelegate
				.getJobEntryAttributeBoolean(id_jobentry, code);
	}

	public boolean getJobEntryAttributeBoolean(ObjectId id_jobentry, int nr,
			String code) throws KettleException {
		return connectionDelegate.getJobEntryAttributeBoolean(id_jobentry, nr,
				code);
	}

	public boolean getJobEntryAttributeBoolean(ObjectId id_jobentry,
			String code, boolean def) throws KettleException {
		return connectionDelegate.getJobEntryAttributeBoolean(id_jobentry,
				code, def);
	}

	public long getJobEntryAttributeInteger(ObjectId id_jobentry, String code)
			throws KettleException {
		return connectionDelegate
				.getJobEntryAttributeInteger(id_jobentry, code);
	}

	public long getJobEntryAttributeInteger(ObjectId id_jobentry, int nr,
			String code) throws KettleException {
		return connectionDelegate.getJobEntryAttributeInteger(id_jobentry, nr,
				code);
	}

	public String getJobEntryAttributeString(ObjectId id_jobentry, String code)
			throws KettleException {
		return connectionDelegate.getJobEntryAttributeString(id_jobentry, code);
	}

	public String getJobEntryAttributeString(ObjectId id_jobentry, int nr,
			String code) throws KettleException {
		return connectionDelegate.getJobEntryAttributeString(id_jobentry, nr,
				code);
	}

	// put

	public void saveJobEntryAttribute(ObjectId id_job, ObjectId id_jobentry,
			int nr, String code, String value) throws KettleException {
		connectionDelegate.saveJobEntryAttribute(id_job, id_jobentry, nr, code,
				value);
	}

	public void saveJobEntryAttribute(ObjectId id_job, ObjectId id_jobentry,
			String code, String value) throws KettleException {
		connectionDelegate.saveJobEntryAttribute(id_job, id_jobentry, code,
				value);
	}

	public void saveJobEntryAttribute(ObjectId id_job, ObjectId id_jobentry,
			int nr, String code, boolean value) throws KettleException {
		connectionDelegate.saveJobEntryAttribute(id_job, id_jobentry, nr, code,
				value);
	}

	public void saveJobEntryAttribute(ObjectId id_job, ObjectId id_jobentry,
			String code, boolean value) throws KettleException {
		connectionDelegate.saveJobEntryAttribute(id_job, id_jobentry, code,
				value);
	}

	public void saveJobEntryAttribute(ObjectId id_job, ObjectId id_jobentry,
			int nr, String code, long value) throws KettleException {
		connectionDelegate.saveJobEntryAttribute(id_job, id_jobentry, nr, code,
				value);
	}

	public void saveJobEntryAttribute(ObjectId id_job, ObjectId id_jobentry,
			String code, long value) throws KettleException {
		connectionDelegate.saveJobEntryAttribute(id_job, id_jobentry, code,
				value);
	}
	...
	public boolean getStepAttributeBoolean(ObjectId id_step, int nr,
			String code, boolean def) throws KettleException {
		return connectionDelegate.getStepAttributeBoolean(id_step, nr, code,
				def);
	}

	public boolean getStepAttributeBoolean(ObjectId id_step, int nr, String code)
			throws KettleException {
		return connectionDelegate.getStepAttributeBoolean(id_step, nr, code);
	}

	public boolean getStepAttributeBoolean(ObjectId id_step, String code)
			throws KettleException {
		return connectionDelegate.getStepAttributeBoolean(id_step, code);
	}

	public long getStepAttributeInteger(ObjectId id_step, int nr, String code)
			throws KettleException {
		return connectionDelegate.getStepAttributeInteger(id_step, nr, code);
	}

	public long getStepAttributeInteger(ObjectId id_step, String code)
			throws KettleException {
		return connectionDelegate.getStepAttributeInteger(id_step, code);
	}

	public String getStepAttributeString(ObjectId id_step, int nr, String code)
			throws KettleException {
		return connectionDelegate.getStepAttributeString(id_step, nr, code);
	}

	public String getStepAttributeString(ObjectId id_step, String code)
			throws KettleException {
	...
	public void saveStepAttribute(ObjectId id_transformation, ObjectId id_step,
			int nr, String code, String value) throws KettleException {
		connectionDelegate.saveStepAttribute(id_transformation, id_step, nr,
				code, value);
	}

	public void saveStepAttribute(ObjectId id_transformation, ObjectId id_step,
			String code, String value) throws KettleException {
		connectionDelegate.saveStepAttribute(id_transformation, id_step, code,
				value);
	}
	...
	public void saveStepAttribute(ObjectId id_transformation, ObjectId id_step,
			int nr, String code, double value) throws KettleException {
		connectionDelegate.saveStepAttribute(id_transformation, id_step, nr,
				code, value);
	}

	public void saveStepAttribute(ObjectId id_transformation, ObjectId id_step,
			String code, double value) throws KettleException {
		connectionDelegate.saveStepAttribute(id_transformation, id_step, code,
				value);
	}

	/**
	 * This method is only used to check backward compatibility with the 2.x
	 * series.
	 * 
	 * @param id_step
	 * @param nr
	 ...
	 * @throws KettleException
	 */
	public ObjectId findStepAttributeID(ObjectId id_step, int nr, String code)
			throws KettleException {
	...
	public void loadJobEntry(JobEntryBase jobEntryBase, ObjectId id_jobentry,
			List<DatabaseMeta> databases, List<SlaveServer> slaveServers)
			throws KettleException {
		jobEntryDelegate.loadJobEntryBase(jobEntryBase, id_jobentry, databases,
				slaveServers);
	}
	...
	public ObjectId getTransformationID(String name,
			RepositoryDirectoryInterface repositoryDirectory)
			throws KettleException {
		return transDelegate.getTransformationID(name, repositoryDirectory
				.getObjectId());
	}

	public ObjectId insertJobEntry(ObjectId id_job, JobEntryBase jobEntryBase)
			throws KettleException {
	...
	public DatabaseMeta loadDatabaseMetaFromStepAttribute(ObjectId idStep,
		...
		return DatabaseMeta.findDatabase(databases, new LongObjectId(
				id_database));
	}

	/**
	 * This method saves the object ID of the database object (if not null) in
	 * the step attributes
	 * 
	 * @param id_transformation
	 * @param id_step
	...
	public void saveDatabaseMetaStepAttribute(ObjectId id_transformation,
			ObjectId id_step, String code, DatabaseMeta database)
			throws KettleException {
		ObjectId id = null;
		if (database != null) {
			id = database.getObjectId();
			Long id_database = id == null ? Long.valueOf(-1L)
					: new LongObjectId(id).longValue();
	 ...
	 */
	public void saveDatabaseMetaJobEntryAttribute(ObjectId id_job,
			ObjectId id_jobentry, int nr, String nameCode, String idCode,
			DatabaseMeta database) throws KettleException {
		ObjectId id = null;
		if (database != null) {
			id = database.getObjectId();
			...
			saveJobEntryAttribute(id_job, id_jobentry, nr, nameCode,
					id_database);
	...
	/**
	 * Load a condition from the repository with the Object ID stored in a step
	 * attribute.
	 * 
	 * @param id_step
	 * @param code
	...
	public Condition loadConditionFromStepAttribute(ObjectId id_step,
		...
		if (id_condition > 0) {
			return loadCondition(new LongObjectId(id_condition)); // this
			// repository
			// uses
			// longs
		} else {
	...
	/**
	 * This method saves the object ID of the condition object (if not null) in
	 * the step attributes
	 * 
	 * @param id_step
	 * @param code
	...
	public void saveConditionStepAttribute(ObjectId id_transformation,
			ObjectId id_step, String code, Condition condition)
			throws KettleException {
		ObjectId id = null;
		if (condition != null) {
			id = saveCondition(condition);
			Long id_condition = id == null ? Long.valueOf(-1L)
					: new LongObjectId(id).longValue();
			saveStepAttribute(id_transformation, id_step, code, id_condition);

			insertTransStepCondition(id_transformation, id_step, condition
					.getObjectId());
		}
	}
	...
	public List<RepositoryElementMetaInterface> getJobAndTransformationObjects(
			ObjectId id_directory, boolean includeDeleted)
			throws KettleException {
	...
	public RepositoryDirectory getDefaultSaveDirectory(
	...
	public RepositoryObject getObjectInformation(ObjectId objectId,
		...
		try {
			...
			case TRANSFORMATION: {
				...
				long dirId = row
						.getInteger(
								...
								0);
				RepositoryDirectoryInterface directory = loadRepositoryDirectoryTree()
						.findDirectory(new LongObjectId(dirId));
				return new RepositoryObject(objectId, name, directory,
						modifiedUser, modifiedDate, objectType, description,
						false);
			}
			...
			case JOB: {
				...
				String name = row.getString(
						KettleDatabaseRepository.FIELD_JOB_NAME, null);
				String description = row.getString(
						KettleDatabaseRepository.FIELD_JOB_DESCRIPTION, null);
				String modifiedUser = row.getString(
						KettleDatabaseRepository.FIELD_JOB_MODIFIED_USER, "-");
				Date modifiedDate = row.getDate(
						KettleDatabaseRepository.FIELD_JOB_MODIFIED_DATE, null);
				long dirId = row.getInteger(
						KettleDatabaseRepository.FIELD_JOB_ID_DIRECTORY, 0);
				RepositoryDirectoryInterface directory = loadRepositoryDirectoryTree()
						.findDirectory(new LongObjectId(dirId));
				return new RepositoryObject(objectId, name, directory,
						modifiedUser, modifiedDate, objectType, description,
	...
	public JobMeta loadJob(ObjectId idJob, String versionLabel)
			throws KettleException {
		RepositoryObject jobInfo = getObjectInformation(idJob,
				RepositoryObjectType.JOB);
		return loadJob(jobInfo.getName(), jobInfo.getRepositoryDirectory(),
				null, versionLabel);
	}

	public TransMeta loadTransformation(ObjectId idTransformation,
			String versionLabel) throws KettleException {
		RepositoryObject jobInfo = getObjectInformation(idTransformation,
				RepositoryObjectType.TRANSFORMATION);
		return loadTransformation(jobInfo.getName(), jobInfo
				.getRepositoryDirectory(), null, true, versionLabel);
	}

...
public class KettleDatabaseRepositorySecurityProvider extends BaseRepositorySecurityProvider implements
  ...
  public IUser loadUserInfo(ObjectId id_user) throws KettleException {
	    return userDelegate.loadUserInfo(id_user);
  }
  ...
  public synchronized void delUser(ObjectId id_user) throws KettleException {
    ...
    repository.connectionDelegate.performDelete("DELETE FROM " + repository.quoteTable(KettleDatabaseRepository.TABLE_R_USER_ROLE) + 
			" WHERE " + repository.quote(KettleDatabaseRepository.FIELD_USER_ROLE_UID) + " = ?", id_user);
  }
  ...
  public void deleteRole(ObjectId roleId) throws KettleException {
    repository.connectionDelegate.performDelete("DELETE FROM " + repository.quoteTable(KettleDatabaseRepository.TABLE_R_ROLE) + 
        " WHERE " + repository.quote(KettleDatabaseRepository.FIELD_ROLE_ID_ROLE) + " = ? ", roleId);
    repository.connectionDelegate.performDelete("DELETE FROM " + repository.quoteTable(KettleDatabaseRepository.TABLE_R_USER_ROLE) + 
			" WHERE " + repository.quote(KettleDatabaseRepository.FIELD_USER_ROLE_RID) + " = ?", roleId);
    repository.connectionDelegate.performDelete("DELETE FROM " + repository.quoteTable(KettleDatabaseRepository.TABLE_R_AUTHORIZATION) + 
			" WHERE " + repository.quote(KettleDatabaseRepository.FIELD_AUTHORIZATION_RID) + " = ?", roleId);
    repository.commit();
  }
}

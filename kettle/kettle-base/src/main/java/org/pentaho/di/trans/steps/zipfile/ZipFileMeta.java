...
public class ZipFileMeta extends BaseStepMeta implements StepMetaInterface {
	private static Class<?> PKG = ZipFileMeta.class; // for i18n purposes,
														// needed by
														// Translator2!!
														// $NON-NLS-1$
	 ...
	 */
	public final static String operationTypeCode[] = { "", "move", "delete" };

	public final static int OPERATION_TYPE_NOTHING = 0;
	...
	public String getXML() {
		...
		retval
				.append("    " + XMLHandler.addTagValue("sourcefilenamefield", sourcefilenamefield)); //$NON-NLS-1$ //$NON-NLS-2$
		retval.append("    "
				+ XMLHandler.addTagValue("targetfilenamefield",
						targetfilenamefield));
		retval.append("    "
				+ XMLHandler.addTagValue("baseFolderField", baseFolderField));
		retval.append("    "
				+ XMLHandler.addTagValue("operation_type",
						getOperationTypeCode(operationType)));
		retval.append("    ").append(
				XMLHandler
						.addTagValue("addresultfilenames", addresultfilenames));
		retval.append("    ").append(
				XMLHandler.addTagValue("overwritezipentry", overwritezipentry));
		retval.append("    ").append(
				XMLHandler
						.addTagValue("createparentfolder", createparentfolder));
		retval.append("    ").append(
				XMLHandler.addTagValue("keepsourcefolder", keepsourcefolder));
		retval.append("    "
				+ XMLHandler
						.addTagValue("movetofolderfield", movetofolderfield));
	...
	private void readData(Node stepnode) throws KettleXMLException {
		try {
			sourcefilenamefield = XMLHandler.getTagValue(stepnode,
					"sourcefilenamefield");
			targetfilenamefield = XMLHandler.getTagValue(stepnode,
					"targetfilenamefield");
			baseFolderField = XMLHandler.getTagValue(stepnode,
					"baseFolderField");
			operationType = getOperationTypeByCode(Const.NVL(XMLHandler
					.getTagValue(stepnode, "operation_type"), ""));
			addresultfilenames = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "addresultfilenames"));
			overwritezipentry = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "overwritezipentry"));
			createparentfolder = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "createparentfolder"));
			keepsourcefolder = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "keepsourcefolder"));
			movetofolderfield = XMLHandler.getTagValue(stepnode,
					"movetofolderfield");

		} catch (Exception e) {
			throw new KettleXMLException(BaseMessages.getString(PKG,
					"ZipFileMeta.Exception.UnableToReadStepInfo"), e); //$NON-NLS-1$
		}
	}
	...
	public void readRep(Repository rep, ObjectId id_step,
		...
		try {
			sourcefilenamefield = rep.getStepAttributeString(id_step,
					"sourcefilenamefield"); //$NON-NLS-1$
			targetfilenamefield = rep.getStepAttributeString(id_step,
					"targetfilenamefield");
			baseFolderField = rep.getStepAttributeString(id_step,
					"baseFolderField");
			operationType = getOperationTypeByCode(Const.NVL(rep
					.getStepAttributeString(id_step, "operation_type"), ""));
			addresultfilenames = rep.getStepAttributeBoolean(id_step,
					"addresultfilenames");
			overwritezipentry = rep.getStepAttributeBoolean(id_step,
					"overwritezipentry");
			createparentfolder = rep.getStepAttributeBoolean(id_step,
					"createparentfolder");
			keepsourcefolder = rep.getStepAttributeBoolean(id_step,
					"keepsourcefolder");
			movetofolderfield = rep.getStepAttributeString(id_step,
					"movetofolderfield");

		} catch (Exception e) {
			throw new KettleException(BaseMessages.getString(PKG,
					"ZipFileMeta.Exception.UnexpectedErrorReadingStepInfo"), e); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, ObjectId id_transformation,
		...
		try {
			rep.saveStepAttribute(id_transformation, id_step,
					"sourcefilenamefield", sourcefilenamefield); //$NON-NLS-1$
			rep.saveStepAttribute(id_transformation, id_step,
					"targetfilenamefield", targetfilenamefield); //$NON-NLS-1$
			rep.saveStepAttribute(id_transformation, id_step,
					"baseFolderField", baseFolderField);
			rep.saveStepAttribute(id_transformation, id_step, "operation_type",
					getOperationTypeCode(operationType));
			rep.saveStepAttribute(id_transformation, id_step,
					"addresultfilenames", addresultfilenames);
			rep.saveStepAttribute(id_transformation, id_step,
					"overwritezipentry", overwritezipentry);
			rep.saveStepAttribute(id_transformation, id_step,
					"createparentfolder", createparentfolder);
			rep.saveStepAttribute(id_transformation, id_step,
					"keepsourcefolder", keepsourcefolder);
			rep.saveStepAttribute(id_transformation, id_step,
					"movetofolderfield", movetofolderfield);

		} catch (Exception e) {
			throw new KettleException(BaseMessages.getString(PKG,
					"ZipFileMeta.Exception.UnableToSaveStepInfo") + id_step, e); //$NON-NLS-1$
		}
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (Const.isEmpty(sourcefilenamefield)) {
			error_message = BaseMessages.getString(PKG,
					"ZipFileMeta.CheckResult.SourceFileFieldMissing"); //$NON-NLS-1$
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, error_message,
					stepMeta);
			remarks.add(cr);
		} else {
			error_message = BaseMessages.getString(PKG,
					"ZipFileMeta.CheckResult.TargetFileFieldOK"); //$NON-NLS-1$
		...
		if (input.length > 0) {
			cr = new CheckResult(
					...
					BaseMessages
							.getString(PKG,
									"ZipFileMeta.CheckResult.ReceivingInfoFromOtherSteps"), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		} else {
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG,
							"ZipFileMeta.CheckResult.NoInpuReceived"), stepMeta); //$NON-NLS-1$
			remarks.add(cr);
		}

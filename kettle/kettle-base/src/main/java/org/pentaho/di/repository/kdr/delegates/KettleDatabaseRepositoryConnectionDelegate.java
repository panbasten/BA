...
public class KettleDatabaseRepositoryConnectionDelegate extends
	...
	public synchronized void connect(boolean no_lookup, boolean ignoreVersion)
			throws KettleException {
		if (repository.isConnected()) {
			throw new KettleException("Repository is already by class "
			...
			if (!no_lookup) {
				try {
					...
					throw new KettleException(
							"Error setting lookup prep.statements", dbe);
				}
			}
	...
	protected void verifyVersion() throws KettleException {
		...
		String versionTable = databaseMeta.getQuotedSchemaTableCombination(
				null, KettleDatabaseRepository.TABLE_R_VERSION);
		try {
			lastUpgrade = database
					.getOneRow("SELECT "
							+ quote(KettleDatabaseRepository.FIELD_VERSION_MAJOR_VERSION)
							+ ", "
							+ quote(KettleDatabaseRepository.FIELD_VERSION_MINOR_VERSION)
							+ ", "
							+ quote(KettleDatabaseRepository.FIELD_VERSION_UPGRADE_DATE)
							+ " FROM "
							+ versionTable
							+ " ORDER BY "
							+ quote(KettleDatabaseRepository.FIELD_VERSION_UPGRADE_DATE)
			...
			try {
				...
				String userTable = databaseMeta
						.getQuotedSchemaTableCombination(null,
								KettleDatabaseRepository.TABLE_R_USER);
		...
		if (lastUpgrade != null) {
			majorVersion = (int) lastUpgrade.getInteger(
					KettleDatabaseRepository.FIELD_VERSION_MAJOR_VERSION, -1);
			minorVersion = (int) lastUpgrade.getInteger(
					KettleDatabaseRepository.FIELD_VERSION_MINOR_VERSION, -1);
		}
		...
		if (majorVersion == 3 && minorVersion == 0) {
			...
			String tableName = databaseMeta.getQuotedSchemaTableCombination(
					null,
					KettleDatabaseRepository.TABLE_R_TRANS_PARTITION_SCHEMA);
	...
	public synchronized void disconnect() {
		try {
			...
			log.logError("Error disconnecting from database : "
					+ dbe.getMessage());
		} finally {
			database.disconnect();
			sqlMap.clear();
	...
	public synchronized void fillStepAttributesBuffer(ObjectId id_transformation)
			throws KettleException {
		String sql = "SELECT "
				...
				+ databaseMeta.getQuotedSchemaTableCombination(null,
						KettleDatabaseRepository.TABLE_R_STEP_ATTRIBUTE)
				+ " "
				+ "WHERE "
		...
		ResultSet resultSet = database.openQuery(ps, parameter.getRowMeta(),
				parameter.getData());
	...
	private synchronized RowMetaAndData searchStepAttributeInBuffer(
			ObjectId id_step, String code, long nr) throws KettleValueException {
	...
	private synchronized int searchStepAttributeIndexInBuffer(ObjectId id_step,
	...
	private synchronized int searchNrStepAttributes(ObjectId id_step,
			String code) throws KettleValueException {
		...
		while (lookID == new LongObjectId(id_step).longValue()
				&& code.equalsIgnoreCase(lookCode)) {
			// Find the maximum
			//
	...
	public synchronized void setLookupTransAttribute() throws KettleException {
		String sql = "SELECT "
				...
				+ " FROM "
				+ databaseMeta.getQuotedSchemaTableCombination(null,
						KettleDatabaseRepository.TABLE_R_TRANS_ATTRIBUTE)
				+ " WHERE "
	...
	public synchronized void setLookupJobAttribute() throws KettleException {
		String sql = "SELECT "
				...
				+ " FROM "
				+ databaseMeta.getQuotedSchemaTableCombination(null,
						KettleDatabaseRepository.TABLE_R_JOB_ATTRIBUTE)
				+ " WHERE "
				+ quote(KettleDatabaseRepository.FIELD_JOB_ATTRIBUTE_ID_JOB)
				+ " = ?  AND "
				+ quote(KettleDatabaseRepository.FIELD_JOB_ATTRIBUTE_CODE)
				+ " = ? AND " + KettleDatabaseRepository.FIELD_JOB_ATTRIBUTE_NR
				+ " = ? ";

		psJobAttributesLookup = database.prepareSQL(sql);
	}

	public synchronized void closeJobAttributeLookupPreparedStatement()
			throws KettleException {
		database.closePreparedStatement(psTransAttributesLookup);
	...
	public synchronized void closeStepAttributeInsertPreparedStatement()
		...
		if (psStepAttributesInsert != null) {
			database.emptyAndCommit(psStepAttributesInsert, useBatchProcessing,
					1); // batch mode!
			psStepAttributesInsert = null;
		}
	}
	...
	public synchronized void closeTransAttributeInsertPreparedStatement()
		...
		if (psTransAttributesInsert != null) {
			database.emptyAndCommit(psTransAttributesInsert,
					useBatchProcessing, 1); // batch mode!
			psTransAttributesInsert = null;
		}
	}

	public synchronized void closeJobAttributeInsertPreparedStatement()
			throws KettleException {
		if (psJobAttributesInsert != null) {
			database.emptyAndCommit(psJobAttributesInsert, useBatchProcessing,
					1); // batch mode!
			psJobAttributesInsert = null;
		}
	}

	private RowMetaAndData getStepAttributeRow(ObjectId id_step, int nr,
		...
		database.setValues(par.getRowMeta(), par.getData(),
	...
	public RowMetaAndData getJobAttributeRow(ObjectId id_job, int nr,
			String code) throws KettleException {
		RowMetaAndData par = new RowMetaAndData();
		par.addValue(new ValueMeta(
	...
	public synchronized long getStepAttributeInteger(ObjectId id_step, int nr,
			String code) throws KettleException {
		...
		if (r == null)
			return -1L;
		long id = r.getInteger(
				KettleDatabaseRepository.FIELD_STEP_ATTRIBUTE_VALUE_NUM, -1L);
		return id;
	}

	public synchronized ObjectId findStepAttributeID(ObjectId id_step, int nr,
			String code) throws KettleException {
		RowMetaAndData r = null;
		if (stepAttributesBuffer != null)
			r = searchStepAttributeInBuffer(id_step, code, (long) nr);
		...
		long id = r.getInteger(
				KettleDatabaseRepository.FIELD_STEP_ATTRIBUTE_ID_STEP, -1L);
		if (id < 0)
			return null;
	...
	public synchronized String getStepAttributeString(ObjectId id_step, int nr,
			String code) throws KettleException {
		RowMetaAndData r = null;
		if (stepAttributesBuffer != null)
			r = searchStepAttributeInBuffer(id_step, code, (long) nr);
		...
		if (r == null)
			return null;
		return r.getString(
				KettleDatabaseRepository.FIELD_STEP_ATTRIBUTE_VALUE_STR, null);
	}
	...
	public synchronized long getStepAttributeInteger(ObjectId id_step,
			String code) throws KettleException {
		return getStepAttributeInteger(id_step, 0, code);
	}

	public synchronized String getStepAttributeString(ObjectId id_step,
			String code) throws KettleException {
	...
	public synchronized ObjectId saveStepAttribute(ObjectId id_transformation,
	...
	public synchronized ObjectId saveStepAttribute(ObjectId id_transformation,
			ObjectId id_step, String code, boolean value)
			throws KettleException {
		return saveStepAttribute(code, 0, id_transformation, id_step, 0.0,
				value ? "Y" : "N");
	}
	...
	public synchronized ObjectId saveStepAttribute(ObjectId id_transformation,
			ObjectId id_step, long nr, String code, String value)
			throws KettleException {
	...
	public synchronized ObjectId saveStepAttribute(ObjectId id_transformation,
			ObjectId id_step, long nr, String code, double value)
			throws KettleException {
	...
	public synchronized ObjectId saveStepAttribute(ObjectId id_transformation,
			ObjectId id_step, long nr, String code, boolean value)
			throws KettleException {
		return saveStepAttribute(code, nr, id_transformation, id_step, 0.0,
				value ? "Y" : "N");
	}

	private ObjectId saveStepAttribute(String code, long nr,
			ObjectId id_transformation, ObjectId id_step, double value_num,
	...
	public synchronized int countNrStepAttributes(ObjectId id_step, String code)
		...
		if (stepAttributesBuffer != null) // see if we can do this in memory...
		{
			...
			table.addValue(new ValueMeta(
					KettleDatabaseRepository.FIELD_STEP_ATTRIBUTE_CODE,
					ValueMetaInterface.TYPE_STRING), code);
			RowMetaAndData r = database.getOneRow(sql, table.getRowMeta(),
					table.getData());
	...
	public synchronized String getTransAttributeString(
			ObjectId id_transformation, int nr, String code)
			throws KettleException {
		RowMetaAndData r = null;
		...
		if (r == null)
			return null;
		return r.getString(
				KettleDatabaseRepository.FIELD_TRANS_ATTRIBUTE_VALUE_STR, null);
	}
	...
	public synchronized double getTransAttributeNumber(
			ObjectId id_transformation, int nr, String code)
			throws KettleException {
		RowMetaAndData r = null;
		r = getTransAttributeRow(id_transformation, nr, code);
		if (r == null)
			return 0.0;
		return r.getNumber(
				KettleDatabaseRepository.FIELD_TRANS_ATTRIBUTE_VALUE_NUM, 0.0);
	}

	public synchronized long getTransAttributeInteger(
			ObjectId id_transformation, int nr, String code)
			throws KettleException {
		RowMetaAndData r = null;
		r = getTransAttributeRow(id_transformation, nr, code);
		if (r == null)
			return 0L;
		return r.getInteger(
				KettleDatabaseRepository.FIELD_TRANS_ATTRIBUTE_VALUE_NUM, 0L);
	}

	public synchronized int countNrTransAttributes(ObjectId id_transformation,
			String code) throws KettleException {
		String sql = "SELECT COUNT(*) FROM "
				+ databaseMeta.getQuotedSchemaTableCombination(null,
						KettleDatabaseRepository.TABLE_R_TRANS_ATTRIBUTE)
		...
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_TRANS_ATTRIBUTE_CODE,
				ValueMetaInterface.TYPE_STRING), code);
		RowMetaAndData r = database.getOneRow(sql, table.getRowMeta(), table
				.getData());
		if (r == null || r.getData() == null)
			return 0;
	...
	public synchronized List<Object[]> getTransAttributes(
			ObjectId id_transformation, String code, long nr)
			throws KettleException {
		String sql = "SELECT *"
				+ " FROM "
				+ databaseMeta.getQuotedSchemaTableCombination(null,
				...
				+ quote(KettleDatabaseRepository.FIELD_TRANS_ATTRIBUTE_CODE)
				+ " = ? AND "
				+ quote(KettleDatabaseRepository.FIELD_TRANS_ATTRIBUTE_NR)
				+ " = ?"
				+ " ORDER BY "
				+ quote(KettleDatabaseRepository.FIELD_TRANS_ATTRIBUTE_VALUE_NUM);
	...
	public synchronized String getJobAttributeString(ObjectId id_job, int nr,
			String code) throws KettleException {
		RowMetaAndData r = null;
		...
		if (r == null)
			return null;
		return r.getString(
				KettleDatabaseRepository.FIELD_JOB_ATTRIBUTE_VALUE_STR, null);
	}

	public synchronized boolean getJobAttributeBoolean(ObjectId id_job, int nr,
			String code) throws KettleException {
		RowMetaAndData r = null;
		r = getJobAttributeRow(id_job, nr, code);
		if (r == null)
			return false;
		return r.getBoolean(
				KettleDatabaseRepository.FIELD_JOB_ATTRIBUTE_VALUE_STR, false);
	}

	public synchronized double getJobAttributeNumber(ObjectId id_job, int nr,
			String code) throws KettleException {
		RowMetaAndData r = null;
		r = getJobAttributeRow(id_job, nr, code);
		if (r == null)
			return 0.0;
		return r.getNumber(
				KettleDatabaseRepository.FIELD_JOB_ATTRIBUTE_VALUE_NUM, 0.0);
	}

	public synchronized long getJobAttributeInteger(ObjectId id_job, int nr,
			String code) throws KettleException {
		RowMetaAndData r = null;
		r = getJobAttributeRow(id_job, nr, code);
		if (r == null)
			return 0L;
		return r.getInteger(
				KettleDatabaseRepository.FIELD_JOB_ATTRIBUTE_VALUE_NUM, 0L);
	}

	public synchronized int countNrJobAttributes(ObjectId id_job, String code)
			throws KettleException {
		String sql = "SELECT COUNT(*) FROM "
				+ databaseMeta.getQuotedSchemaTableCombination(null,
						KettleDatabaseRepository.TABLE_R_JOB_ATTRIBUTE)
	...
	public synchronized List<Object[]> getJobAttributes(ObjectId id_job,
			String code, long nr) throws KettleException {
		String sql = "SELECT *"
				+ " FROM "
				+ databaseMeta.getQuotedSchemaTableCombination(null,
						KettleDatabaseRepository.TABLE_R_JOB_ATTRIBUTE)
				...
				+ quote(KettleDatabaseRepository.FIELD_JOB_ATTRIBUTE_CODE)
				+ " = ? AND "
				+ quote(KettleDatabaseRepository.FIELD_JOB_ATTRIBUTE_NR)
				+ " = ?" + " ORDER BY "
				+ quote(KettleDatabaseRepository.FIELD_JOB_ATTRIBUTE_VALUE_NUM);
	...
	// long etc)
	//
	public synchronized ObjectId saveJobEntryAttribute(ObjectId id_job,
	...
	public synchronized ObjectId saveJobEntryAttribute(ObjectId id_job,
			ObjectId id_jobentry, String code, double value)
			throws KettleException {
	...
	public synchronized ObjectId saveJobEntryAttribute(ObjectId id_job,
			ObjectId id_jobentry, long nr, String code, String value)
			throws KettleException {
		return saveJobEntryAttribute(code, nr, id_job, id_jobentry, 0.0, value);
	}

	public synchronized ObjectId saveJobEntryAttribute(ObjectId id_job,
			ObjectId id_jobentry, long nr, String code, double value)
			throws KettleException {
		return saveJobEntryAttribute(code, nr, id_job, id_jobentry, value, null);
	}

	public synchronized ObjectId saveJobEntryAttribute(ObjectId id_job,
			ObjectId id_jobentry, long nr, String code, boolean value)
			throws KettleException {
		return saveJobEntryAttribute(code, nr, id_job, id_jobentry, 0.0,
				value ? "Y" : "N");
	}

	private ObjectId saveJobEntryAttribute(String code, long nr,
			ObjectId id_job, ObjectId id_jobentry, double value_num,
			String value_str) throws KettleException {
		return insertJobEntryAttribute(id_job, id_jobentry, nr, code,
				value_num, value_str);
	}

	public synchronized ObjectId insertJobEntryAttribute(ObjectId id_job,
			ObjectId id_jobentry, long nr, String code, double value_num,
			String value_str) throws KettleException {
		...
		table.addValue(new ValueMeta(
				...
				ValueMetaInterface.TYPE_STRING), code);
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_JOBENTRY_ATTRIBUTE_VALUE_NUM,
		...
		database.prepareInsert(table.getRowMeta(),
	...
	public synchronized LongObjectId getNextJobEntryAttributeID()
			throws KettleException {
		return getNextID(
				databaseMeta.getQuotedSchemaTableCombination(null,
						KettleDatabaseRepository.TABLE_R_JOBENTRY_ATTRIBUTE),
	...
	public synchronized LongObjectId getNextID(String tableName,
	...
	private synchronized LongObjectId getNextTableID(String tablename,
		...
		if (r != null) {
			...
			if (id == null) {
				if (log.isDebug())
					log.logDebug("no max(" + idfield + ") found in table "
							+ tablename);
				retval = new LongObjectId(1);
			} else {
				if (log.isDebug())
					log.logDebug("max(" + idfield + ") found in table "
							+ tablename + " --> " + idfield + " number: " + id);
	...
	private RowMetaAndData getJobEntryAttributeRow(ObjectId id_jobentry,
			int nr, String code) throws KettleException {
		RowMetaAndData par = new RowMetaAndData();
		par.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_JOBENTRY_ATTRIBUTE_ID_JOBENTRY,
	...
	public synchronized long getJobEntryAttributeInteger(ObjectId id_jobentry,
			int nr, String code) throws KettleException {
	...
	public double getJobEntryAttributeNumber(ObjectId id_jobentry, int nr,
			String code) throws KettleException {
		RowMetaAndData r = getJobEntryAttributeRow(id_jobentry, nr, code);
		if (r == null)
			return 0.0;
	...
	public synchronized String getJobEntryAttributeString(ObjectId id_jobentry,
			int nr, String code) throws KettleException {
		RowMetaAndData r = getJobEntryAttributeRow(id_jobentry, nr, code);
		if (r == null)
			return null;
	...
	public double getJobEntryAttributeNumber(ObjectId id_jobentry, String code)
			throws KettleException {
	...
	public synchronized ObjectId getNextTransformationID()
			throws KettleException {
		return getNextID(
	...
	public synchronized ObjectId getNextDatabaseConnectionTypeID()
			throws KettleException {
		return getNextID(
	...
	public synchronized LongObjectId getNextJobEntryCopyID()
			throws KettleException {
		return getNextID(
	...
	public synchronized LongObjectId getNextStepAttributeID()
			throws KettleException {
		return getNextID(
	...
	public synchronized LongObjectId getNextTransAttributeID()
			throws KettleException {
		return getNextID(
	...
	public synchronized LongObjectId getNextJobAttributeID()
			throws KettleException {
		return getNextID(
	...
	public synchronized LongObjectId getNextDatabaseAttributeID()
			throws KettleException {
		return getNextID(
	...
	public synchronized ObjectId getNextPartitionSchemaID()
			throws KettleException {
		return getNextID(
	...
	public synchronized ObjectId getNextTransformationPartitionSchemaID()
			throws KettleException {
		return getNextID(
	...
	public synchronized ObjectId getNextTransformationSlaveID()
			throws KettleException {
		return getNextID(
	...
	public synchronized ObjectId getNextTransformationClusterID()
			throws KettleException {
		return getNextID(
	...
	public synchronized ObjectId insertStepAttribute(
			ObjectId id_transformation, ObjectId id_step, long nr, String code,
		...
		table
				.addValue(
						new ValueMeta(
								KettleDatabaseRepository.FIELD_STEP_ATTRIBUTE_ID_STEP_ATTRIBUTE,
								ValueMetaInterface.TYPE_INTEGER), id);
		table
				.addValue(
						new ValueMeta(
								KettleDatabaseRepository.FIELD_STEP_ATTRIBUTE_ID_TRANSFORMATION,
								ValueMetaInterface.TYPE_INTEGER),
						id_transformation);
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_STEP_ATTRIBUTE_ID_STEP,
		...
		table.addValue(new ValueMeta(
				...
				ValueMetaInterface.TYPE_STRING), code);
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_STEP_ATTRIBUTE_VALUE_NUM,
				ValueMetaInterface.TYPE_NUMBER), new Double(value_num));
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_STEP_ATTRIBUTE_VALUE_STR,
				ValueMetaInterface.TYPE_STRING), value_str);
		...
		/*
		 * If we have prepared the insert, we don't do it again. We assume that
		 * all the step insert statements come one after the other.
		 */

		if (psStepAttributesInsert == null) {
			String sql = database.getInsertStatement(
					KettleDatabaseRepository.TABLE_R_STEP_ATTRIBUTE, table
							.getRowMeta());
	...
	public synchronized ObjectId insertTransAttribute(
			ObjectId id_transformation, long nr, String code, long value_num,
			String value_str) throws KettleException {
		...
		table.addValue(new ValueMeta(
				...
				ValueMetaInterface.TYPE_STRING), code);
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_TRANS_ATTRIBUTE_VALUE_NUM,
		...
		if (psTransAttributesInsert == null) {
			String sql = database.getInsertStatement(
					KettleDatabaseRepository.TABLE_R_TRANS_ATTRIBUTE, table
							.getRowMeta());
	...
	public synchronized ObjectId insertJobAttribute(ObjectId id_job, long nr,
			String code, long value_num, String value_str)
			throws KettleException {
		...
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_JOB_ATTRIBUTE_ID_JOB_ATTRIBUTE,
				ValueMetaInterface.TYPE_INTEGER), id);
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_JOB_ATTRIBUTE_ID_JOB,
				ValueMetaInterface.TYPE_INTEGER), id_job);
		...
		table.addValue(new ValueMeta(
				...
				ValueMetaInterface.TYPE_STRING), code);
		table.addValue(new ValueMeta(
				KettleDatabaseRepository.FIELD_JOB_ATTRIBUTE_VALUE_NUM,
				ValueMetaInterface.TYPE_INTEGER), new Long(value_num));
		table.addValue(new ValueMeta(
		...
		if (psJobAttributesInsert == null) {
			String sql = database.getInsertStatement(
					KettleDatabaseRepository.TABLE_R_JOB_ATTRIBUTE, table
							.getRowMeta());
	...
	public synchronized void updateTableRow(String tablename, String idfield,
			RowMetaAndData values, ObjectId id) throws KettleException {
		...
		values.addValue(
				new ValueMeta(idfield, ValueMetaInterface.TYPE_INTEGER), id);
	 ...
	 * @throws KettleException
	 */
	public synchronized List<RepositoryElementMetaInterface> getRepositoryObjects(
		...
		try {
			...
			RepositoryDirectoryInterface repositoryDirectory = repository.directoryDelegate
					.loadPathToRoot(id_directory);
			...
			ResultSet rs = database.openQuery(sql, directoryIdRow.getRowMeta(),
					directoryIdRow.getData());
			if (rs != null) {
				...
				for (Object[] r : rows) {
					...
					repositoryObjects.add(new RepositoryObject(id, rowMeta
							.getString(r, 0), repositoryDirectory, rowMeta
							.getString(r, 1), rowMeta.getDate(r, 2),
							objectType, rowMeta.getString(r, 3), false));
				}
			}
			...
			throw new KettleException(
					"Unable to get list of repository objects", e);
		}
	}

	public ObjectId[] getIDs(String sql, ObjectId... objectId)
			throws KettleException {
		...
		ResultSet resultSet = database.openQuery(ps, parameterMeta,
				parameterData);
	...
	public String[] getStrings(String sql, ObjectId... objectId)
			throws KettleException {
		// Get the prepared statement
		//
		PreparedStatement ps = sqlMap.get(sql);
	...
	private String[] getQuotedSchemaTablenames(String[] tables) {
		...
		for (int i = 0; i < quoted.length; i++) {
			quoted[i] = database.getDatabaseMeta()
					.getQuotedSchemaTableCombination(null, tables[i]);
		}
		return quoted;
	}
	...
	public synchronized void lockRepository() throws KettleException {
		if (database.getDatabaseMeta().needsToLockAllTables()) {
			database
					.lockTables(getQuotedSchemaTablenames(KettleDatabaseRepository.repositoryTableNames));
		} else {
			database
	...
	public synchronized void unlockRepository() throws KettleException {
		if (database.getDatabaseMeta().needsToLockAllTables()) {
			database
					.unlockTables(KettleDatabaseRepository.repositoryTableNames);
		} else {
			database
					.unlockTables(new String[] { KettleDatabaseRepository.TABLE_R_REPOSITORY_LOG, });
		}
	}
	...
	public synchronized ObjectId getIDWithValue(String tablename,
			String idfield, String lookupfield, String value)
			throws KettleException {
		RowMetaAndData par = new RowMetaAndData();
		par.addValue(new ValueMeta("value", ValueMetaInterface.TYPE_STRING),
				value);
		RowMetaAndData result = getOneRow("SELECT " + idfield + " FROM "
				+ tablename + " WHERE " + lookupfield + " = ?", par
	...
	public synchronized ObjectId getIDWithValue(String tablename,
			String idfield, String lookupfield, String value, String lookupkey,
			ObjectId key) throws KettleException {
		RowMetaAndData par = new RowMetaAndData();
		par.addValue(new ValueMeta("value", ValueMetaInterface.TYPE_STRING),
				value);
		par.addValue(new ValueMeta("key", ValueMetaInterface.TYPE_INTEGER),
				new LongObjectId(key));
		RowMetaAndData result = getOneRow("SELECT " + idfield + " FROM "
				+ tablename + " WHERE " + lookupfield + " = ? AND " + lookupkey
	...
	public synchronized ObjectId getIDWithValue(String tablename,
			String idfield, String lookupkey[], ObjectId key[])
			throws KettleException {
		RowMetaAndData par = new RowMetaAndData();
		...
		for (int i = 0; i < lookupkey.length; i++) {
			...
			par.addValue(new ValueMeta(lookupkey[i],
					ValueMetaInterface.TYPE_INTEGER), new LongObjectId(key[i]));
			sql += lookupkey[i] + " = ? ";
		}
	...
	public synchronized ObjectId getIDWithValue(String tablename,
		...
		for (int i = 0; i < lookupkey.length; i++) {
			par.addValue(new ValueMeta(lookupkey[i],
					ValueMetaInterface.TYPE_STRING), new LongObjectId(key[i]));
			sql += "AND " + lookupkey[i] + " = ? ";
		}
	...
	public RowMetaAndData getOneRow(String tablename, String keyfield,
		...
		parameterMeta.addValueMeta(new ValueMeta("id",
				ValueMetaInterface.TYPE_INTEGER));
		...
		try {
			...
			if (result == null)
				return new RowMetaAndData(database.getReturnRowMeta(),
						RowDataUtil.allocateRowData(database.getReturnRowMeta()
								.size()));
	...
	public RowMetaAndData getOneRow(String sql, RowMetaInterface rowMeta,
			Object[] rowData) throws KettleDatabaseException {
		return database.getOneRow(sql, rowMeta, rowData);
	}

	public synchronized String getStringWithID(String tablename,
			String keyfield, ObjectId id, String fieldname)
			throws KettleException {
	...
	public Collection<RowMetaAndData> getDatabaseAttributes(ObjectId id_database)
		...
		parameterMeta.addValueMeta(new ValueMeta("id",
				ValueMetaInterface.TYPE_INTEGER));
		Object[] parameterData = new Object[] { ((LongObjectId) id_database)
				.longValue(), };
		...
		try {
			...
			List<Object[]> rows = database.getRows(resultSet, 0, null);
			for (Object[] row : rows) {
				RowMetaAndData rowWithMeta = new RowMetaAndData(

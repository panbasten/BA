 ...
 * @since 2011-01-13
 */
public class XMLInputStreamMeta extends BaseStepMeta implements
		StepMetaInterface {
	private final static int DEFAULT_STRING_LEN_FILENAME = 256; // default
																// length for
																// XML path
	private final static int DEFAULT_STRING_LEN_PATH = 1024; // default length
																// for XML path
	public final static String DEFAULT_STRING_LEN = "1024"; // used by
															// defaultStringLen
	...
	private String nrRowsToSkip; // String for variable usage, enables chunk
									// loading defined in an outer loop

	/** The maximum number of lines to read */
	private String rowLimit; // String for variable usage, enables chunk loading
								// defined in an outer loop
	...
	private boolean enableTrim; // trim is also eliminating white spaces, tab,
								// cr, lf at the beginning and end of the string
	...
	public void getFields(RowMetaInterface r, String name,
		...
		int defaultStringLenNameValueElements = Const.toInt(space
				.environmentSubstitute(defaultStringLen), new Integer(
				DEFAULT_STRING_LEN));
		...
		if (includeXmlDataTypeNumericField) {
			ValueMetaInterface vdtn = new ValueMeta(space
					.environmentSubstitute(xmlDataTypeNumericField),
					ValueMeta.TYPE_INTEGER);
		...
		if (includeXmlDataTypeDescriptionField) {
			ValueMetaInterface vdtd = new ValueMeta(space
					.environmentSubstitute(xmlDataTypeDescriptionField),
					ValueMeta.TYPE_STRING);
			vdtd.setLength(25);
			vdtd.setOrigin(name);
			r.addValueMeta(vdtd);
		...
		if (includeXmlLocationLineField) {
			ValueMetaInterface vline = new ValueMeta(space
					.environmentSubstitute(xmlLocationLineField),
					ValueMeta.TYPE_INTEGER);
		...
		if (includeXmlLocationColumnField) {
			ValueMetaInterface vcol = new ValueMeta(space
					.environmentSubstitute(xmlLocationColumnField),
					ValueMeta.TYPE_INTEGER);
		...
		if (includeXmlElementIDField) {
			ValueMetaInterface vdid = new ValueMeta("xml_element_id",
					ValueMeta.TYPE_INTEGER);
		...
		if (includeXmlParentElementIDField) {
			ValueMetaInterface vdparentid = new ValueMeta(
					"xml_parent_element_id", ValueMeta.TYPE_INTEGER);
		...
		if (includeXmlElementLevelField) {
			ValueMetaInterface vdlevel = new ValueMeta("xml_element_level",
					ValueMeta.TYPE_INTEGER);
		...
		if (includeXmlPathField) {
			ValueMetaInterface vdparentxp = new ValueMeta("xml_path",
					ValueMeta.TYPE_STRING);
		...
		if (includeXmlParentPathField) {
			ValueMetaInterface vdparentpxp = new ValueMeta("xml_parent_path",
					ValueMeta.TYPE_STRING);
		...
		if (includeXmlDataNameField) {
			ValueMetaInterface vdname = new ValueMeta("xml_data_name",
					ValueMeta.TYPE_STRING);
		...
		if (includeXmlDataValueField) {
			ValueMetaInterface vdval = new ValueMeta("xml_data_value",
					ValueMeta.TYPE_STRING);
	...
	public void loadXML(Node stepnode, List<DatabaseMeta> databases,
		...
		try {
			...
			nrRowsToSkip = Const.NVL(XMLHandler.getTagValue(stepnode,
					"nrRowsToSkip"), "0");
			rowLimit = Const.NVL(XMLHandler.getTagValue(stepnode, "rowLimit"),
					"0");
			defaultStringLen = Const.NVL(XMLHandler.getTagValue(stepnode,
					"defaultStringLen"), DEFAULT_STRING_LEN);
			encoding = Const.NVL(XMLHandler.getTagValue(stepnode, "encoding"),
					DEFAULT_ENCODING);
			enableNamespaces = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "enableNamespaces"));
			enableTrim = "Y".equalsIgnoreCase(XMLHandler.getTagValue(stepnode,
					"enableTrim"));

			// The fields in the output stream
			// When they are undefined (checked with NVL) the original default
			// value will be taken
			includeFilenameField = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "includeFilenameField"));
			filenameField = Const.NVL(XMLHandler.getTagValue(stepnode,
					"filenameField"), filenameField);

			includeRowNumberField = "Y".equalsIgnoreCase(XMLHandler
					.getTagValue(stepnode, "includeRowNumberField"));
			rowNumberField = Const.NVL(XMLHandler.getTagValue(stepnode,
					"rowNumberField"), rowNumberField);

			includeXmlDataTypeNumericField = "Y".equalsIgnoreCase(XMLHandler
					.getTagValue(stepnode, "includeDataTypeNumericField"));
			xmlDataTypeNumericField = Const.NVL(XMLHandler.getTagValue(
					stepnode, "dataTypeNumericField"), xmlDataTypeNumericField);

			includeXmlDataTypeDescriptionField = "Y"
					.equalsIgnoreCase(XMLHandler.getTagValue(stepnode,
							"includeDataTypeDescriptionField"));
			xmlDataTypeDescriptionField = Const.NVL(XMLHandler.getTagValue(
					stepnode, "dataTypeDescriptionField"),
					xmlDataTypeDescriptionField);

			includeXmlLocationLineField = "Y".equalsIgnoreCase(XMLHandler
					.getTagValue(stepnode, "includeXmlLocationLineField"));
			xmlLocationLineField = Const.NVL(XMLHandler.getTagValue(stepnode,
					"xmlLocationLineField"), xmlLocationLineField);

			includeXmlLocationColumnField = "Y".equalsIgnoreCase(XMLHandler
					.getTagValue(stepnode, "includeXmlLocationColumnField"));
			xmlLocationColumnField = Const.NVL(XMLHandler.getTagValue(stepnode,
					"xmlLocationColumnField"), xmlLocationColumnField);

			includeXmlElementIDField = "Y".equalsIgnoreCase(XMLHandler
					.getTagValue(stepnode, "includeXmlElementIDField"));
			xmlElementIDField = Const.NVL(XMLHandler.getTagValue(stepnode,
					"xmlElementIDField"), xmlElementIDField);

			includeXmlParentElementIDField = "Y".equalsIgnoreCase(XMLHandler
					.getTagValue(stepnode, "includeXmlParentElementIDField"));
			xmlParentElementIDField = Const.NVL(XMLHandler.getTagValue(
					stepnode, "xmlParentElementIDField"),
					xmlParentElementIDField);

			includeXmlElementLevelField = "Y".equalsIgnoreCase(XMLHandler
					.getTagValue(stepnode, "includeXmlElementLevelField"));
			xmlElementLevelField = Const.NVL(XMLHandler.getTagValue(stepnode,
					"xmlElementLevelField"), xmlElementLevelField);

			includeXmlPathField = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "includeXmlPathField"));
			xmlPathField = Const.NVL(XMLHandler.getTagValue(stepnode,
					"xmlPathField"), xmlPathField);

			includeXmlParentPathField = "Y".equalsIgnoreCase(XMLHandler
					.getTagValue(stepnode, "includeXmlParentPathField"));
			xmlParentPathField = Const.NVL(XMLHandler.getTagValue(stepnode,
					"xmlParentPathField"), xmlParentPathField);

			includeXmlDataNameField = "Y".equalsIgnoreCase(XMLHandler
					.getTagValue(stepnode, "includeXmlDataNameField"));
			xmlDataNameField = Const.NVL(XMLHandler.getTagValue(stepnode,
					"xmlDataNameField"), xmlDataNameField);

			includeXmlDataValueField = "Y".equalsIgnoreCase(XMLHandler
					.getTagValue(stepnode, "includeXmlDataValueField"));
			xmlDataValueField = Const.NVL(XMLHandler.getTagValue(stepnode,
					"xmlDataValueField"), xmlDataValueField);
	...
	public String getXML() {
		...
		retval.append("    "
				+ XMLHandler.addTagValue("nrRowsToSkip", nrRowsToSkip));
		retval.append("    " + XMLHandler.addTagValue("rowLimit", rowLimit));
		retval.append("    "
				+ XMLHandler.addTagValue("defaultStringLen", defaultStringLen));
		retval.append("    " + XMLHandler.addTagValue("encoding", encoding));
		retval.append("    "
				+ XMLHandler.addTagValue("enableNamespaces", enableNamespaces));
		retval
				.append("    "
						+ XMLHandler.addTagValue("enableTrim", enableTrim));

		// The fields in the output stream
		retval.append("    "
				+ XMLHandler.addTagValue("includeFilenameField",
						includeFilenameField));
		retval.append("    "
				+ XMLHandler.addTagValue("filenameField", filenameField));

		retval.append("    "
				+ XMLHandler.addTagValue("includeRowNumberField",
						includeRowNumberField));
		retval.append("    "
				+ XMLHandler.addTagValue("rowNumberField", rowNumberField));

		retval.append("    "
				+ XMLHandler.addTagValue("includeDataTypeNumericField",
						includeXmlDataTypeNumericField));
		retval.append("    "
				+ XMLHandler.addTagValue("dataTypeNumericField",
						xmlDataTypeNumericField));

		retval.append("    "
				+ XMLHandler.addTagValue("includeDataTypeDescriptionField",
						includeXmlDataTypeDescriptionField));
		retval.append("    "
				+ XMLHandler.addTagValue("dataTypeDescriptionField",
						xmlDataTypeDescriptionField));

		retval.append("    "
				+ XMLHandler.addTagValue("includeXmlLocationLineField",
						includeXmlLocationLineField));
		retval.append("    "
				+ XMLHandler.addTagValue("xmlLocationLineField",
						xmlLocationLineField));

		retval.append("    "
				+ XMLHandler.addTagValue("includeXmlLocationColumnField",
						includeXmlLocationColumnField));
		retval.append("    "
				+ XMLHandler.addTagValue("xmlLocationColumnField",
						xmlLocationColumnField));

		retval.append("    "
				+ XMLHandler.addTagValue("includeXmlElementIDField",
						includeXmlElementIDField));
		retval.append("    "
				+ XMLHandler
						.addTagValue("xmlElementIDField", xmlElementIDField));

		retval.append("    "
				+ XMLHandler.addTagValue("includeXmlParentElementIDField",
						includeXmlParentElementIDField));
		retval.append("    "
				+ XMLHandler.addTagValue("xmlParentElementIDField",
						xmlParentElementIDField));

		retval.append("    "
				+ XMLHandler.addTagValue("includeXmlElementLevelField",
						includeXmlElementLevelField));
		retval.append("    "
				+ XMLHandler.addTagValue("xmlElementLevelField",
						xmlElementLevelField));

		retval.append("    "
				+ XMLHandler.addTagValue("includeXmlPathField",
						includeXmlPathField));
		retval.append("    "
				+ XMLHandler.addTagValue("xmlPathField", xmlPathField));

		retval.append("    "
				+ XMLHandler.addTagValue("includeXmlParentPathField",
						includeXmlParentPathField));
		retval.append("    "
				+ XMLHandler.addTagValue("xmlParentPathField",
						xmlParentPathField));

		retval.append("    "
				+ XMLHandler.addTagValue("includeXmlDataNameField",
						includeXmlDataNameField));
		retval.append("    "
				+ XMLHandler.addTagValue("xmlDataNameField", xmlDataNameField));

		retval.append("    "
				+ XMLHandler.addTagValue("includeXmlDataValueField",
						includeXmlDataValueField));
		retval.append("    "
				+ XMLHandler
						.addTagValue("xmlDataValueField", xmlDataValueField));
	...
	public void readRep(Repository rep, ObjectId id_step,
		...
		try {
			filename = Const.NVL(rep
					.getStepAttributeString(id_step, "filename"), "");
			addResultFile = rep.getStepAttributeBoolean(id_step,
					"addResultFile");

			nrRowsToSkip = Const.NVL(rep.getStepAttributeString(id_step,
					"nrRowsToSkip"), "0");
			rowLimit = Const.NVL(rep
					.getStepAttributeString(id_step, "rowLimit"), "0");
			defaultStringLen = Const.NVL(rep.getStepAttributeString(id_step,
					"defaultStringLen"), DEFAULT_STRING_LEN);
			encoding = Const.NVL(rep
					.getStepAttributeString(id_step, "encoding"),
					DEFAULT_ENCODING);
			enableNamespaces = rep.getStepAttributeBoolean(id_step,
					"enableNamespaces");
			...
			// When they are undefined (checked with NVL) the original default
			// value will be taken
			includeFilenameField = rep.getStepAttributeBoolean(id_step,
					"includeFilenameField");
			filenameField = Const.NVL(rep.getStepAttributeString(id_step,
					"filenameField"), filenameField);

			includeRowNumberField = rep.getStepAttributeBoolean(id_step,
					"includeRowNumberField");
			rowNumberField = Const.NVL(rep.getStepAttributeString(id_step,
					"rowNumberField"), rowNumberField);

			includeXmlDataTypeNumericField = rep.getStepAttributeBoolean(
					id_step, "includeDataTypeNumericField");
			xmlDataTypeNumericField = Const.NVL(rep.getStepAttributeString(
					id_step, "dataTypeNumericField"), xmlDataTypeNumericField);

			includeXmlDataTypeDescriptionField = rep.getStepAttributeBoolean(
					id_step, "includeDataTypeDescriptionField");
			xmlDataTypeDescriptionField = Const.NVL(rep.getStepAttributeString(
					id_step, "dataTypeDescriptionField"),
					xmlDataTypeDescriptionField);

			includeXmlLocationLineField = rep.getStepAttributeBoolean(id_step,
					"includeXmlLocationLineField");
			xmlLocationLineField = Const.NVL(rep.getStepAttributeString(
					id_step, "xmlLocationLineField"), xmlLocationLineField);

			includeXmlLocationColumnField = rep.getStepAttributeBoolean(
					id_step, "includeXmlLocationColumnField");
			xmlLocationColumnField = Const.NVL(rep.getStepAttributeString(
					id_step, "xmlLocationColumnField"), xmlLocationColumnField);

			includeXmlElementIDField = rep.getStepAttributeBoolean(id_step,
					"includeXmlElementIDField");
			xmlElementIDField = Const.NVL(rep.getStepAttributeString(id_step,
					"xmlElementIDField"), xmlElementIDField);

			includeXmlParentElementIDField = rep.getStepAttributeBoolean(
					id_step, "includeXmlParentElementIDField");
			xmlParentElementIDField = Const.NVL(rep.getStepAttributeString(
					id_step, "xmlParentElementIDField"),
					xmlParentElementIDField);

			includeXmlElementLevelField = rep.getStepAttributeBoolean(id_step,
					"includeXmlElementLevelField");
			xmlElementLevelField = Const.NVL(rep.getStepAttributeString(
					id_step, "xmlElementLevelField"), xmlElementLevelField);

			includeXmlPathField = rep.getStepAttributeBoolean(id_step,
					"includeXmlPathField");
			xmlPathField = Const.NVL(rep.getStepAttributeString(id_step,
					"xmlPathField"), xmlPathField);

			includeXmlParentPathField = rep.getStepAttributeBoolean(id_step,
					"includeXmlParentPathField");
			xmlParentPathField = Const.NVL(rep.getStepAttributeString(id_step,
					"xmlParentPathField"), xmlParentPathField);

			includeXmlDataNameField = rep.getStepAttributeBoolean(id_step,
					"includeXmlDataNameField");
			xmlDataNameField = Const.NVL(rep.getStepAttributeString(id_step,
					"xmlDataNameField"), xmlDataNameField);

			includeXmlDataValueField = rep.getStepAttributeBoolean(id_step,
					"includeXmlDataValueField");
			xmlDataValueField = Const.NVL(rep.getStepAttributeString(id_step,
					"xmlDataValueField"), xmlDataValueField);
	...
	public void saveRep(Repository rep, ObjectId id_transformation,
		...
		try {
			...
			rep.saveStepAttribute(id_transformation, id_step, "nrRowsToSkip",
					nrRowsToSkip);
			rep.saveStepAttribute(id_transformation, id_step, "rowLimit",
					rowLimit);
			rep.saveStepAttribute(id_transformation, id_step,
					"defaultStringLen", defaultStringLen);
			rep.saveStepAttribute(id_transformation, id_step, "encoding",
					encoding);
			rep.saveStepAttribute(id_transformation, id_step,
					"enableNamespaces", enableNamespaces);
			rep.saveStepAttribute(id_transformation, id_step, "enableTrim",
					enableTrim);

			// The fields in the output stream
			rep.saveStepAttribute(id_transformation, id_step,
					"includeFilenameField", includeFilenameField);
			rep.saveStepAttribute(id_transformation, id_step, "filenameField",
					filenameField);

			rep.saveStepAttribute(id_transformation, id_step,
					"includeRowNumberField", includeRowNumberField);
			rep.saveStepAttribute(id_transformation, id_step, "rowNumberField",
					rowNumberField);

			rep.saveStepAttribute(id_transformation, id_step,
					"includeDataTypeNumericField",
					includeXmlDataTypeNumericField);
			rep.saveStepAttribute(id_transformation, id_step,
					"dataTypeNumericField", xmlDataTypeNumericField);

			rep.saveStepAttribute(id_transformation, id_step,
					"includeDataTypeDescriptionField",
					includeXmlDataTypeDescriptionField);
			rep.saveStepAttribute(id_transformation, id_step,
					"dataTypeDescriptionField", xmlDataTypeDescriptionField);

			rep.saveStepAttribute(id_transformation, id_step,
					"includeXmlLocationLineField", includeXmlLocationLineField);
			rep.saveStepAttribute(id_transformation, id_step,
					"xmlLocationLineField", xmlLocationLineField);
			...
			rep.saveStepAttribute(id_transformation, id_step,
					"includeXmlElementIDField", includeXmlElementIDField);
			rep.saveStepAttribute(id_transformation, id_step,
					"xmlElementIDField", xmlElementIDField);
			...
			rep.saveStepAttribute(id_transformation, id_step,
					...
					includeXmlParentElementIDField);
			rep.saveStepAttribute(id_transformation, id_step,
					"xmlParentElementIDField", xmlParentElementIDField);

			rep.saveStepAttribute(id_transformation, id_step,
					"includeXmlElementLevelField", includeXmlElementLevelField);
			rep.saveStepAttribute(id_transformation, id_step,
					"xmlElementLevelField", xmlElementLevelField);

			rep.saveStepAttribute(id_transformation, id_step,
					"includeXmlPathField", includeXmlPathField);
			rep.saveStepAttribute(id_transformation, id_step, "xmlPathField",
					xmlPathField);

			rep.saveStepAttribute(id_transformation, id_step,
					"includeXmlParentPathField", includeXmlParentPathField);
			rep.saveStepAttribute(id_transformation, id_step,
					"xmlParentPathField", xmlParentPathField);

			rep.saveStepAttribute(id_transformation, id_step,
					"includeXmlDataNameField", includeXmlDataNameField);
			rep.saveStepAttribute(id_transformation, id_step,
					"xmlDataNameField", xmlDataNameField);
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (Const.isEmpty(filename)) {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR,
					"Filename is not given", stepinfo);
		} else {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
					"Filename is given", stepinfo);
		}
		remarks.add(cr);
	...
	public StepInterface getStep(StepMeta stepMeta,
			...
			Trans trans) {
		return new XMLInputStream(stepMeta, stepDataInterface, cnr, transMeta,
				trans);
	}
	...
	public void setXmlDataTypeDescriptionField(
			String xmlDataTypeDescriptionField) {
	...
	public void setIncludeXmlLocationLineField(
			boolean includeXmlLocationLineField) {
	...
	public void setIncludeXmlElementLevelField(
			boolean includeXmlElementLevelField) {

...
/**
 * This class is responsible for implementing common functionality regarding
 * step meta, such as logging. All Kettle steps have an extension of this where
 * private fields have been added with public accessors.
 * <p>
 * For example, the "Text File Output" step's TextFileOutputMeta class extends
 * BaseStepMeta by adding fields for the output file name, compression, file
 * format, etc...
 * <p>
 * 
...
public class BaseStepMeta implements Cloneable, StepAttributesInterface {
	 ...
	 * 
	 * @param impact
	 *            The list of impacts @see
	 *            org.pentaho.di.transMeta.DatabaseImpact
	 * @param transMeta
	 ...
	 * needs in order to work correctly. This can mean "create table",
	 * "create index" statements but also "alter table ... add/drop/modify"
	 * statements.
	 * 
	...
	/**
	 * This method is added to exclude certain steps from copy/distribute
	 * checking.
	 * 
	 * @since 4.0.0
	 */
	 ...
	 * expected name is in the org.pentaho.di.ui tree and has a class name that
	 * is the name of the job entry with 'Dialog' added to the end.
	 * 
	 * e.g. if the job entry is
	 * org.pentaho.di.job.entries.zipfile.JobEntryZipFile the dialog would be
	...
	public String getDialogFileName() {
		...
		return "di.ui." + className + "Dialog.h";
	}
	...
	/**
	 * Returns the Input/Output metadata for this step. By default, each step
	 * produces and accepts optional input.
	 */
	...
	/**
	 * @return The list of optional input streams. It allows the user to select
	 *         from a list of possible actions like "New target step"
	 */
	...
	/**
	 * When an optional stream is selected, this method is called to handled the
	 * ETL metadata implications of that.
	 * 
	 * @param stream
	 ...
	 *         metadata in a standardized fasion. This method will return null
	 *         if the interface is not available for this step.
	 */
	...
	protected StepInjectionMetaEntry findParentEntry(
		...
		for (StepInjectionMetaEntry look : entries) {
			if (look.getKey().equals(key))
				return look;
			StepInjectionMetaEntry check = findParentEntry(look.getDetails(),
					key);
			if (check != null)
				return check;
		}
	 ...
	 * @return the step injection meta entry
	 */
	protected StepInjectionMetaEntry createEntry(KettleAttributeInterface attr,
			Class<?> PKG) {
		return new StepInjectionMetaEntry(attr.getKey(), attr.getType(),
				BaseMessages.getString(PKG, attr.getDescription()));
	}

	/**
	 * Describe the metadata attributes that can be injected into this step
	 * metadata object.
	 */
	public List<StepInjectionMetaEntry> getStepInjectionMetadataEntries(
		...
		for (KettleAttributeInterface attr : attributes) {
			if (attr.getParent() == null) {
				...
				StepInjectionMetaEntry parentEntry = findParentEntry(entries,
						attr.getParent().getKey());
				if (parentEntry == null) {
		...
		try {
			...
			if (inputStream != null) {
				...
				for (Node node : nodes) {
					...
					String description = XMLHandler.getTagValue(node,
							"description");
					String tooltip = XMLHandler.getTagValue(node, "tooltip");
					int valueType = ValueMeta.getType(XMLHandler.getTagValue(
							node, "valuetype"));
					String parentId = XMLHandler.getTagValue(node, "parentid");

					KettleAttribute attribute = new KettleAttribute(key,
							xmlCode, repCode, description, tooltip, valueType,
							findParent(attributes, parentId));
					attributes.add(attribute);
				}
			}
		} catch (Exception e) {
			throw new KettleException("Unable to load file "
					+ STEP_ATTRIBUTES_FILE, e);
		}
	}
	 ...
	 * 
	 * @see
	 * org.pentaho.di.trans.step.StepAttributesInterface#findParent(java.util
	 ...
	 * 
	 * @see
	 * org.pentaho.di.trans.step.StepAttributesInterface#findAttribute(java.
	 * lang.String)
	 */
	 ...
	 * 
	 * @see
	 * org.pentaho.di.trans.step.StepAttributesInterface#getXmlCode(java.lang
	 * .String)
	 */
	 ...
	 * 
	 * @see
	 * org.pentaho.di.trans.step.StepAttributesInterface#getRepCode(java.lang
	 * .String)
	 */
	public String getRepCode(String attributeKey) {
		KettleAttributeInterface attr = findAttribute(attributeKey);
		return Const.isEmpty(attr.getRepCode()) ? attr.getXmlCode() : attr
				.getRepCode();
	}
	 ...
	 * 
	 * @see
	 * org.pentaho.di.trans.step.StepAttributesInterface#getDescription(java
	 * .lang.String)
	 */
	 ...
	 * 
	 * @see
	 * org.pentaho.di.trans.step.StepAttributesInterface#getTooltip(java.lang
	 * .String)
	 */
	...
	public TransformationType[] getSupportedTransformationTypes() {
		return new TransformationType[] { TransformationType.Normal,
				TransformationType.SingleThreaded, };
	}

	/**
	 * @return True if the job entry defines one or more references to a
	 *         repository object.
	 */
	...
	/**
	 * @return The objects referenced in the step, like a mapping, a
	 *         transformation, a job, ...
	 */
	 ...
	 * @throws KettleException
	 */
	public Object loadReferencedObject(int index, Repository rep,
	 ...
	 * @return
	 */
	public String getParameter(Map<String, List<String>> parameterHolder,
			String paramName) {
	...
	public List<String> getParameterValues(
			Map<String, List<String>> parameterHolder, String paramName) {
		if (StringUtils.isEmpty(paramName)) {
			return null;
		}
	...
	public String getStringParameter(Map<String, List<String>> parameterHolder,
	...
	public int getIntParameter(Map<String, List<String>> parameterHolder,
	...
	public long getLongParameter(Map<String, List<String>> parameterHolder,

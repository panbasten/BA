...
public class NumberRangeMeta extends BaseStepMeta implements StepMetaInterface {
	...
	public NumberRangeMeta(Node stepnode, List<DatabaseMeta> databases,
	...
	public String getXML() {
		...
		retval.append("    ").append(
				XMLHandler.addTagValue("inputField", inputField));
		retval.append("    ").append(
				XMLHandler.addTagValue("outputField", outputField));
		retval.append("    ").append(
		...
		for (NumberRangeRule rule : rules) {
			...
			retval
					.append("        ").append(XMLHandler.addTagValue("lower_bound", rule.getLowerBound())); //$NON-NLS-1$ //$NON-NLS-2$
			retval
					.append("        ").append(XMLHandler.addTagValue("upper_bound", rule.getUpperBound())); //$NON-NLS-1$ //$NON-NLS-2$
			retval
					.append("        ").append(XMLHandler.addTagValue("value", rule.getValue())); //$NON-NLS-1$ //$NON-NLS-2$
			retval.append("      </rule>").append(Const.CR); //$NON-NLS-1$
		}
		retval.append("    </rules>").append(Const.CR); //$NON-NLS-1$
	...
	public void readRep(Repository rep, ObjectId id_step,
		...
		try {
			...
			emptyRules();
			String fallBackValue = rep.getStepAttributeString(id_step,
					"fallBackValue");
			setFallBackValue(fallBackValue);
			int nrfields = rep.countNrStepAttributes(id_step, "lower_bound"); //$NON-NLS-1$
			for (int i = 0; i < nrfields; i++) {
				String lowerBoundStr = rep.getStepAttributeString(id_step, i,
						"lower_bound"); //$NON-NLS-1$
				String upperBoundStr = rep.getStepAttributeString(id_step, i,
						"upper_bound"); //$NON-NLS-1$
	...
	public void saveRep(Repository rep, ObjectId id_transformation,
		...
		try {
			rep.saveStepAttribute(id_transformation, id_step, "inputField",
					inputField);
			rep.saveStepAttribute(id_transformation, id_step, "outputField",
					outputField);
			rep.saveStepAttribute(id_transformation, id_step, "fallBackValue",
					getFallBackValue());

			int i = 0;
			for (NumberRangeRule rule : rules) {
				rep.saveStepAttribute(id_transformation, id_step, i,
						"lower_bound", String.valueOf(rule.getLowerBound())); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i,
						"upper_bound", String.valueOf(rule.getUpperBound())); //$NON-NLS-1$
				rep.saveStepAttribute(id_transformation, id_step, i,
						"value", String.valueOf(rule.getValue())); //$NON-NLS-1$
				i++;
			}
	...
	public StepInterface getStep(StepMeta stepMeta,
			...
			Trans disp) {
		return new NumberRange(stepMeta, stepDataInterface, cnr, transMeta,
				disp);
	}

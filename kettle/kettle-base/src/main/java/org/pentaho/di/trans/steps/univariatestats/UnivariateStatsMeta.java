...
/**
 * Contains the meta-data for the UnivariateStats step: calculates predefined
 * univariate statistics
 * 
 * @author Mark Hall (mhall{[at]}pentaho.org)
 * @version 1.0
 */
public class UnivariateStatsMeta extends BaseStepMeta implements
		StepMetaInterface {
	 ...
	 * Each UnivariateStatsMetaFunction represents an input field to be
	 * processed along with the user-requested stats to compute for it. The same
	 * input field may occur in more than one UnivariateStatsMetaFunction as
	 * more than one percentile may be required.
	 * 
	...
	public void loadXML(Node stepnode, List<DatabaseMeta> databases,
		...
		int nrStats = XMLHandler.countNodes(stepnode,
				UnivariateStatsMetaFunction.XML_TAG);

		allocate(nrStats);
		for (int i = 0; i < nrStats; i++) {
			Node statnode = XMLHandler.getSubNodeByNr(stepnode,
	...
	public String getXML() {
		...
		if (m_stats != null) {
			for (int i = 0; i < m_stats.length; i++) {
				retval.append("       ").append(m_stats[i].getXML()).append(
						Const.CR);
			}
		}
	...
	public Object clone() {
		...
		if (m_stats != null) {
			...
			for (int i = 0; i < m_stats.length; i++) {
				retval.getInputFieldMetaFunctions()[i] = (UnivariateStatsMetaFunction) m_stats[i]
						.clone();
			}
		} else {
			retval.allocate(0);
	...
	/**
	 * Returns an array of ValueMetaInterface that contains the meta data for
	 * each value computed by the supplied UnivariateStatsMetaFunction
	 * 
	 * @param fn
	 *            the <code>UnivariateStatsMetaFunction</code> to construct meta
	 *            data for
	 * @param origin
	 *            the origin
	...
	private ValueMetaInterface[] getValueMetas(UnivariateStatsMetaFunction fn,
		...
		ValueMetaInterface[] v = new ValueMetaInterface[fn
		...
		if (fn.getCalcPercentile() >= 0) {
			...
			v[index] = new ValueMeta(fn.getSourceFieldName() + "(" + res
					+ "th percentile)", ValueMetaInterface.TYPE_NUMBER);
			v[index].setOrigin(origin);
			index++;
		}
		return v;

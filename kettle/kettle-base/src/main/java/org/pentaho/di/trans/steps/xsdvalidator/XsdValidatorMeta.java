...
public class XsdValidatorMeta extends BaseStepMeta implements StepMetaInterface {
	private static Class<?> PKG = XsdValidatorMeta.class; // for i18n purposes,
															// needed by
															// Translator2!!
															// $NON-NLS-1$
	...
	private void readData(Node stepnode) throws KettleXMLException {
		try {
			...
			xsdDefinedField = XMLHandler.getTagValue(stepnode,
					"xsddefinedfield");
			xsdSource = XMLHandler.getTagValue(stepnode, "xsdsource");

			addValidationMessage = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "addvalidationmsg"));

			validationMessageField = XMLHandler.getTagValue(stepnode,
					"validationmsgfield"); //$NON-NLS-1$
			ifXmlValid = XMLHandler.getTagValue(stepnode, "ifxmlvalid");
			ifXmlInvalid = XMLHandler.getTagValue(stepnode, "ifxmlunvalid");
			outputStringField = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "outputstringfield"));
			xmlSourceFile = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "xmlsourcefile"));

		} catch (Exception e) {
			throw new KettleXMLException(
					BaseMessages
							.getString(PKG,
									"XsdValidatorMeta.Exception.UnableToLoadStepInfoFromXML"), e); //$NON-NLS-1$
		}
	}
	...
	public void getFields(RowMetaInterface inputRowMeta, String name,
		...
		// Add String Field that contain validation message (most the time,
		// errors)
		if (addValidationMessage && !Const.isEmpty(validationMessageField)) {
			ValueMetaInterface v = new ValueMeta(space
					.environmentSubstitute(validationMessageField),
					ValueMeta.TYPE_STRING);
			inputRowMeta.addValueMeta(v);
		}
	...
	public String getXML() {
		...
		retval
				.append("    " + XMLHandler.addTagValue("xdsfilename", xsdFilename)); //$NON-NLS-1$ //$NON-NLS-2$
		...
		retval.append("    "
				+ XMLHandler.addTagValue("addvalidationmsg",
						addValidationMessage));
		retval.append("    "
				+ XMLHandler.addTagValue("validationmsgfield",
						validationMessageField));
		retval.append("    "
				+ XMLHandler.addTagValue("ifxmlunvalid", ifXmlInvalid));
		retval
				.append("    "
						+ XMLHandler.addTagValue("ifxmlvalid", ifXmlValid));

		retval.append("    "
				+ XMLHandler
						.addTagValue("outputstringfield", outputStringField));
		retval.append("    "
				+ XMLHandler.addTagValue("xmlsourcefile", xmlSourceFile));
		retval.append("    "
				+ XMLHandler.addTagValue("xsddefinedfield", xsdDefinedField));
	...
	public void readRep(Repository rep, ObjectId id_step,
		...
		try {
			...
			xmlSourceFile = rep.getStepAttributeBoolean(id_step,
					"xmlsourcefile");
			addValidationMessage = rep.getStepAttributeBoolean(id_step,
					"addvalidationmsg");
			validationMessageField = rep.getStepAttributeString(id_step,
					"validationmsgfield");
			ifXmlValid = rep.getStepAttributeString(id_step, "ifxmlvalid");
			ifXmlInvalid = rep.getStepAttributeString(id_step, "ifxmlunvalid");

			outputStringField = rep.getStepAttributeBoolean(id_step,
					"outputstringfield");
			xsdDefinedField = rep.getStepAttributeString(id_step,
					"xsddefinedfield");
			...
			throw new KettleException(
					BaseMessages
							.getString(PKG,
									"XsdValidatorMeta.Exception.UnexpectedErrorInReadingStepInfo"), e); //$NON-NLS-1$
		}
	}
	...
	public void saveRep(Repository rep, ObjectId id_transformation,
		...
		try {
			rep.saveStepAttribute(id_transformation, id_step,
					"xdsfilename", xsdFilename); //$NON-NLS-1$
			rep.saveStepAttribute(id_transformation, id_step,
					"xmlstream", xmlStream); //$NON-NLS-1$
			rep.saveStepAttribute(id_transformation, id_step,
					"resultfieldname", resultFieldname);
			rep.saveStepAttribute(id_transformation, id_step, "xmlsourcefile",
					xmlSourceFile);
			rep.saveStepAttribute(id_transformation, id_step,
					"addvalidationmsg", addValidationMessage);
			rep.saveStepAttribute(id_transformation, id_step,
					"validationmsgfield", validationMessageField);
			rep.saveStepAttribute(id_transformation, id_step, "ifxmlvalid",
					ifXmlValid);
			rep.saveStepAttribute(id_transformation, id_step, "ifxmlunvalid",
					ifXmlInvalid);
			rep.saveStepAttribute(id_transformation, id_step,
					"outputstringfield", outputStringField);
			rep.saveStepAttribute(id_transformation, id_step,
					"xsddefinedfield", xsdDefinedField);
			rep.saveStepAttribute(id_transformation, id_step, "xsdsource",
					xsdSource);

		} catch (Exception e) {
			throw new KettleException(
					BaseMessages.getString(PKG,
							"XsdValidatorMeta.Exception.UnableToSaveStepInfo") + id_step, e); //$NON-NLS-1$
		}
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (Const.isEmpty(xmlStream)) {
			cr = new CheckResult(
					...
					BaseMessages.getString(PKG,
							"XsdValidatorMeta.CheckResult.XMLStreamFieldEmpty"), stepinfo); //$NON-NLS-1$ //$NON-NLS-2$
			remarks.add(cr);
		} else {
			cr = new CheckResult(
					CheckResult.TYPE_RESULT_OK,
					BaseMessages.getString(PKG,
							"XsdValidatorMeta.CheckResult.XMLStreamFieldOK"), stepinfo); //$NON-NLS-1$ //$NON-NLS-2$
			remarks.add(cr);
		}
		...
		if (Const.isEmpty(resultFieldname)) {
			cr = new CheckResult(
					CheckResult.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG,
							"XsdValidatorMeta.CheckResult.ResultFieldEmpty"), stepinfo); //$NON-NLS-1$ //$NON-NLS-2$
			remarks.add(cr);
		} else {
			cr = new CheckResult(
					CheckResult.TYPE_RESULT_OK,
					BaseMessages.getString(PKG,
							"XsdValidatorMeta.CheckResult.ResultFieldOK"), stepinfo); //$NON-NLS-1$ //$NON-NLS-2$
			remarks.add(cr);
		}
		...
		if (xsdSource.equals(SPECIFY_FILENAME)) {
			if (Const.isEmpty(xsdFilename)) {
				cr = new CheckResult(
						CheckResult.TYPE_RESULT_ERROR,
						BaseMessages.getString(PKG,
								"XsdValidatorMeta.CheckResult.XSDFieldEmpty"), stepinfo); //$NON-NLS-1$ //$NON-NLS-2$
				remarks.add(cr);
			}
		}
		...
		if (prev != null && prev.size() > 0) {
			cr = new CheckResult(
					...
					BaseMessages
							.getString(
									PKG,
									"XsdValidatorMeta.CheckResult.ConnectedStepOK", String.valueOf(prev.size())), stepinfo); //$NON-NLS-1$ //$NON-NLS-2$
			remarks.add(cr);
		} else {
			cr = new CheckResult(
					CheckResult.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG,
							"XsdValidatorMeta.CheckResult.NoInputReceived"), stepinfo); //$NON-NLS-1$
			remarks.add(cr);
		}
		...
		if (input.length > 0) {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_OK,
					BaseMessages.getString(PKG,
							"XsdValidatorMeta.CheckResult.ExpectedInputOk"),
					stepinfo);
			remarks.add(cr);
		} else {
			cr = new CheckResult(CheckResultInterface.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG,
							"XsdValidatorMeta.CheckResult.ExpectedInputError"),
					stepinfo);
			remarks.add(cr);
		}
	}
	...
	public StepInterface getStep(StepMeta stepMeta,
			...
			Trans trans) {
		return new XsdValidator(stepMeta, stepDataInterface, cnr, transMeta,
				trans);
	...
	public String exportResources(VariableSpace space,
		...
		try {
			...
			// From :
			// ${Internal.Transformation.Filename.Directory}/../foo/bar.xsd
			// To : /home/matt/test/files/foo/bar.xsd
			//
			if (!Const.isEmpty(xsdFilename)) {
				FileObject fileObject = KettleVFS.getFileObject(space
						.environmentSubstitute(xsdFilename), space);
				xsdFilename = resourceNamingInterface.nameResource(fileObject,
						space, true);
				return xsdFilename;
			}

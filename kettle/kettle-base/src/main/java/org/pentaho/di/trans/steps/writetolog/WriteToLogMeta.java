...
public class WriteToLogMeta extends BaseStepMeta implements StepMetaInterface {
	private static Class<?> PKG = WriteToLogMeta.class; // for i18n purposes,
														// needed by
														// Translator2!!
														// $NON-NLS-1$
	...
	public String getXML() {
		...
		retval.append("      "
				+ XMLHandler.addTagValue("displayHeader", displayHeader));
	...
	public void readRep(Repository rep, ObjectId id_step,
		...
		try {
			...
			displayHeader = rep.getStepAttributeBoolean(id_step,
					"displayHeader");
	...
	public void saveRep(Repository rep, ObjectId id_transformation,
		...
		try {
			rep.saveStepAttribute(id_transformation, id_step, "loglevel",
					loglevel);
			rep.saveStepAttribute(id_transformation, id_step, "displayHeader",
					displayHeader);

			rep.saveStepAttribute(id_transformation, id_step, "logmessage",
					logmessage);
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transmeta,
		...
		if (prev == null || prev.size() == 0) {
			cr = new CheckResult(
					...
					BaseMessages.getString(PKG,
							"WriteToLogMeta.CheckResult.NotReceivingFields"), stepinfo); //$NON-NLS-1$
			remarks.add(cr);
		} else {
			cr = new CheckResult(
					...
					BaseMessages
							.getString(
									PKG,
									"WriteToLogMeta.CheckResult.StepRecevingData", prev.size() + ""), stepinfo); //$NON-NLS-1$ //$NON-NLS-2$
			remarks.add(cr);
			...
			if (error_found) {
				error_message = BaseMessages
						.getString(PKG,
								"WriteToLogMeta.CheckResult.FieldsFound",
								error_message);
		...
		if (input.length > 0) {
			cr = new CheckResult(
					CheckResult.TYPE_RESULT_OK,
					BaseMessages.getString(PKG,
							"WriteToLogMeta.CheckResult.StepRecevingData2"), stepinfo); //$NON-NLS-1$
			remarks.add(cr);
		} else {
			cr = new CheckResult(
					...
					BaseMessages
							.getString(PKG,
									"WriteToLogMeta.CheckResult.NoInputReceivedFromOtherSteps"), stepinfo); //$NON-NLS-1$
			remarks.add(cr);
		}
	}

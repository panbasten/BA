...
public class SQLFileOutputMeta extends BaseStepMeta implements
		StepMetaInterface {
	private static Class<?> PKG = SQLFileOutputMeta.class; // for i18n purposes,
															// needed by
															// Translator2!!
															// $NON-NLS-1$
	...
	private void readData(Node stepnode,
		...
		try {
			...
			truncateTable = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "truncate"));
			createTable = "Y".equalsIgnoreCase(XMLHandler.getTagValue(stepnode,
					"create"));
			encoding = XMLHandler.getTagValue(stepnode, "encoding");
			dateformat = XMLHandler.getTagValue(stepnode, "dateformat");
			AddToResult = "Y".equalsIgnoreCase(XMLHandler.getTagValue(stepnode,
					"AddToResult"));

			StartNewLine = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "StartNewLine"));

			fileName = XMLHandler.getTagValue(stepnode, "file", "name");
			createparentfolder = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "file", "create_parent_folder"));
			...
			DoNotOpenNewFileInit = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "file", "DoNotOpenNewFileInit"));
	...
	public String getXML() {
		...
		retval
				.append("    "
						+ XMLHandler.addTagValue("dateformat", dateformat));
		retval.append("    "
				+ XMLHandler.addTagValue("addtoresult", AddToResult));

		retval.append("    "
				+ XMLHandler.addTagValue("startnewline", StartNewLine));
	...
	public void readRep(Repository rep, ObjectId id_step,
		...
		try {
			...
			timeInFilename = rep.getStepAttributeBoolean(id_step,
					"file_add_time");
			createparentfolder = rep.getStepAttributeBoolean(id_step,
	...
	public void saveRep(Repository rep, ObjectId id_transformation,
		...
		try {
			...
			rep.saveStepAttribute(id_transformation, id_step, "truncate",
					truncateTable);
			rep.saveStepAttribute(id_transformation, id_step, "create",
					createTable);
			rep.saveStepAttribute(id_transformation, id_step, "encoding",
					encoding);
			rep.saveStepAttribute(id_transformation, id_step, "dateformat",
					dateformat);
			rep.saveStepAttribute(id_transformation, id_step, "addtoresult",
					AddToResult);

			rep.saveStepAttribute(id_transformation, id_step, "file_name",
					fileName);
			rep.saveStepAttribute(id_transformation, id_step, "file_extention",
					extension);
			rep.saveStepAttribute(id_transformation, id_step, "file_append",
					fileAppended);
			rep.saveStepAttribute(id_transformation, id_step, "file_split",
					splitEvery);
			rep.saveStepAttribute(id_transformation, id_step,
					"file_add_stepnr", stepNrInFilename);
			rep.saveStepAttribute(id_transformation, id_step,
					"file_add_partnr", partNrInFilename);
			rep.saveStepAttribute(id_transformation, id_step, "file_add_date",
					dateInFilename);
			rep.saveStepAttribute(id_transformation, id_step, "file_add_time",
					timeInFilename);
			rep.saveStepAttribute(id_transformation, id_step,
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (databaseMeta != null) {
			CheckResult cr = new CheckResult(CheckResult.TYPE_RESULT_OK,
					BaseMessages.getString(PKG,
							"SQLFileOutputMeta.CheckResult.ConnectionExists"),
					stepMeta);
			remarks.add(cr);
			...
			try {
				...
				cr = new CheckResult(CheckResult.TYPE_RESULT_OK, BaseMessages
						.getString(PKG,
								"SQLFileOutputMeta.CheckResult.ConnectionOk"),
						stepMeta);
				remarks.add(cr);
				...
				if (!Const.isEmpty(tablename)) {
					...
					if (db.checkTableExists(schemaTable)) {
						cr = new CheckResult(
								...
								BaseMessages
										.getString(
												PKG,
												"SQLFileOutputMeta.CheckResult.TableAccessible",
						...
						if (r != null) {
							cr = new CheckResult(
									...
									BaseMessages
											.getString(
													PKG,
													"SQLFileOutputMeta.CheckResult.TableOk",
													schemaTable), stepMeta);
							remarks.add(cr);
							...
							if (prev != null && prev.size() > 0) {
								...
								if (error_found) {
									...
									cr = new CheckResult(
											...
											BaseMessages
													.getString(PKG,
															"SQLFileOutputMeta.CheckResult.AllFieldsFoundInOutput"),
											stepMeta);
									remarks.add(cr);
								}
								...
								if (error_found) {
									error_message = BaseMessages
											.getString(
													PKG,
													"SQLFileOutputMeta.CheckResult.FieldsNotFound",
													error_message);
									...
									cr = new CheckResult(
											...
											BaseMessages
													.getString(PKG,
															"SQLFileOutputMeta.CheckResult.AllFieldsFound"),
											stepMeta);
									remarks.add(cr);
								}
							} else {
								cr = new CheckResult(
										...
										BaseMessages
												.getString(PKG,
														"SQLFileOutputMeta.CheckResult.NoFields"),
										stepMeta);
								remarks.add(cr);
							}
						} else {
							cr = new CheckResult(
									...
									BaseMessages
											.getString(PKG,
													"SQLFileOutputMeta.CheckResult.TableNotAccessible"),
									stepMeta);
							remarks.add(cr);
						}
					} else {
						cr = new CheckResult(
								...
								BaseMessages
										.getString(
												PKG,
												"SQLFileOutputMeta.CheckResult.TableError",
												schemaTable), stepMeta);
						remarks.add(cr);
					}
				} else {
					cr = new CheckResult(
							...
							BaseMessages
									.getString(PKG,
											"SQLFileOutputMeta.CheckResult.NoTableName"),
							stepMeta);
					remarks.add(cr);
				}
				...
				cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR,
						BaseMessages.getString(PKG,
								"SQLFileOutputMeta.CheckResult.UndefinedError",
			...
			CheckResult cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG,
							"SQLFileOutputMeta.CheckResult.NoConnection"),
					stepMeta);
			remarks.add(cr);
		}
		...
		if (input.length > 0) {
			CheckResult cr = new CheckResult(CheckResult.TYPE_RESULT_OK,
					BaseMessages.getString(PKG,
							"SQLFileOutputMeta.CheckResult.ExpectedInputOk"),
					stepMeta);
			remarks.add(cr);
		} else {
			CheckResult cr = new CheckResult(
					CheckResult.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG,
							"SQLFileOutputMeta.CheckResult.ExpectedInputError"),
					stepMeta);
			remarks.add(cr);
		}
	}
	...
	public StepInterface getStep(StepMeta stepMeta,
			...
			Trans trans) {
		return new SQLFileOutput(stepMeta, stepDataInterface, cnr, transMeta,
				trans);
	}
	...
	public SQLStatement getSQLStatements(TransMeta transMeta,
		...
		if (databaseMeta != null) {
			if (prev != null && prev.size() > 0) {
				if (!Const.isEmpty(tablename)) {
					...
					try {
						...
						retval.setError(BaseMessages.getString(PKG,
								"SQLFileOutputMeta.Error.ErrorConnecting", dbe
										.getMessage()));
					} finally {
						db.disconnect();
					}
				...
				retval.setError(BaseMessages.getString(PKG,
						"SQLFileOutputMeta.Error.NoInput"));
			}
		} else {
			retval.setError(BaseMessages.getString(PKG,
					"SQLFileOutputMeta.Error.NoConnection"));
		}
	...
	public RowMetaInterface getRequiredFields(VariableSpace space)
		...
		if (databaseMeta != null) {
			...
			try {
				...
				throw new KettleException(BaseMessages.getString(PKG,
						"SQLFileOutputMeta.Exception.ErrorGettingFields"), e);
			} finally {
				db.disconnect();
			}
		} else {
	...
	public String exportResources(VariableSpace space,
		...
		try {
			...
			//
			// From :
			// ${Internal.Transformation.Filename.Directory}/../foo/bar.data
			...
			if (fileObject.exists()) {
				...
				// 
				fileName = resourceNamingInterface.nameResource(fileObject,
						space, true);

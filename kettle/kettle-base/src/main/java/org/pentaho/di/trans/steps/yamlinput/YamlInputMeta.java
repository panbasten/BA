...
public class YamlInputMeta extends BaseStepMeta implements StepMetaInterface {
	private static Class<?> PKG = YamlInputMeta.class; // for i18n purposes,
														// needed by
														// Translator2!!
														// $NON-NLS-1$
	...
	public String getXML() {
		...
		retval.append("    ").append(
				XMLHandler.addTagValue("YamlField", yamlField));
	...
	private void readData(Node stepnode) throws KettleXMLException {
		try {
			...
			throw new KettleXMLException(BaseMessages.getString(PKG,
					"YamlInputMeta.Exception.ErrorLoadingXML", e.toString()));
		}
	}
		...
		try {
			...
			for (int i = 0; i < nrFields; i++) {
				...
				field.setPrecision((int) rep.getStepAttributeInteger(id_step,
						i, "field_precision"));
				field
						.setTrimType(YamlInputField.getTrimTypeByCode(rep
			...
			throw new KettleException(BaseMessages.getString(PKG,
					"YamlInputMeta.Exception.ErrorReadingRepository"), e);
		}
	}
	...
	public void saveRep(Repository rep, ObjectId id_transformation,
		...
		try {
			...
			rep.saveStepAttribute(id_transformation, id_step, "YamlField",
					yamlField);
		} catch (Exception e) {
			throw new KettleException(BaseMessages.getString(PKG,
	...
	public FileInputList getFiles(VariableSpace space) {
		return FileInputList.createFileList(space, fileName, fileMask,
				fileRequired, includeSubFolderBoolean());
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (input.length <= 0) {
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG,
							"YamlInputMeta.CheckResult.NoInputExpected"),
					stepMeta);
			remarks.add(cr);
		} else {
			cr = new CheckResult(CheckResult.TYPE_RESULT_OK, BaseMessages
					.getString(PKG, "YamlInputMeta.CheckResult.NoInput"),
					stepMeta);
			remarks.add(cr);
		}
		...
		if (getInputFields().length <= 0) {
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR, BaseMessages
					.getString(PKG, "YamlInputMeta.CheckResult.NoInputField"),
					stepMeta);
			remarks.add(cr);
		}
		...
		if (isInFields()) {
			if (Const.isEmpty(getYamlField())) {
				...
				cr = new CheckResult(CheckResult.TYPE_RESULT_OK, BaseMessages
						.getString(PKG, "YamlInputMeta.CheckResult.FieldOk"),
						stepMeta);
				remarks.add(cr);
			}
		} else {
			...
			if (fileInputList == null || fileInputList.getFiles().size() == 0) {
				...
				cr = new CheckResult(CheckResult.TYPE_RESULT_OK, BaseMessages
						.getString(PKG, "YamlInputMeta.CheckResult.FilesOk", ""

...
public class JobEntryFTPPUT extends JobEntryBase implements Cloneable,
		JobEntryInterface {
	private static Class<?> PKG = JobEntryFTPPUT.class; // for i18n purposes,
														// needed by
														// Translator2!!
														// $NON-NLS-1$
	...
	public String getXML() {
		...
		retval.append("      ").append(
				XMLHandler.addTagValue("remoteDirectory", remoteDirectory));
		retval.append("      ").append(
		...
		retval.append("      ").append(
				XMLHandler.addTagValue("only_new", onlyPuttingNewFiles));
		retval.append("      ").append(
	...
	public void loadXML(Node entrynode, List<DatabaseMeta> databases,
		...
		try {
			...
			remoteDirectory = XMLHandler.getTagValue(entrynode,
					"remoteDirectory");
			...
			remove = "Y".equalsIgnoreCase(XMLHandler.getTagValue(entrynode,
					"remove"));
			onlyPuttingNewFiles = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					entrynode, "only_new"));
			activeConnection = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					entrynode, "active"));
			controlEncoding = XMLHandler.getTagValue(entrynode,
					"control_encoding");
			...
			proxyPassword = Encr.decryptPasswordOptionallyEncrypted(XMLHandler
					.getTagValue(entrynode, "proxy_password")); //$NON-NLS-1$
			socksProxyHost = XMLHandler.getTagValue(entrynode,
					"socksproxy_host"); //$NON-NLS-1$
			socksProxyPort = XMLHandler.getTagValue(entrynode,
					"socksproxy_port"); //$NON-NLS-1$
			socksProxyUsername = XMLHandler.getTagValue(entrynode,
					"socksproxy_username"); //$NON-NLS-1$
			socksProxyPassword = Encr
					.decryptPasswordOptionallyEncrypted(XMLHandler.getTagValue(
							entrynode, "socksproxy_password")); //$NON-NLS-1$

			if (controlEncoding == null) {
				// if we couldn't retrieve an encoding, assume it's an old
				// instance and
			...
			throw new KettleXMLException(BaseMessages.getString(PKG,
					"JobFTPPUT.Log.UnableToLoadFromXml"), xe);
		}
	}
	...
	public void loadRep(Repository rep, ObjectId id_jobentry,
		...
		try {
			...
			remoteDirectory = rep.getJobEntryAttributeString(id_jobentry,
					"remoteDirectory");
			...
			timeout = (int) rep.getJobEntryAttributeInteger(id_jobentry,
					"timeout");
			remove = rep.getJobEntryAttributeBoolean(id_jobentry, "remove");
			onlyPuttingNewFiles = rep.getJobEntryAttributeBoolean(id_jobentry,
					"only_new");
			activeConnection = rep.getJobEntryAttributeBoolean(id_jobentry,
					"active");
			controlEncoding = rep.getJobEntryAttributeString(id_jobentry,
					"control_encoding");
			if (controlEncoding == null) {
				// if we couldn't retrieve an encoding, assume it's an old
				// instance and
			...
			proxyHost = rep.getJobEntryAttributeString(id_jobentry,
					"proxy_host"); //$NON-NLS-1$
			proxyPort = rep.getJobEntryAttributeString(id_jobentry,
					"proxy_port"); //$NON-NLS-1$
			proxyUsername = rep.getJobEntryAttributeString(id_jobentry,
					"proxy_username"); //$NON-NLS-1$
			proxyPassword = Encr.decryptPasswordOptionallyEncrypted(rep
					.getJobEntryAttributeString(id_jobentry, "proxy_password")); //$NON-NLS-1$
			socksProxyHost = rep.getJobEntryAttributeString(id_jobentry,
					"socksproxy_host"); //$NON-NLS-1$
			socksProxyPort = rep.getJobEntryAttributeString(id_jobentry,
					"socksproxy_port"); //$NON-NLS-1$
			socksProxyUsername = rep.getJobEntryAttributeString(id_jobentry,
					"socksproxy_username"); //$NON-NLS-1$
			socksProxyPassword = Encr.decryptPasswordOptionallyEncrypted(rep
					.getJobEntryAttributeString(id_jobentry,
							"socksproxy_password")); //$NON-NLS-1$

		} catch (KettleException dbe) {
			throw new KettleException(BaseMessages.getString(PKG,
					"JobFTPPUT.UnableToLoadFromRepo",
	...
	public void saveRep(Repository rep, ObjectId id_job) throws KettleException {
		try {
			rep.saveJobEntryAttribute(id_job, getObjectId(), "servername",
					serverName);
			rep.saveJobEntryAttribute(id_job, getObjectId(), "serverport",
					serverPort);
			rep.saveJobEntryAttribute(id_job, getObjectId(), "username",
					userName);
			rep.saveJobEntryAttribute(id_job, getObjectId(), "password",
					Encr.encryptPasswordIfNotUsingVariables(password));
			rep.saveJobEntryAttribute(id_job, getObjectId(), "remoteDirectory",
					remoteDirectory);
			rep.saveJobEntryAttribute(id_job, getObjectId(), "localDirectory",
					localDirectory);
			rep.saveJobEntryAttribute(id_job, getObjectId(), "wildcard",
					wildcard);
			rep.saveJobEntryAttribute(id_job, getObjectId(), "binary",
					binaryMode);
			rep.saveJobEntryAttribute(id_job, getObjectId(), "timeout", timeout);
			rep.saveJobEntryAttribute(id_job, getObjectId(), "remove", remove);
			rep.saveJobEntryAttribute(id_job, getObjectId(), "only_new",
					onlyPuttingNewFiles);
			rep.saveJobEntryAttribute(id_job, getObjectId(), "active",
					activeConnection);
			rep.saveJobEntryAttribute(id_job, getObjectId(),
					"control_encoding", controlEncoding);

			rep.saveJobEntryAttribute(id_job, getObjectId(),
					"proxy_host", proxyHost); //$NON-NLS-1$
			rep.saveJobEntryAttribute(id_job, getObjectId(),
					"proxy_port", proxyPort); //$NON-NLS-1$
			rep.saveJobEntryAttribute(id_job, getObjectId(),
					"proxy_username", proxyUsername); //$NON-NLS-1$
			rep.saveJobEntryAttribute(
					id_job,
					getObjectId(),
					"proxy_password", Encr.encryptPasswordIfNotUsingVariables(proxyPassword)); //$NON-NLS-1$
			rep.saveJobEntryAttribute(id_job, getObjectId(),
					"socksproxy_host", socksProxyHost); //$NON-NLS-1$
			rep.saveJobEntryAttribute(id_job, getObjectId(),
					"socksproxy_port", socksProxyPort); //$NON-NLS-1$
			rep.saveJobEntryAttribute(id_job, getObjectId(),
					"socksproxy_username", socksProxyUsername); //$NON-NLS-1$
			rep.saveJobEntryAttribute(
					id_job,
					getObjectId(),
			...
			throw new KettleException(
					BaseMessages.getString(
							PKG,
							"JobFTPPUT.UnableToSaveToRepo", String.valueOf(id_job)), dbe); //$NON-NLS-1$
		}
	}
	 ...
	 * translated in dir e.g. It does impact the contents of the files being
	 * ftp'ed.
	 * 
	 * @param encoding
	...
	public Result execute(Result previousResult, int nr) {
		...
		try {
			...
			if (!Const.isEmpty(proxyHost)) {
				...
				if (log.isDetailed())
					logDetailed(BaseMessages.getString(PKG,
							"JobEntryFTPPUT.OpenedProxyConnectionOn",
							realProxy_host));
				...
				if (log.isDetailed())
					logDetailed(BaseMessages.getString(PKG,
							"JobEntryFTPPUT.OpenConnection", realServerName));
			}
			...
			if (activeConnection) {
				...
				if (log.isDetailed())
					logDetailed(BaseMessages.getString(PKG,
							"JobFTPPUT.Log.SetActiveConnection"));
			} else {
				ftpclient.setConnectMode(FTPConnectMode.PASV);
				if (log.isDetailed())
					logDetailed(BaseMessages.getString(PKG,
							"JobFTPPUT.Log.SetPassiveConnection"));
			}
			...
			if (!Const.isEmpty(socksProxyHost)) {
				...
				if (!Const.isEmpty(socksProxyUsername)
						...
						|| Const.isEmpty(socksProxyUsername)
						&& !Const.isEmpty(socksProxyPassword)) {
			...
			if (binaryMode) {
				...
				if (log.isDetailed())
					logDetailed(BaseMessages.getString(PKG,
							"JobFTPPUT.Log.BinaryMode"));
			}

			// Remove password from logging, you don't know where it ends up.
			if (log.isDetailed())
				logDetailed(BaseMessages.getString(PKG, "JobFTPPUT.Log.Logged",
						realUsername));

			// Fix for PDI-2534 - add auxilliary FTP File List parsers to the
			// ftpclient object.
			...
			if (!Const.isEmpty(realRemoteDirectory)) {
				...
				if (log.isDetailed())
					logDetailed(BaseMessages.getString(PKG,
							"JobFTPPUT.Log.ChangedDirectory",
							realRemoteDirectory));
			}
			...
			// Joerg: ..that's for Java5
			// String[] filelist = myFileList.toArray(new
			// String[myFileList.size()]);
			...
			if (log.isDetailed())
				logDetailed(BaseMessages.getString(PKG,
						"JobFTPPUT.Log.FoundFileLocalDirectory", ""
								+ filelist.length, realLocalDirectory));
					...
					if (log.isDebug()) {
						if (fileExist)
							logDebug(BaseMessages.getString(PKG,
									"JobFTPPUT.Log.FileExists", filelist[i]));
						else
							logDebug(BaseMessages.getString(PKG,
									"JobFTPPUT.Log.FileDoesNotExists",
									filelist[i]));
					}
						...
						if (log.isDebug())
							logDebug(BaseMessages
									.getString(
											...
											filelist[i], realRemoteDirectory));

						String localFilename = realLocalDirectory
			...
			if (log.isDetailed())
				logDebug(BaseMessages.getString(PKG, "JobFTPPUT.Log.WeHavePut",
						"" + filesput));
		} catch (Exception e) {
			result.setNrErrors(1);
			logError(BaseMessages.getString(PKG,
					"JobFTPPUT.Log.ErrorPuttingFiles", e.getMessage()));
			...
			if (ftpclient != null && ftpclient.connected()) {
				try {
					...
					logError(BaseMessages.getString(PKG,
							"JobFTPPUT.Log.ErrorQuitingFTP", e.getMessage()));
				}
			}
	...
	/**
	 * Hook in known parsers, and then those that have been specified in the
	 * variable ftp.file.parser.class.names
	 * 
	 * @param ftpClient
	 * @throws FTPException
	 * @throws IOException
	 */
	protected void hookInOtherParsers(FTPClient ftpClient) throws FTPException,
			IOException {
		if (log.isDebug()) {
			logDebug(BaseMessages.getString(PKG,
					"JobEntryFTP.DEBUG.Hooking.Parsers"));
		}
		String system = ftpClient.system();
		MVSFileParser parser = new MVSFileParser();
		if (log.isDebug()) {
			logDebug(BaseMessages.getString(PKG,
					"JobEntryFTP.DEBUG.Created.MVS.Parser"));
		}
		FTPFileFactory factory = new FTPFileFactory(system);
		if (log.isDebug()) {
			logDebug(BaseMessages.getString(PKG,
					"JobEntryFTP.DEBUG.Created.Factory"));
		}
		factory.addParser(parser);
		ftpClient.setFTPFileFactory(factory);
		if (log.isDebug()) {
			logDebug(BaseMessages.getString(PKG,
					"JobEntryFTP.DEBUG.Get.Variable.Space"));
		}
		VariableSpace vs = this.getVariables();
		if (vs != null) {
			if (log.isDebug()) {
				logDebug(BaseMessages.getString(PKG,
						"JobEntryFTP.DEBUG.Getting.Other.Parsers"));
			}
			String otherParserNames = vs
					.getVariable("ftp.file.parser.class.names");
			if (otherParserNames != null) {
				if (log.isDebug()) {
					logDebug(BaseMessages.getString(PKG,
							"JobEntryFTP.DEBUG.Creating.Parsers"));
				}
					...
					if (cName.length() > 0) {
						try {
							...
							if (parserInstance instanceof FTPFileParser) {
								if (log.isDetailed()) {
									logDetailed(BaseMessages
											.getString(
													PKG,
													"JobEntryFTP.DEBUG.Created.Other.Parser",
													cName));
								}
							...
							if (log.isDebug()) {
								...
								logError(BaseMessages.getString(PKG,
										"JobEntryFTP.ERROR.Creating.Parser",
										cName));
							}
						}
					}

<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/">
		<xsl:apply-templates mode="filter-empty-elements"
			select="." />
	</xsl:template>

	<xsl:template name="f-e-e" match="*" mode="filter-empty-elements">
		<!--
			Filter for nodes with attributes, text or descendants who have
			attributes or text or attributes
		-->
		<xsl:if
			test="(descendant::*/text() or descendant::*/@*) or normalize-space(text()) or @*">
			<!--
			...
			<xsl:element name="{$elName}">
				...
				<xsl:for-each select="@*">
					<!--
						Extract the current attribute's name to create a copy in the
						output
					-->

...
public class Database implements VariableSpace, LoggingObjectInterface {
	private static Class<?> PKG = Database.class; // for i18n purposes, needed
	// by Translator2!!
	// $NON-NLS-1$
	...
	/**
	 * Number of times a connection was opened using this object. Only used in
	 * the context of a database connection map
	 */
	private int opened;
	...
	public Database(DatabaseMeta databaseMeta) {
		...
		// We also don't know what log level to attach to it, so we have to
	...
	public synchronized void connect(String group, String partitionId)
			throws KettleDatabaseException {
		try {

			log.snap(Metrics.METRIC_DATABASE_CONNECT_START, databaseMeta
					.getName());

			// Before anything else, let's see if we already have a connection
			// defined for this group/partition!
			// The group is called after the thread-name of the transformation
			// or job that is running
			// The name of that thread name is expected to be unique (it is in
			// Kettle)
			// So the deal is that if there is another thread using that, we go
			// for it.
			// 
			if (!Const.isEmpty(group)) {
				...
				{
					// Do a normal connect and then store this database object
					// for later re-use.
				...
				// If we have a connection group or transaction ID, disable auto
				// commit!
				//
				setAutoCommit(false);
			} else {
	...
	public void normalConnect(String partitionId)
		...
		try {
			...
			// own.
				...
				try {
					this.connection = ConnectionPoolUtil.getConnection(log,
							databaseMeta, partitionId);
				} catch (Exception e) {
					throw new KettleDatabaseException(
				...
				// only execute if the SQL is not empty, null and is not just a
				// bunch of spaces, tabs, CR etc.
				if (!Const.isEmpty(sql) && !Const.onlySpaces(sql)) {
					execStatements(sql);
					if (log.isDetailed())
						log.logDetailed("Executed connect time SQL statements:"
								+ Const.CR + sql);
				}
			}
		} catch (Exception e) {
	 ...
	 * method uses the DataSourceProviderFactory to get the provider of
	 * DataSource objects.
	 * 
	 * @param dataSourceName
	 * @throws KettleDatabaseException
	 */
	private void initWithNamedDataSource(String dataSourceName)
			throws KettleDatabaseException {
		connection = null;
		...
		if (dataSource != null) {
			try {
				...
				throw new KettleDatabaseException(
						"Invalid JNDI connection " + dataSourceName + " : " + e.getMessage()); //$NON-NLS-1$
			}
			if (connection == null) {
				throw new KettleDatabaseException(
						"Invalid JNDI connection " + dataSourceName); //$NON-NLS-1$
			}
		} else {
			throw new KettleDatabaseException(
					"Invalid JNDI connection " + dataSourceName); //$NON-NLS-1$
	 ...
	 *         wrong.
	 */
	private void connectUsingClass(String classname, String partitionId)
			throws KettleDatabaseException {
		// Install and load the jdbc Driver

		// first see if this is a JNDI connection
		if (databaseMeta.getAccessType() == DatabaseMeta.TYPE_ACCESS_JNDI) {
			initWithNamedDataSource(environmentSubstitute(databaseMeta
					.getDatabaseName()));
			return;
		}
		...
		try {
			...
			throw new KettleDatabaseException("Exception while loading class",
					e);
		...
		try {
			...
			if (databaseMeta.isPartitioned() && !Const.isEmpty(partitionId)) {
				...
				PartitionDatabaseMeta partition = databaseMeta
						.getPartitionMeta(partitionId);
				if (partition != null) {
					clusterUsername = partition.getUsername();
					clusterPassword = Encr
							.decryptPasswordOptionallyEncrypted(partition
									.getPassword());
				}
			}
			...
			if (!Const.isEmpty(clusterUsername)) {
				...
				password = Encr
						.decryptPasswordOptionallyEncrypted(environmentSubstitute(databaseMeta
								.getPassword()));
			}
			...
			if (databaseMeta.supportsOptionsInURL()) {
				if (!Const.isEmpty(username) || !Const.isEmpty(password)) {
					if (databaseMeta.getDatabaseInterface() instanceof MSSQLServerNativeDatabaseMeta) {
						...
						//
						String instance = environmentSubstitute(databaseMeta
						...
						// also allow for empty username with given password, in
						// this case username must be given with one space
						connection = DriverManager.getConnection(url, Const
								.NVL(username, " "), Const.NVL(password, ""));
					}
				} else {
					// Perhaps the username is in the URL or no username is
					// required...
					connection = DriverManager.getConnection(url);
				}
			} else {
			...
			throw new KettleDatabaseException(
					"Error connecting to database: (using class " + classname
							+ ")", e);
		} catch (Throwable e) {
			throw new KettleDatabaseException(
					"Error connecting to database: (using class " + classname
	...
	public synchronized void disconnect() {
		try {
			...
			// See if there are other steps using this connection in a
			// connection group.
			...
			log.logError("Error disconnecting from database:" + Const.CR
					+ ex.getMessage());
	...
	public synchronized void closeConnectionOnly()
		...
		try {
			...
			throw new KettleDatabaseException(
					"Error disconnecting from database '" + toString() + "'", e);
		}
	}
	...
	public void setCommit(int commsize) {
		...
		try {
			...
			if (log.isDebug())
				log.logDebug("Can't turn auto commit " + onOff + Const.CR
	...
	public void commit(boolean force) throws KettleDatabaseException {
		try {
			...
			// When the last database copy (opened counter) is about to be
			// closed, we do a commit
			// There is one catch, we need to catch the rollback
			// The transformation will stop everything and then we'll do the
			// rollback.
			...
			if (getDatabaseMetaData().supportsTransactions()) {
				...
				if (log.isDetailed())
					log
							.logDetailed("No commit possible on database connection ["
									+ toString() + "]");
			}
		} catch (Exception e) {
			if (databaseMeta.supportsEmptyTransactions())
				throw new KettleDatabaseException("Error comitting connection",
						e);
		}
	}
	...
	public void rollback(boolean force) throws KettleDatabaseException {
		try {
			...
			if (getDatabaseMetaData().supportsTransactions()) {
				if (connection != null) {
					if (log.isDebug())
						log.logDebug("Rollback on database connection ["
								+ toString() + "]");
					connection.rollback();
				}
			} else {
				if (log.isDetailed())
					log
							.logDetailed("No rollback possible on database connection ["
									+ toString() + "]");
			}

		} catch (SQLException e) {
			throw new KettleDatabaseException(
					"Error performing rollback on connection", e);
		}
	}
	 ...
	 *             if something went wrong.
	 */
	public void prepareInsert(RowMetaInterface rowMeta, String tableName)
			throws KettleDatabaseException {
	 ...
	 *             if something went wrong.
	 */
	public void prepareInsert(RowMetaInterface rowMeta, String schemaName,
			String tableName) throws KettleDatabaseException {
		if (rowMeta.size() == 0) {
	...
	public PreparedStatement prepareSQL(String sql, boolean returnKeys)
		...
		try {
			...
			throw new KettleDatabaseException("Couldn't prepare statement:"
					+ Const.CR + sql, ex);
		}
	}
...

	{
		if (prepStatementInsert != null) {
			try {
				...
				throw new KettleDatabaseException(
						"Error closing insert prepared statement.", e);
			}
		}
	}
	...
	public void closeUpdate() throws KettleDatabaseException {
		if (prepStatementUpdate != null) {
			try {
				...
				throw new KettleDatabaseException(
						"Error closing update prepared statement.", e);
			}
		}
	}
	...
	public void setValuesUpdate(RowMetaInterface rowMeta, Object[] data)
			throws KettleDatabaseException {
		setValues(rowMeta, data, prepStatementUpdate);
	}

	public void setValuesLookup(RowMetaInterface rowMeta, Object[] data)
			throws KettleDatabaseException {
		setValues(rowMeta, data, prepStatementLookup);
	}

	public void setProcValues(RowMetaInterface rowMeta, Object[] data,
			int argnrs[], String argdir[], boolean result)
			throws KettleDatabaseException {
		int pos;
	...
	public void setValue(PreparedStatement ps, ValueMetaInterface v,
			Object object, int pos) throws KettleDatabaseException {
		String debug = "";
		...
		try {
			...
			case ValueMetaInterface.TYPE_BOOLEAN:
				...
				if (databaseMeta.supportsBooleanDataType()) {
					...
					if (!v.isNull(object)) {
						ps
								.setString(pos, v.getBoolean(object)
										.booleanValue() ? "Y" : "N");
					} else {
	...
	public void setValues(RowMetaInterface rowMeta, Object[] data,
			PreparedStatement ps) throws KettleDatabaseException {
		...
		for (int i = 0; i < rowMeta.size(); i++) {
			...
			try {
				...
				throw new KettleDatabaseException("offending row : " + rowMeta,
						e);
			}
		}
	}
	 ...
	 */
	public void setValues(RowMetaInterface rowMeta, Object[] data,
			PreparedStatement ps, int ignoreThisValueIndex)
			throws KettleDatabaseException {
		// now set the values in the row!
		int index = 0;
		for (int i = 0; i < rowMeta.size(); i++) {
			if (i != ignoreThisValueIndex) {
				...
				try {
					...
					throw new KettleDatabaseException("offending row : "
							+ rowMeta, e);
				}
			}
		}
	...
	public Long getNextSequenceValue(String schemaName, String sequenceName,
			String keyfield) throws KettleDatabaseException {
		Long retval = null;

		String schemaSequence = databaseMeta.getQuotedSchemaTableCombination(
				schemaName, sequenceName);

		try {
			if (pstmt_seq == null) {
				pstmt_seq = connection
						.prepareStatement(databaseMeta
								.getSeqNextvalSQL(databaseMeta
										.stripCR(schemaSequence)));
			}
			ResultSet rs = null;
			try {
			...
			throw new KettleDatabaseException(
					"Unable to get next value for sequence : " + schemaSequence,
					ex);
		}
	...
	public void insertRow(String tableName, RowMetaInterface fields,
			Object[] data) throws KettleDatabaseException {
		insertRow(null, tableName, fields, data);
	}

	public void insertRow(String schemaName, String tableName,
	 ...
	 */
	public boolean insertRow(PreparedStatement ps, boolean batch,
			boolean handleCommit) throws KettleDatabaseException {
		String debug = "insertRow start";
		boolean rowsAreSafe = false;

		try {
			// Unique connections and Batch inserts don't mix when you want to
			// roll back on certain databases.
			// That's why we disable the batch insert in that case.
			//
			boolean useBatchInsert = batch
			...
			if (!isAutoCommit()) {
				if (useBatchInsert) {
					...
					ps.addBatch(); // Add the batch, but don't forget to run the
					// batch
				} else {
			...
			if (handleCommit) { // some steps handle the commit themselves (see
				// e.g. TableOutput step)
			...
			KettleDatabaseBatchException kdbe = new KettleDatabaseBatchException(
					"Error updating batch", ex);
			...
			// log.logError(Const.getStackTracker(ex));
			throw new KettleDatabaseException("Error inserting/updating row",
					ex);
		} catch (Exception e) {
	...
	public void clearBatch(PreparedStatement preparedStatement)
		...
		try {
			...
			throw new KettleDatabaseException(
					"Unable to clear batch for prepared statement", e);
		}
	}
	 ...
	 */
	public void emptyAndCommit(PreparedStatement ps, boolean batch,
			int batchCounter) throws KettleDatabaseException {
		...
		try {
			if (ps != null) {
				if (!isAutoCommit()) {
					...
					if (batch && getDatabaseMetaData().supportsBatchUpdates()
							&& batchCounter > 0) {
						// The problem with the batch counters is that you can't
						// just execute the current batch.
						// Certain databases have a problem if you execute the
						// batch and if there are no statements in it.
						// You can't just catch the exception either because you
						// would have to roll back on certain databases before
						// you can then continue to do anything.
						// That leaves the task of keeping track of the number
						// of rows up to our responsibility.
						//
						ps.executeBatch();
						commit();
						ps.clearBatch();
			...
			// This construction is specifically done for some JDBC drivers,
			// these drivers
			// always return the same exception on getNextException() (and thus
			...
			throw new KettleDatabaseException(
					"Unable to empty ps and commit connection.", ex);
		}
	}
	 ...
	 *            The prepared statement to empty and close.
	 * @param batch
	 *            true if you are using batch processing (typically true for
	 *            this method)
	 * @param psBatchCounter
	 *            The number of rows on the batch queue
	 * @throws KettleDatabaseException
	...
	public Result execStatement(String rawsql, Object[] data)
		...
		return execStatement(rawsql, null, null);
	}

	public Result execStatement(String rawsql, RowMetaInterface params,
			Object[] data) throws KettleDatabaseException {
		...
		try {
			...
			int count;
			if (params != null) {
				PreparedStatement prep_stmt = connection
			...
			throw new KettleDatabaseException("Couldn't execute SQL: " + sql
					+ Const.CR, ex);
		} catch (Exception e) {
			throw new KettleDatabaseException(
					"Unexpected error executing SQL: " + Const.CR, e);
		}
	 ...
	 *         deleted or read.
	 */
	public Result execStatements(String script, RowMetaInterface params,
			Object[] data) throws KettleDatabaseException {
		Result result = new Result();
		...
		for (int i = 0; i < statements.length; i++) {
			...
			if (!Const.onlySpaces(stat)) {
				...
				if (sql.toUpperCase().startsWith("SELECT")) {
					// A Query
					if (log.isDetailed())
						log.logDetailed("launch SELECT statement: " + Const.CR
								+ sql);
					...
					try {
						...
						if (rs != null) {
							...
							while (row != null) {
								result
										.setNrLinesRead(result.getNrLinesRead() + 1);
								if (log.isDetailed())
							...
							if (log.isDebug())
								log.logDebug("Error executing query: "
										+ Const.CR + sql);
						}
					} catch (KettleValueException e) {
						throw new KettleDatabaseException(e); // just pass the
						// error
						// upwards.
					} finally {
						try {
							...
							if (log.isDebug())
								log.logDebug("Error closing query: " + Const.CR
										+ sql);
						}
					}
				...
				{
					if (log.isDetailed())
						log.logDetailed("launch DDL statement: " + Const.CR
								+ sql);
		...
		if (log.isDetailed())
			log.logDetailed(nrstats + " statement" + (nrstats == 1 ? "" : "s")
					+ " executed");
	 ...
	 * 
	 * @param sql
	 *            The SQL to launch with question marks (?) as placeholders for
	 *            the parameters
	 * @param params
	 ...
	 */
	public ResultSet openQuery(String sql, RowMetaInterface params,
			Object[] data) throws KettleDatabaseException {
		return openQuery(sql, params, data, ResultSet.FETCH_FORWARD);
	}

	public ResultSet openQuery(String sql, RowMetaInterface params,
			Object[] data, int fetch_mode) throws KettleDatabaseException {
		return openQuery(sql, params, data, fetch_mode, false);
	}

	public ResultSet openQuery(String sql, RowMetaInterface params,
			Object[] data, int fetch_mode, boolean lazyConversion)
			throws KettleDatabaseException {
		ResultSet res;

		// Create a Statement
		try {
			log.snap(Metrics.METRIC_DATABASE_OPEN_QUERY_START, databaseMeta
					.getName());
			if (params != null) {
				log.snap(Metrics.METRIC_DATABASE_PREPARE_SQL_START,
						databaseMeta.getName());
				pstmt = connection
						.prepareStatement(databaseMeta.stripCR(sql),
								ResultSet.TYPE_FORWARD_ONLY,
								ResultSet.CONCUR_READ_ONLY);
				log.snap(Metrics.METRIC_DATABASE_PREPARE_SQL_STOP, databaseMeta
						.getName());

				log.snap(Metrics.METRIC_DATABASE_SQL_VALUES_START, databaseMeta
						.getName());
				setValues(params, data); // set the dates etc!
				log.snap(Metrics.METRIC_DATABASE_SQL_VALUES_STOP, databaseMeta
						.getName());
				...
				log.snap(Metrics.METRIC_DATABASE_EXECUTE_SQL_START,
						databaseMeta.getName());
				res = pstmt.executeQuery();
				log.snap(Metrics.METRIC_DATABASE_EXECUTE_SQL_STOP, databaseMeta
						.getName());
			} else {
				log.snap(Metrics.METRIC_DATABASE_CREATE_SQL_START, databaseMeta
						.getName());
				sel_stmt = connection.createStatement();
				log.snap(Metrics.METRIC_DATABASE_CREATE_SQL_STOP, databaseMeta
						.getName());
			...
			// MySQL Hack only. It seems too much for the cursor type of
			// operation on MySQL, to have another cursor opened
			// to get the length of a String field. So, on MySQL, we ingore the
			// length of Strings in result rows.
			//
			rowMeta = getRowInfo(res.getMetaData(), databaseMeta
					.isMySQLVariant(), lazyConversion);
		} catch (SQLException ex) {
			throw new KettleDatabaseException(
					"An error occurred executing SQL: " + Const.CR + sql, ex);
		} catch (Exception e) {
			throw new KettleDatabaseException(
					"An error occurred executing SQL:" + Const.CR + sql, e);
		} finally {
			log.snap(Metrics.METRIC_DATABASE_OPEN_QUERY_STOP, databaseMeta
					.getName());
		}
	...
	private boolean canWeSetFetchSize(Statement statement) throws SQLException {
		return databaseMeta.isFetchSizeSupported()
				&& (statement.getMaxRows() > 0
						|| databaseMeta.getDatabaseInterface() instanceof PostgreSQLDatabaseMeta || (databaseMeta
	...
	public ResultSet openQuery(PreparedStatement ps, RowMetaInterface params,
		...
		try {
			...
			if (canWeSetFetchSize(ps)) {
				int fs = Const.FETCH_SIZE <= ps.getMaxRows() ? ps.getMaxRows()
						: Const.FETCH_SIZE;
				if (databaseMeta.isMySQLVariant()
						&& databaseMeta.isStreamingResults()
						&& getDatabaseMetaData().getDriverMajorVersion() == 3) {
			...
			log.snap(Metrics.METRIC_DATABASE_EXECUTE_SQL_START, databaseMeta
					.getName());
			res = ps.executeQuery();
			log.snap(Metrics.METRIC_DATABASE_EXECUTE_SQL_STOP, databaseMeta
					.getName());
			...
			// MySQL Hack only. It seems too much for the cursor type of
			// operation on MySQL, to have another cursor opened
			// to get the length of a String field. So, on MySQL, we ignore the
			// length of Strings in result rows.
			// 
			log.snap(Metrics.METRIC_DATABASE_GET_ROW_META_START, databaseMeta
					.getName());
			rowMeta = getRowInfo(res.getMetaData(), databaseMeta
					.isMySQLVariant(), false);
			log.snap(Metrics.METRIC_DATABASE_GET_ROW_META_STOP, databaseMeta
					.getName());
		} catch (SQLException ex) {
	...
	public RowMetaInterface getQueryFields(String sql, boolean param)
			throws KettleDatabaseException {
	...
	public boolean checkTableExists(String tablename)
		...
		try {
			 ...
			 * alltables.close();
			 * 
			 * return found; } else { throw newKettleDatabaseException(
			 * "Unable to read table-names from the database meta-data."); } }
			 * else { throw newKettleDatabaseException(
			 * "Unable to get database meta-data from the database."); }
			 */
		} catch (Exception e) {
			throw new KettleDatabaseException("Unable to check if table ["
					+ tablename + "] exists on connection ["
	...
	public boolean checkColumnExists(String columnname, String tablename)
		...
		try {
			if (log.isDebug())
				log.logDebug("Checking if column [" + columnname
						+ "] exists in table [" + tablename + "] !");
			...
			throw new KettleDatabaseException("Unable to check if column ["
					+ columnname + "] exists in table [" + tablename
	...
	public boolean checkIndexExists(String schemaName, String tableName,
			String idx_fields[]) throws KettleDatabaseException {
		...
		if (log.isDebug())
			log.logDebug("CheckIndexExists() tablename = " + tablename
					+ " type = " + databaseMeta.getPluginId());

		return databaseMeta.getDatabaseInterface().checkIndexExists(this,
				schemaName, tableName, idx_fields);
	}
	...
	public String getCreateIndexStatement(String tablename, String indexname,
			...
			boolean semi_colon) {
		return getCreateIndexStatement(null, tablename, indexname, idx_fields,
				tk, unique, bitmap, semi_colon);
	}
	...
	public String getCreateIndexStatement(String schemaname, String tablename,
		...
		String cr_index = "";
		DatabaseInterface databaseInterface = databaseMeta
		...
		cr_index += databaseInterface.getIndexTablespaceDDL(variables,
				databaseMeta);
	...
	public String getCreateSequenceStatement(String sequence, long start_at,
		...
		return getCreateSequenceStatement(null, sequence, Long
				.toString(start_at), Long.toString(increment_by), Long
	...
	public String getCreateSequenceStatement(String sequence, String start_at,
		...
		return getCreateSequenceStatement(null, sequence, start_at,
				increment_by, max_value, semi_colon);
	}

	public String getCreateSequenceStatement(String schemaName,
			String sequence, long start_at, long increment_by, long max_value,
			boolean semi_colon) {
		return getCreateSequenceStatement(schemaName, sequence, Long
				.toString(start_at), Long.toString(increment_by), Long
				.toString(max_value), semi_colon);
	}
	...
	public String getCreateSequenceStatement(String schemaName,
		...
		if (databaseMeta.supportsSequences()) {
			String schemaSequence = databaseMeta
					.getQuotedSchemaTableCombination(schemaName, sequenceName);
			...
			if (max_value != null) {
				// "-1" means there is no maxvalue, must be handles different by
				// DB2 / AS400
				//
				if (databaseMeta.supportsSequenceNoMaxValueOption()
	...
	public RowMetaInterface getQueryFields(String sql, boolean param,
		...
		// they failed to implement the getMetaData method on a prepared
		// statement. (!!!)
		...
		if (databaseMeta.supportsPreparedStatementMetadataRetrieval()) {
			...
			try {
				preparedStatement = connection.prepareStatement(databaseMeta
	...
	private RowMetaInterface getQueryFieldsFallback(String sql, boolean param,
			RowMetaInterface inform, Object[] data)
			throws KettleDatabaseException {
		RowMetaInterface fields;
		...
		try {
			if (inform == null
			// Hack for MSSQL jtds 1.2 when using xxx NOT IN yyy we have to use
					// a prepared statement (see BugID 3214)
			...
			throw new KettleDatabaseException("Couldn't get field info from ["
					+ sql + "]" + Const.CR, ex);
		}
	 ...
	 */
	private RowMetaInterface getRowInfo(ResultSetMetaData rm,
		...
		try {
			...
			try {
				// TODO If we do lazy conversion, we need to find out about the
				// encoding
				//
				int fieldNr = 1;
	...
	private ValueMetaInterface getValueFromSQLType(String name,
		...
		case java.sql.Types.NUMERIC:
			...
			if (databaseMeta.getDatabaseInterface() instanceof PostgreSQLDatabaseMeta
				...
				if (type == java.sql.Types.NUMERIC && length == 0
						&& precision == 0) {
					valtype = ValueMetaInterface.TYPE_BIGNUMBER;
					length = -1;
					precision = -1;
		...
		case java.sql.Types.LONGVARBINARY:
			...
			if (databaseMeta.isDisplaySizeTwiceThePrecision()
				...
				length = -1; // keep the length to -1, e.g. for string functions
				// (e.g. CONCAT see PDI-4812)
			} else {
				length = -1;
			}
			precision = -1;
		...
		// v.setOriginalAutoIncrement(originalAutoIncrement); DISABLED FOR
		// PERFORMANCE REASONS : PDI-1788
		// v.setOriginalNullable(originalNullable); DISABLED FOR PERFORMANCE
		// REASONS : PDI-1788
	...
	public boolean absolute(ResultSet rs, int position)
		...
		try {
			...
			throw new KettleDatabaseException(
					"Unable to move resultset to position " + position, e);
		}
	}
	...
	public boolean relative(ResultSet rs, int rows)
		...
		try {
			...
			throw new KettleDatabaseException(
					"Unable to move the resultset forward " + rows + " rows", e);
		}
	}
	...
	public void afterLast(ResultSet rs) throws KettleDatabaseException {
		try {
			...
			throw new KettleDatabaseException(
					"Unable to move resultset to after the last position", e);
		}
	}
	...
	public void first(ResultSet rs) throws KettleDatabaseException {
		try {
			...
			throw new KettleDatabaseException(
					"Unable to move resultset to the first position", e);
		}
	}
	 ...
	 * @return one row or null if no row was found on the resultset or if an
	 *         error occurred.
	 */
	 ...
	 *         error occurred.
	 */
	public Object[] getRow(ResultSet rs, boolean lazyConversion)
			throws KettleDatabaseException {
		if (rowMeta == null) {
	...
	public Object[] getRow(ResultSet rs, ResultSetMetaData dummy,
			RowMetaInterface rowInfo) throws KettleDatabaseException {
		...
		try {
			...
			throw new KettleDatabaseException(
					"Couldn't get row from result set", ex);
		} finally {
			if (log.isGatheringMetrics()) {
				long time = System.currentTimeMillis() - startTime;
				log.snap(Metrics.METRIC_DATABASE_GET_ROW_SUM_TIME, databaseMeta
						.getName(), time);
				log.snap(Metrics.METRIC_DATABASE_GET_ROW_MIN_TIME, databaseMeta
						.getName(), time);
				log.snap(Metrics.METRIC_DATABASE_GET_ROW_MAX_TIME, databaseMeta
						.getName(), time);
				log.snap(Metrics.METRIC_DATABASE_GET_ROW_COUNT, databaseMeta
						.getName());
			}
		}
	}
	...
	public void setLookup(String tableName, String codes[], String condition[],
		...
		setLookup(null, tableName, codes, condition, gets, rename, orderby,
	...
	public void setLookup(String schemaName, String tableName, String codes[],
			...
			boolean checkForMultipleResults) throws KettleDatabaseException {
		try {
			log.snap(Metrics.METRIC_DATABASE_SET_LOOKUP_START, databaseMeta
					.getName());

			String table = databaseMeta.getQuotedSchemaTableCombination(
					schemaName, tableName);
			...
			for (int i = 0; i < gets.length; i++) {
				...
				if (rename != null && rename[i] != null
						&& !gets[i].equalsIgnoreCase(rename[i])) {
			...
			try {
				if (log.isDetailed())
					log.logDetailed("Setting preparedStatement to [" + sql
							+ "]");
				prepStatementLookup = connection.prepareStatement(databaseMeta
						.stripCR(sql));
				if (!checkForMultipleResults
						&& databaseMeta.supportsSetMaxRows()) {
					prepStatementLookup.setMaxRows(1); // alywas get only 1 line
					// back!
				}
			} catch (SQLException ex) {
				throw new KettleDatabaseException(
			...
			log.snap(Metrics.METRIC_DATABASE_SET_LOOKUP_STOP, databaseMeta
					.getName());
		}
	}
	...
	public boolean prepareUpdate(String schemaName, String tableName,
		...
		try {
			log.snap(Metrics.METRIC_DATABASE_PREPARE_UPDATE_START, databaseMeta
					.getName());
			...
			sql.append("UPDATE ").append(schemaTable).append(Const.CR).append(
					"SET ");
			...
			try {
				...
				if (log.isDetailed())
					log.logDetailed("Setting update preparedStatement to [" + s
							+ "]");
				prepStatementUpdate = connection.prepareStatement(databaseMeta
						.stripCR(s));
			} catch (SQLException ex) {
			...
			log.snap(Metrics.METRIC_DATABASE_PREPARE_UPDATE_STOP, databaseMeta
					.getName());
		}
	}
	...
	public boolean prepareDelete(String schemaName, String tableName,
		...
		try {
			log.snap(Metrics.METRIC_DATABASE_PREPARE_DELETE_START, databaseMeta
					.getName());
			...
			try {
				if (log.isDetailed())
					log.logDetailed("Setting update preparedStatement to ["
							+ sql + "]");
				prepStatementUpdate = connection.prepareStatement(databaseMeta
						.stripCR(sql));
			} catch (SQLException ex) {
			...
			log.snap(Metrics.METRIC_DATABASE_PREPARE_DELETE_STOP, databaseMeta
					.getName());
		}
	}

	public void setProcLookup(String proc, String arg[], String argdir[],
			int argtype[], String returnvalue, int returntype)
			throws KettleDatabaseException {
		try {
			log.snap(Metrics.METRIC_DATABASE_PREPARE_DBPROC_START, databaseMeta
					.getName());
			String sql;
			int pos = 0;
			...
			try {
				...
				for (int i = 0; i < arg.length; i++) {
					if (argdir[i].equalsIgnoreCase("OUT")
						...
						switch (argtype[i]) {
						case ValueMetaInterface.TYPE_NUMBER:
							cstmt.registerOutParameter(i + pos,
				...
				throw new KettleDatabaseException(
						"Unable to prepare database procedure call", ex);
			}
		} finally {
			log.snap(Metrics.METRIC_DATABASE_PREPARE_DBPROC_STOP, databaseMeta
					.getName());
		}
	...
	public Object[] getLookup(PreparedStatement ps,
			boolean failOnMultipleResults) throws KettleDatabaseException {
		ResultSet res = null;
		try {
			log.snap(Metrics.METRIC_DATABASE_GET_LOOKUP_START, databaseMeta
					.getName());
			res = ps.executeQuery();
			...
			if (failOnMultipleResults) {
				if (ret != null && res.next()) {
					// if the previous row was null, there's no reason to try
					// res.next()
					// again.
					// on DB2 this will even cause an exception (because of the
					// buggy DB2
					// JDBC driver).
					throw new KettleDatabaseException(
			...
			throw new KettleDatabaseException(
					"Error looking up row in database", ex);
		} finally {
			try {
				if (res != null)
				...
				log.snap(Metrics.METRIC_DATABASE_GET_LOOKUP_STOP, databaseMeta
						.getName());
			}
		}
	}
	...
	public DatabaseMetaData getDatabaseMetaData()
			throws KettleDatabaseException {
		try {
			log.snap(Metrics.METRIC_DATABASE_GET_DBMETA_START, databaseMeta
					.getName());
			if (dbmd == null)
			...
			log.snap(Metrics.METRIC_DATABASE_GET_DBMETA_STOP, databaseMeta
					.getName());
		}
	...
	public String getDDL(String tablename, RowMetaInterface fields)
			throws KettleDatabaseException {
		return getDDL(tablename, fields, null, false, null, true);
	}

	public String getDDL(String tablename, RowMetaInterface fields, String tk,
			boolean use_autoinc, String pk) throws KettleDatabaseException {
	...
	public String getDDL(String tableName, RowMetaInterface fields, String tk,
		...
		if (checkTableExists(tableName)) {
			retval = getAlterTableStatement(tableName, fields, quotedTk,
					use_autoinc, pk, semicolon);
		} else {
			retval = getCreateTableStatement(tableName, fields, quotedTk,
					use_autoinc, pk, semicolon);
		}
	...
	public String getCreateTableStatement(String tableName,
			RowMetaInterface fields, String tk, boolean use_autoinc, String pk,
			boolean semicolon) {
		...
		for (int i = 0; i < fields.size(); i++) {
			...
			retval.append(databaseMeta.getFieldDefinition(v, tk, pk,
					use_autoinc));
		}
		...
		if (tk != null) {
			if (databaseMeta.requiresCreateTablePrimaryKeyAppend()) {
				retval.append(", PRIMARY KEY (").append(tk).append(")").append(
						Const.CR);
			}
		}
		...
		if (pk != null) {
			if (databaseMeta.requiresCreateTablePrimaryKeyAppend()) {
				retval.append(", PRIMARY KEY (").append(pk).append(")").append(
						Const.CR);
			}
		}
		retval.append(")").append(Const.CR);

		retval.append(databaseMeta.getDatabaseInterface().getDataTablespaceDDL(
				variables, databaseMeta));

		if (pk == null
				&& tk == null
				&& databaseMeta.getDatabaseInterface() instanceof NeoviewDatabaseMeta) {
			retval.append("NO PARTITION"); // use this as a default when no
			// pk/tk is there, otherwise you get
			// an error
		}
		...
		// TODO: All this custom database code shouldn't really be in
		// Database.java. It should be in the DB implementations.
		//
		if (databaseMeta.getDatabaseInterface() instanceof VerticaDatabaseMeta) {
			retval.append(Const.CR).append("CREATE PROJECTION ").append(
					tableName).append("_unseg_super").append(Const.CR);
			...
			for (int i = 0; i < fields.size(); i++) {
				...
				retval.append(fields.getValueMeta(i).getName())
						.append(Const.CR);
			}
			retval.append(")").append(Const.CR);
			...
			retval
					.append(
							"-- Replace UNSEGMENTED with a hash segmentation for optimum performance")
					.append(Const.CR);
	...
	public String getAlterTableStatement(String tableName,
		...
		if (missing.size() != 0) {
			for (int i = 0; i < missing.size(); i++) {
				...
				retval += databaseMeta.getAddColumnStatement(tableName, v, tk,
						use_autoinc, pk, true);
			}
		}
		...
		if (surplus.size() != 0) {
			for (int i = 0; i < surplus.size(); i++) {
				...
				retval += databaseMeta.getDropColumnStatement(tableName, v, tk,
						use_autoinc, pk, true);
			}
		}
		...
		for (int i = 0; i < fields.size(); i++) {
			...
			ValueMetaInterface currentField = tabFields
					.searchValueMeta(desiredField.getName());
			if (desiredField != null && currentField != null) {
				String desiredDDL = databaseMeta.getFieldDefinition(
						desiredField, tk, pk, use_autoinc);
				String currentDDL = databaseMeta.getFieldDefinition(
						currentField, tk, pk, use_autoinc);
		...
		if (modify.size() > 0) {
			for (int i = 0; i < modify.size(); i++) {
				...
				retval += databaseMeta.getModifyColumnStatement(tableName, v,
						tk, use_autoinc, pk, true);
			}
		}
	...
	public void truncateTable(String tablename) throws KettleDatabaseException {
		if (Const.isEmpty(connectionGroup)) {
			execStatement(databaseMeta.getTruncateTableStatement(null,
					tablename));
		} else {
	...
	public void truncateTable(String schema, String tablename)
			throws KettleDatabaseException {
		if (Const.isEmpty(connectionGroup)) {
			execStatement(databaseMeta.getTruncateTableStatement(schema,
					tablename));
		} else {
			execStatement("DELETE FROM "
					+ databaseMeta.getQuotedSchemaTableCombination(schema,
							tablename));
		}
	}
	...
	public RowMetaAndData getOneRow(String sql) throws KettleDatabaseException {
		...
		if (rs != null) {
			...
			try {
				...
				throw new KettleDatabaseException("Unable to close resultset",
						e);
			}
			...
			if (pstmt != null) {
				try {
					...
					throw new KettleDatabaseException(
							"Unable to close prepared statement pstmt", e);
				}
				pstmt = null;
			}
			if (sel_stmt != null) {
				try {
					...
					throw new KettleDatabaseException(
							"Unable to close prepared statement sel_stmt", e);
				}
				sel_stmt = null;
			}
	...
	public RowMeta getMetaFromRow(Object[] row, ResultSetMetaData md)
		...
		for (int i = 0; i < md.getColumnCount(); i++) {
			...
			ValueMetaInterface valueMeta = getValueFromSQLType(name, md, i + 1,
					true, false);
	...
	public RowMetaAndData getOneRow(String sql, RowMetaInterface param,
			Object[] data) throws KettleDatabaseException {
	...
	public void writeLogRecord(LogTableInterface logTable, LogStatus status,
			Object subject, Object parent) throws KettleException {
		try {
			...
			boolean update = (logTable.getKeyField() != null)
					&& !status.equals(LogStatus.START);
			String schemaTable = databaseMeta.getQuotedSchemaTableCombination(
			...
			if (update) {
				...
				for (int i = 1; i < rowMeta.size(); i++) // Without ID_JOB or
				// ID_BATCH
				{
					...
					sqlBuff
							.append(
									databaseMeta
											.quoteField(valueMeta.getName()))
							.append("=? ");

					updateRowMeta.addValueMeta(valueMeta);
					updateRowData[i - 1] = rowData[i];
				}
				sqlBuff.append("WHERE ").append(
						databaseMeta.quoteField(keyValueMeta.getName()))
						.append("=? ");
				...
				insertRow(
						environmentSubstitute(logTable.getActualSchemaName()),
						environmentSubstitute(logTable.getActualTableName()),
						logRecord.getRowMeta(), logRecord.getData());

			}
		} catch (Exception e) {
			throw new KettleDatabaseException(
					"Unable to write log record to log table "
							+ logTable.getActualTableName(), e);
		}
	}
	...
	public void cleanupLogRecords(LogTableInterface logTable)
		...
		try {
			double timeout = Const.toDouble(Const
					.trim(environmentSubstitute(logTable.getTimeoutInDays())),
					0.0);
			if (timeout > 0.000001) {
				// The timeout has to be at least a few seconds, otherwise we
				// don't bother
				//
				String schemaTable = databaseMeta
						.getQuotedSchemaTableCombination(
								environmentSubstitute(logTable
										.getActualSchemaName()),
								environmentSubstitute(logTable
										.getActualTableName()));
				...
				if (logField != null) {
					String sql = "DELETE FROM " + schemaTable + " WHERE "
							+ databaseMeta.quoteField(logField.getFieldName())
							+ " < ?"; // $NON-NLS$1
					...
					long limit = now
							- Math.round(timeout * 24 * 60 * 60 * 1000);
					RowMetaAndData row = new RowMetaAndData();
					row.addValue(logField.getFieldName(),
							ValueMetaInterface.TYPE_DATE, new Date(limit));
					...
					throw new KettleException(
							BaseMessages
									.getString(
											PKG,
											"Database.Exception.LogTimeoutDefinedOnTableWithoutLogField",
			...
			throw new KettleDatabaseException(
					BaseMessages
							.getString(
									PKG,
									"Database.Exception.UnableToCleanUpOlderRecordsFromLogTable",
									logTable.getActualTableName()), e);
		}
	}

	public Object[] getLastLogDate(String logtable, String name, boolean job,
			LogStatus status) throws KettleDatabaseException {
		Object[] row = null;

		String jobtrans = job ? databaseMeta.quoteField("JOBNAME")
				: databaseMeta.quoteField("TRANSNAME");

		String sql = "";
		sql += " SELECT " + databaseMeta.quoteField("ENDDATE") + ", "
				+ databaseMeta.quoteField("DEPDATE") + ", "
				+ databaseMeta.quoteField("STARTDATE");
		sql += " FROM " + logtable;
		sql += " WHERE  " + databaseMeta.quoteField("ERRORS") + "    = 0";
		sql += " AND    " + databaseMeta.quoteField("STATUS") + "    = 'end'";
		sql += " AND    " + jobtrans + " = ?";
		sql += " ORDER BY " + databaseMeta.quoteField("LOGDATE") + " DESC, "
				+ databaseMeta.quoteField("ENDDATE") + " DESC";

		try {
			pstmt = connection.prepareStatement(databaseMeta.stripCR(sql));

			RowMetaInterface r = new RowMeta();
			r.addValueMeta(new ValueMeta("TRANSNAME",
			...
			throw new KettleDatabaseException(
					"Unable to obtain last logdate from table " + logtable, ex);
		}

		return row;
	}

	public synchronized Long getNextValue(Hashtable<String, Counter> counters,
			String tableName, String val_key) throws KettleDatabaseException {
		return getNextValue(counters, null, tableName, val_key);
	}

	public synchronized Long getNextValue(Hashtable<String, Counter> counters,
			String schemaName, String tableName, String val_key)
			throws KettleDatabaseException {
		Long nextValue = null;
		...
		if (counter == null) {
			RowMetaAndData rmad = getOneRow("SELECT MAX("
					+ databaseMeta.quoteField(val_key) + ") FROM "
					+ schemaTable);
			if (rmad != null) {
				long previous;
				try {
					...
					// A "select max(x)" on a table with no matching rows will
					// return null.
					if (tmp != null)
	 ...
	 *            The SQL to launch
	 * @param limit
	 *            <=0 means unlimited, otherwise this specifies the maximum
	 *            number of rows read.
	...
	public List<RowMetaAndData> getRowsWithMeta(String sql, int limit)
			throws KettleDatabaseException {
		return getRowsWithMeta(sql, limit, null);
	}
	 ...
	 */
	public List<Object[]> getRows(String sql, int limit,
			ProgressMonitorListener monitor) throws KettleDatabaseException {
		if (monitor != null)
	...
	public List<RowMetaAndData> getRowsWithMeta(String sql, int limit,
		...
		return getRowsWithMeta(rset, limit, monitor);
	}
	...
	public List<Object[]> getRows(ResultSet rset, int limit,
		...
		try {
			...
			throw new KettleDatabaseException(
					"Unable to get list of rows from ResultSet : ", e);
		}
	}

	public List<RowMetaAndData> getRowsWithMeta(ResultSet rset, int limit,
			ProgressMonitorListener monitor) throws KettleDatabaseException {
		try {
			...
			if (rset != null) {
				...
				while ((limit <= 0 || i < limit) && !stop) {
					Object[] row = getRow(rset);
					if (row != null) {
						RowMetaAndData r = new RowMetaAndData(rowMeta, row);
						result.add(r);
						i++;
					} else {
						stop = true;
	...
	public List<Object[]> getFirstRows(String table_name, int limit)
			throws KettleDatabaseException {
	 ...
	 *            quoted properly
	 * @param limit
	 *            limit <=0 means unlimited, otherwise this specifies the
	 *            maximum number of rows read.
	 * @param monitor
	 *            The progress monitor to update while getting the rows.
	 * @return An ArrayList of rows.
	 * @throws KettleDatabaseException
	...
	public String[] getTableTypes() throws KettleDatabaseException {
		try {
			...
			throw new KettleDatabaseException(
					"Unable to get table types from database!", e);
		}
	}
	...
	public String[] getTablenames(String schemanamein, boolean includeSchema)
		...
		for (String schema : tableMap.keySet()) {
			...
			for (String table : tables)
				if (includeSchema) {
					res.add(databaseMeta.getQuotedSchemaTableCombination(
							schema, table));
				} else {
					res.add(table);
				}
	...
	public Map<String, Collection<String>> getTableMap(String schemanamein)
		...
		try {
			...
			while (alltables.next()) {
				...
				try {
					...
					if (log.isDebug())
						log
								.logDebug("Error getting tables for field TABLE_CAT (ignored): "
										+ e.toString());
				}
				...
				try {
					...
					if (log.isDebug())
						log
								.logDebug("Error getting tables for field TABLE_SCHEM (ignored): "
										+ e.toString());
				}
		...
		if (log.isDetailed())
			log.logDetailed("read :" + multimapSize(tableMap)
					+ " table names from db meta-data.");
	...
	public Map<String, Collection<String>> getViewMap(String schemanamein)
		...
		try {
			allviews = getDatabaseMetaData().getTables(null, schemaname, null,
			...
			while (allviews.next()) {
				...
				try {
					...
					if (log.isDebug())
						log
								.logDebug("Error getting views for field TABLE_CAT (ignored): "
										+ e.toString());
				}
				...
				try {
					...
					if (log.isDebug())
						log
								.logDebug("Error getting views for field TABLE_SCHEM (ignored): "
										+ e.toString());
				}
			...
			throw new KettleDatabaseException(
					"Error getting views from schema [" + schemaname + "]", e);
		} finally {
			try {
				if (allviews != null)
					allviews.close();
		...
		if (log.isDetailed())
			log.logDetailed("read :" + multimapSize(viewMap)
					+ " views from db meta-data.");
	...
	public Map<String, Collection<String>> getSynonymMap(String schemanamein)
		...
		try {
			...
			while (alltables.next()) {
				...
				try {
					...
					if (log.isDebug())
						log
								.logDebug("Error getting synonyms for field TABLE_CAT (ignored): "
										+ e.toString());
				}
				...
				try {
					...
					if (log.isDebug())
						log
								.logDebug("Error getting synonyms for field TABLE_SCHEM (ignored): "
										+ e.toString());
				}
			...
			throw new KettleDatabaseException(
					"Error getting synonyms from schema [" + schemaname + "]",
					e);
		} finally {
			try {
				if (alltables != null)
		...
		if (log.isDetailed())
			log.logDetailed("read :" + multimapSize(synonymMap)
					+ " synonyms from db meta-data.");

		return synonymMap;
	}

	private <K, V> void multimapPut(final K key, final V value,
	...
	public String[] getSchemas() throws KettleDatabaseException {
		...
		try {
			...
			try {
				...
				throw new KettleDatabaseException(
						"Error closing resultset after getting schemas!", e);
			}
		}

		if (log.isDetailed())
			log.logDetailed("read :" + catalogList.size()
					+ " schemas from db meta-data.");
	...
	public String[] getCatalogs() throws KettleDatabaseException {
		...
		try {
			...
			try {
				...
				throw new KettleDatabaseException(
						"Error closing resultset after getting catalogs!", e);
			}
		}

		if (log.isDetailed())
			log.logDetailed("read :" + catalogList.size()
					+ " catalogs from db meta-data.");
		...
		if (sql != null) {
			...
			try {
				...
				for (int i = 0; i < rows.size(); i++) {
					...
					String procCatalog = rowMeta.getString(row,
							"PROCEDURE_CAT", null);
					String procSchema = rowMeta.getString(row,
	...
	public void unlockTables(String tableNames[])
		...
		for (int i = 0; i < tableNames.length; i++)
			quotedTableNames[i] = databaseMeta.getQuotedSchemaTableCombination(
					null, tableNames[i]);
	...
	public RowMetaAndData callProcedure(String arg[], String argdir[],
			int argtype[], String resultname, int resulttype)
			throws KettleDatabaseException {
		RowMetaAndData ret;
		try {
	...
	public String getDDLCreationTable(String tableName, RowMetaInterface fields)
			throws KettleDatabaseException {
		String retval;
		...
		retval = getCreateTableStatement(tableName, fields, quotedTk, false,
				null, true);
	...
	public String getDDLTruncateTable(String schema, String tablename)
		...
		if (Const.isEmpty(connectionGroup)) {
			...
			return ("DELETE FROM " + databaseMeta
					.getQuotedSchemaTableCombination(schema, tablename));
		}
	}
	...
	public String getSQLOutput(String schemaName, String tableName,
			RowMetaInterface fields, Object[] r, String dateFormat)
			throws KettleDatabaseException {
		StringBuffer ins = new StringBuffer(128);
		...
		try {
			...
			java.text.SimpleDateFormat[] fieldDateFormatters = new java.text.SimpleDateFormat[fields
					.size()];
			...
			for (int i = 0; i < fields.size(); i++) {
				...
				if (valueMeta.isNull(valueData)) {
					...
					case ValueMetaInterface.TYPE_STRING:
						...
						// This also adds the single quotes around the string
						...
						if (Const.isEmpty(dateFormat))
							if (databaseMeta.getDatabaseInterface() instanceof OracleDatabaseMeta) {
								if (fieldDateFormatters[i] == null) {
									fieldDateFormatters[i] = new java.text.SimpleDateFormat(
											"yyyy/MM/dd HH:mm:ss");
								}
								ins.append("TO_DATE('").append(
										fieldDateFormatters[i].format(date))
	...
	public Savepoint setSavepoint() throws KettleDatabaseException {
		try {
			...
			throw new KettleDatabaseException(BaseMessages.getString(PKG,
					"Database.Exception.UnableToSetSavepoint"), e);
		}
	}
	...
	public void releaseSavepoint(Savepoint savepoint)
		...
		try {
			...
			throw new KettleDatabaseException(BaseMessages.getString(PKG,
					"Database.Exception.UnableToReleaseSavepoint"), e);
		}
	}
	...
	public void rollback(Savepoint savepoint) throws KettleDatabaseException {
		try {
			...
			throw new KettleDatabaseException(BaseMessages.getString(PKG,
					"Database.Exception.UnableToRollbackToSavepoint"), e);
		}
	}
	...
	public String[] getPrimaryKeyColumnNames(String tablename)
		...
		try {
			allkeys = getDatabaseMetaData().getPrimaryKeys(null, null,
					tablename);
			while (allkeys.next()) {
				...
				if (!names.contains(col_name))
					names.add(col_name);
				if (log.isRowLevel())
					log.logRowlevel(toString(), "getting key : " + keyname
							+ " on column " + col_name);
			}
		} catch (SQLException e) {
			log.logError(toString(),
					"Error getting primary keys columns from table ["
							+ tablename + "]");
		} finally {
			try {
				if (allkeys != null)
	 ...
	 *             in case anything goes wrong.
	 */
	public Result execStatementsFromFile(String filename,
			boolean sendSinglestatement) throws KettleException {

(function($) {
	...
	function _toggle(target, state){
		...
		var opts = t.options;
		if (state) {
			btn.unbind(".pushbutton");
		}else{
			btn.unbind(".pushbutton").bind("focus.pushbutton", function(){
				$(this).addClass("ui-state-hover");
			}).bind("blur.pushbutton", function(){
				$(this).removeClass("ui-state-hover");
			});
			
			Plywet.attachBehaviors(btn,Plywet.assembleBehaviors(opts.events,["focus","blur"]),opts);
			Plywet.attachBehaviorsOn(btn, opts);
		}
	}
...
Plywet.widget.PushButton=function(cfg){
	...
	if(this.parent){
		if(typeof this.parent == "string"){
			this.parent = $(Plywet.escapeClientId(this.parent));
		}
		this.jq = $(this.parent).find(Plywet.escapeClientId(this.id));
		
		if(this.jq.length == 0){
			this.jq = $("<span id='"+this.id+"'></span>").appendTo(this.parent);
		}
	}else{

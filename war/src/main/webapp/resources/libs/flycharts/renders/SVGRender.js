 ...
 */
$FC.SVGElement = function () {
	this.init.apply(this, arguments);
};


$FC.SVGElement.prototype = {
	/**
	...
	init: function (renderer, nodeName) {
		...
		wrapper.element = nodeName === 'span' ?
			...
			$FC.doc.createElementNS($FC.SVG_NS, nodeName);
	...
	attr: function (hash, val) {
		...
		if ($FC.isString(hash) && $FC.defined(val)) {
			key = hash;
			hash = {};
			hash[key] = val;
		}
		...
		if ($FC.isString(hash)) {
			...
			ret = $FC.attr(element, key) || wrapper[key] || 0;
				...
				if (result !== false) {
					...
					// update child tspans x values
						...
						for (i = 0; i < element.childNodes.length; i++) {
							...
							if ($FC.attr(child, 'x') === $FC.attr(element, 'x')) {
								//child.setAttribute('x', value);
								$FC.attr(child, 'x', value);
							}
						}
					...
					// emulate VML's dashstyle implementation
						...
						if (value === 'solid') {
							...
							i = value.length;
							while (i--) {
								value[i] = $FC.pInt(value[i]) * $FC.pick(hash['stroke-width'], wrapper['stroke-width']);
							}
							value = value.join(',');
						}
					...
					// Title requires a subnode, #431
						...
						if (!titleNode) {
							titleNode = $FC.doc.createElementNS($FC.SVG_NS, 'title');
					...
					if (key === 'stroke-width' || key === 'stroke') {
						...
						if (wrapper.stroke && wrapper['stroke-width']) {
							$FC.attr(element, 'stroke', wrapper.stroke);
							$FC.attr(element, 'stroke-width', wrapper['stroke-width']);
	...
	addClass: function (className) {
		var element = this.element,
			currentClassName = $FC.attr(element, 'class') || '';

		if (currentClassName.indexOf(className) === -1) {
			$FC.attr(element, 'class', currentClassName + ' ' + className);
		}
		return this;
	},
	/* hasClass and removeClass are not (yet) needed
	hasClass: function (className) {
		return $FC.attr(this.element, 'class').indexOf(className) !== -1;
	},
	removeClass: function (className) {
		$FC.attr(this.element, 'class', $FC.attr(this.element, 'class').replace(className, ''));
		return this;
	},
	*/
	...
	css: function (styles) {
		...
		if ($FC.isIE && !$FC.hasSVG) { // legacy IE doesn't support setting style attribute
			if (textWidth) {
				delete styles.width;
			}
			$FC.css(elemWrapper.element, styles);
		} else {
			for (n in styles) {
				serializedCss += n.replace(/([A-Z])/g, hyphenate) + ':' + styles[n] + ';';
			}
			$FC.attr(elem, 'style', serializedCss); // #1881
		}
	...
	htmlUpdateTransform: function () {
		...
		if (elem.tagName === 'SPAN') {
			...
			var width, height,
				...
				sintheta = 0,
				quad,
				textWidth = $FC.pInt(wrapper.textWidth),
			...
			if (currentTextTransform !== wrapper.cTT) { // do the calculations and DOM access only if properties changed
				...
				if ($FC.defined(rotation)) {
					
					radians = rotation * deg2rad; // deg to rad
					costheta = $FC.mathCos(radians);
					sintheta = $FC.mathSin(radians);
	
					wrapper.setSpanRotation(rotation, sintheta, costheta);
					
				}

				width = $FC.pick(wrapper.elemWidth, elem.offsetWidth);
				height = $FC.pick(wrapper.elemHeight, elem.offsetHeight);

				// update textWidth
				if (width > textWidth && /[ \-]/.test(elem.textContent || elem.innerText)) { // #983, #1254
					$FC.css(elem, {
						width: textWidth + $FC.PX,
						display: 'block',
						whiteSpace: 'normal'
					});
					width = textWidth;
	...
	setSpanRotation: function (rotation, sintheta, costheta) {
		var rotationStyle = {},
			cssTransformKey = $FC.isIE ? '-ms-transform' : isWebKit ? '-webkit-transform' : $FC.isFirefox ? 'MozTransform' : $FC.isOpera ? '-o-transform' : '';
		
		rotationStyle[cssTransformKey] = rotationStyle.transform = 'rotate(' + rotation + 'deg)';
		$FC.css(this.element, rotationStyle);
	},
	...
	updateTransform: function () {
		...
		// apply scale
		if ($FC.defined(scaleX) || $FC.defined(scaleY)) {
			transform.push('scale(' + $FC.pick(scaleX, 1) + ' ' + $FC.pick(scaleY, 1) + ')');
		}

		if (transform.length) {
			$FC.attr(wrapper.element, 'transform', transform.join(' '));
		}
	},
	/**
	...
	align: function (alignOptions, alignByTranslate, box) {
		...
		box = $FC.pick(box, renderer[alignTo], renderer);
	...
	getBBox: function () {
		...
		if (!bBox) {
			// SVG elements
			if (element.namespaceURI === $FC.SVG_NS || renderer.forExport) {
			...
			if (renderer.isSVG) {
				...
				if ($FC.isIE && styles && styles.fontSize === '11px' && height.toPrecision(3) === '22.7') {
	...
	add: function (parent) {
		...
		if (parentWrapper.handleZ) { // this element or any of its siblings has a z index
			for (i = 0; i < childNodes.length; i++) {
				...
				otherZIndex = $FC.attr(otherElement, 'zIndex');
				if (otherElement !== element && (
						// insert before the first element with a higher zIndex
						$FC.pInt(otherZIndex) > zIndex ||
						// if no zIndex given, insert before the first element with a zIndex
						(!$FC.defined(zIndex) && $FC.defined(otherZIndex))
	...
	shadow: function (shadowOptions, group, cutOff) {
		...
		if (shadowOptions) {
			shadowWidth = $FC.pick(shadowOptions.width, 3);
			shadowElementOpacity = (shadowOptions.opacity || 0.15) / shadowWidth;
			transform = this.parentInverted ? 
				'(-1,-1)' : 
				'(' + $FC.pick(shadowOptions.offsetX, 1) + ', ' + $FC.pick(shadowOptions.offsetY, 1) + ')';
 ...
 */
$FC.SVGRenderer = function () {
	this.init.apply(this, arguments);
};
$FC.SVGRenderer.prototype = {
	Element: $FC.SVGElement,
	...
	init: function (container, width, height, forExport) {
		...
		if (container.innerHTML.indexOf('xmlns') === -1) {
			$FC.attr(element, 'xmlns', $FC.SVG_NS);
		}
		...
		renderer.url = ($FC.isFirefox || $FC.isWebKit) && $FC.doc.getElementsByTagName('base').length ? 
			loc.href
		...
		desc.element.appendChild($FC.doc.createTextNode('Created with ' + $FC.PRODUCT + ' ' + $FC.VERSION));
		...
		if ($FC.isFirefox && container.getBoundingClientRect) {
			renderer.subPixelFix = subPixelFix = function () {
				$FC.css(container, { left: 0, top: 0 });
				rect = container.getBoundingClientRect();
				$FC.css(container, {
					left: ($FC.mathCeil(rect.left) - rect.left) + $FC.PX,
					top: ($FC.mathCeil(rect.top) - rect.top) + $FC.PX
				});
			};
			...
			// run it on resize
			$FC.addEvent($FC.win, 'resize', subPixelFix);
		}
	},
	...
	destroy: function () {
		...
		if (renderer.subPixelFix) {
			removeEvent($FC.win, 'resize', renderer.subPixelFix);
		}
	...
	buildText: function (wrapper) {
		var textNode = wrapper.element,
			...
			lines = $FC.pick(wrapper.textStr, '').toString()
			...
			styleRegex = /style="([^"]+)"/,
			hrefRegex = /href="(http[^"]+)"/,
			parentX = $FC.attr(textNode, 'x'),
			textStyles = wrapper.styles,
			width = textStyles && textStyles.width && $FC.pInt(textStyles.width),
			...
			$FC.each(spans, function (span) {
				if (span !== '' || spans.length === 1) {
					...
					if (styleRegex.test(span)) {
						...
						$FC.attr(tspan, 'style', spanStyle);
					}
					if (hrefRegex.test(span) && !forExport) { // Not for export - #1529
						$FC.attr(tspan, 'onclick', 'location.href=\"' + span.match(hrefRegex)[1] + '\"');
						$FC.css(tspan, { cursor: 'pointer' });
					}
					...
					if (span !== ' ') {
						...
						// add the text node
						tspan.appendChild($FC.doc.createTextNode(span));
						...
						if (width) {
							...
							while (words.length || rest.length) {
								...
								if (words.length) {
									tspan.appendChild($FC.doc.createTextNode(words.join(' ').replace(/- /g, '-')));
								}
							}
						}
					}
	...
	button: function (text, x, y, callback, normalState, hoverState, pressedState) {
		...
		$FC.addEvent(label.element, $FC.isIE ? 'mouseover' : 'mouseenter', function () {
			label.attr(hoverState)
				.css(hoverStyle);
		});
		$FC.addEvent(label.element, $FC.isIE ? 'mouseout' : 'mouseleave', function () {
	...
	setSize: function (width, height, animate) {
		...
		renderer.boxWrapper[$FC.pick(animate, true) ? 'animate' : 'attr']({
			width: width,
			height: height
		});
	...
	symbols: {
		...
		'arc': function (x, y, w, h, options) {
			var start = options.start,
				...
				open = options.open,
				cosStart = $FC.mathCos(start),
				sinStart = $FC.mathSin(start),
				cosEnd = $FC.mathCos(end),
				sinEnd = $FC.mathSin(end),
					...
					if (!htmlGroup) {
						...
						$FC.each(parents.reverse(), function (parentGroup) {
							...
							htmlGroup = parentGroup.div = parentGroup.div || createElement(DIV, {
								className: $FC.attr(parentGroup.element, 'class')
							}, {
								position: ABSOLUTE,
								left: (parentGroup.translateX || 0) + $FC.PX,
								top: (parentGroup.translateY || 0) + $FC.PX
							...
							$FC.extend(parentGroup.attrSetters, {
								...
								translateY: function (value) {
									htmlGroupStyle.top = value + $FC.PX;
								},
	...
	label: function (str, x, y, shape, anchorX, anchorY, useHTML, baseline, className) {
		...
		function updateTextPadding() {
			...
			if ($FC.defined(width) && (textAlign === 'center' || textAlign === 'right')) {
		 ...
		 */
		$FC.addEvent(wrapper, 'add', getSizeAfterAdd);
		...
		attrSetters.padding =  function (value) {
			if ($FC.defined(value) && value !== padding) {
				padding = value;
				updateTextPadding();
			}
			return false;
		...
		attrSetters.paddingLeft =  function (value) {
			if ($FC.defined(value) && value !== paddingLeft) {
				paddingLeft = value;
				updateTextPadding();
			}
			return false;

 ...
 */
$FC.Pointer = function (chart, options) {
	this.init(chart, options);
}

$FC.Pointer.prototype = {
	/**
	 * Initialize Pointer
	 */
	init: function (chart, options) {
		
		var zoomType = $FC.useCanVG ? '' : options.chart.zoomType,
		...
		if (options.tooltip.enabled) {
			chart.tooltip = new $FC.Tooltip(chart, options.tooltip);
		}
	...
	normalize: function (e) {
		...
		e = $FC.washMouseEvent(e);
		
		// iOS
		ePos = e.touches ? e.touches.item(0) : e;

		// get mouse position
		this.chartPosition = chartPosition = $FC.offset(this.chart.container);

		// Old IE and compatibility mode use clientX. #886, #2005.
		return $FC.extend(e, {
			chartX: $FC.mathRound($FC.pick(ePos.pageX, ePos.clientX) - chartPosition.left),
			chartY: $FC.mathRound($FC.pick(ePos.pageY, ePos.clientY) - chartPosition.top)
		});
	},
	...
	getCoordinates: function (e) {
		...
		$FC.each(this.chart.axes, function (axis) {
	...
	pinch: function (e) {
		...
		// Event type is touchmove, handle panning and pinching
			...
			// Set the marker
			if (!selectionMarker) {
				self.selectionMarker = selectionMarker = $FC.extend({
					destroy: noop
				}, chart.plotBox);
			}
			...
			if (this.hasDragged || hasPinched) {
				...
				if (runZoom) {
					fireEvent(chart, 'selection', selectionData, function (args) { 
						chart.zoom($FC.extend(args, hasPinched ? { animation: false } : null)); 
					});
				}
	...
	onContainerClick: function (e) {
		...
		if (!chart.cancelClick) {
			...
			if (hoverPoint && this.inClass(e.target, $FC.PREFIX + 'tracker')) {
				...
				fireEvent(hoverPoint.series, 'click', $FC.extend(e, {
					point: hoverPoint
				}));
			...
			// When clicking outside a tracker, fire a chart event
				...
				$FC.extend(e, this.getCoordinates(e));
	...
	setDOMEvents: function () {
		...
		this._events = events = [
			...
			[$FC.doc, 'mousemove', 'onDocumentMouseMove'],
			[$FC.doc, 'mouseup', 'onDocumentMouseUp']
		];
		...
		if ($FC.hasTouch) {
			events.push(
				...
				[$FC.doc, 'touchend', 'onDocumentTouchEnd']
			);
		}

		$FC.each(events, function (eventConfig) {
			...
			if (eventConfig[1].indexOf('on') === 0) {
				...
				$FC.addEvent(eventConfig[0], eventConfig[1], pointer['_' + eventConfig[2]]);
			}
		});
	...
	destroy: function () {
		...
		$FC.each(pointer._events, function (eventConfig) {	
			if (eventConfig[1].indexOf('on') === 0) {

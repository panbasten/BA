<fly:composition freeLayout="N">
	<fly:verticalLayout margin="10">
		...
		<fly:dataGrid id="${formId}:parameters" singleSelect="true" data="${parameters}" height="280">
			...
			<fly:toolbar>
				<fly:pushbutton id="${formId}:options:append" iconCls="ui-icon-plusthick" onclick="Flywet.editors.toolbarButton.addRow('${formId}:parameters');" title="添加" />
				<fly:pushbutton id="${formId}:options:remove" iconCls="ui-icon-closethick" onclick="Flywet.editors.toolbarButton.deleteRow('${formId}:parameters');" title="删除" />
			</fly:toolbar>
		</fly:dataGrid>
	</fly:verticalLayout>
</fly:composition>

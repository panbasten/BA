<fly:composition freeLayout="N">
	...
	<fly:verticalLayout margin="10">
		...
		<fly:dataGrid id="${formId}:partitioningInformations" singleSelect="true" height="250" data="${partitioningInformations}" disabled="${!dbMeta.partitioned}">
			...
			<fly:toolbar>
				<fly:pushbutton id="${formId}:partitioningInformations:append" btnStyle="link" iconCls="ui-icon-plusthick" onclick="Flywet.editors.toolbarButton.addRow('${formId}:partitioningInformations');" title="添加" />
				<fly:pushbutton id="${formId}:partitioningInformations:remove" btnStyle="link" iconCls="ui-icon-closethick" onclick="Flywet.editors.toolbarButton.deleteRow('${formId}:partitioningInformations');" title="删除" />
			</fly:toolbar>
		</fly:dataGrid>
	</fly:verticalLayout>

...
public interface PlaceHierarchiesOnAxes extends Extension {
	...
	/**
	 * finds the expression that generates members for <code>hier</code>. If
	 * <code>hier</code> is used in the query, the expression from the query
	 * should be returned, so the navigation position remains unchanged. If the
	 * hierarchy is not used in the query, a new expression is created that
	 * selects the root members of the hierarchy.
	 * 
	...
	/**
	 * sets the MemberExpressions for an Axis. The axis definition is replaced
	 * by the memberExpressions.
	 */
	void setQueryAxis(Axis target, Object[] memberExpressions);

	/**
	 * set the "expand All member" flag if this flag is set and an "All" memper
	 * is put onto an axis, the children of the All member will be added as
	 * well.
	 */

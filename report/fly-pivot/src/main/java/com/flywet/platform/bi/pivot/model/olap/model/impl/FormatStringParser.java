	...
	private CellFormatter getCellFormatter(String propValue) {
		synchronized (cellFormatters) {
			...
			if (cf == null) {
				...
				try {
					String className = resources.getString("cellfmt."
							+ propValue);
					Class clazz = Class.forName(className);
					Constructor ctor = clazz.getConstructor(new Class[0]);
					cf = (CellFormatter) ctor.newInstance(new Object[0]);
				} catch (Exception e) {
					logger.error("could not instantiate cell formatter "
							+ propValue, e);
				}
				cellFormatters.put(propValue, cf);
			} else {
				logger.error("Could not find a property definition for exit = "
						+ propValue);
			}
			return cf;
		}

...
/**
 * A native web browser, using Internet Explorer or Mozilla on Windows, and
 * Mozilla on other platforms.<br/>
 * Methods execute when this component is initialized. If the component is not
 * initialized, methods will be executed as soon as it gets initialized. If the
 * initialization fails, the methods will not have any effect. The results from
 * methods have relevant values only when the component is valid.
 * 
...
public class JWebBrowser extends NSPanelComponent {
	...
	/**
	 * The function to use when sending a command from some web content using
	 * Javascript.
	 */
	public static final String COMMAND_FUNCTION = INativeWebBrowser.COMMAND_FUNCTION;

	/**
	 * The prefix to use when sending a command from some web content, using a
	 * static link or by setting window.location from Javascript.
	 */
	public static final String COMMAND_LOCATION_PREFIX = INativeWebBrowser.COMMAND_LOCATION_PREFIX;

	/**
	 * The prefix to use when sending a command from some web content, by
	 * setting window.status from Javascript.
	 */
	...
	/**
	 * Create an option to make the web browser use the Mozilla XULRunner
	 * runtime.
	 * 
	...
	public static interface WebBrowserDecoratorFactory {
		/**
		 * Create the decorator for a web browser, which adds the rendering
		 * component to its component hierarchy and will itself be added to the
		 * web browser.
		 * 
		 * @param webBrowser
		 *            the webbrowser for which to create the decorator.
		 * @param renderingComponent
		 *            the component that renders the web browser's content.
	 ...
	 * 
	 * @param webBrowserDecoratorFactory
	 *            the factory that creates the decorators, or null for default
	 *            decorators.
	 */
	...
	private List<WebBrowserFunction> webBrowserFuncs = new ArrayList<WebBrowserFunction>();
	...
	/**
	 * Create a decorator for this web browser. This method can be overriden so
	 * that the web browser uses a different decorator.
	 * 
	 * @param renderingComponent
	...
	protected WebBrowserDecorator createWebBrowserDecorator(Component renderingComponent) {
		if (webBrowserDecoratorFactory != null) {
			WebBrowserDecorator webBrowserDecorator = webBrowserDecoratorFactory.createWebBrowserDecorator(this,
					renderingComponent);
	...
	private static INativeWebBrowserStatic webBrowserStatic = NativeCoreObjectFactory.create(
			INativeWebBrowserStatic.class, "chrriis.dj.nativeswing.swtimpl.components.core.NativeWebBrowserStatic",
	 ...
	 *            the url.
	 * @param value
	 *            the value, in a cookie form like: <code>foo=bar</code> (basic
	 *            session cookie)
	 *            <code>foo=bar; path=/; domain=.eclipse.org</code> (session
	 *            cookie)
	 *            <code>foo=bar; expires=Thu, 01-Jan-2030 00:00:01 GMT</code>
	 *            (persistent cookie)
	 *            <code>foo=; expires=Thu, 01-Jan-1970 00:00:01 GMT</code>
	 *            (deletes cookie <code>foo</code>)
	 */
	...
	/**
	 * Copy the appearance, the visibility of the various bars, from one web
	 * browser to another.
	 * 
	 * @param fromWebBrowser
	 *            the web browser to copy the appearance from.
	 * @param toWebBrowser
	 *            the web browser to copy the appearance to.
	 */
	...
	/**
	 * Copy the content, whether a URL or its HTML content, from one web browser
	 * to another.
	 * 
	 * @param fromWebBrowser
	 *            the web browser to copy the content from.
	 * @param toWebBrowser
	 *            the web browser to copy the content to.
	 */
	...
	public JWebBrowser(NSOption... options) {
		...
		nativeWebBrowser = NativeCoreObjectFactory.create(INativeWebBrowser.class,
				"chrriis.dj.nativeswing.swtimpl.components.core.NativeWebBrowser", new Class<?>[] { JWebBrowser.class,
						INativeWebBrowser.WebBrowserRuntime.class }, new Object[] { this, runtime });
	 ...
	 * 
	 * @param isMenuBarVisible
	 *            true if the menu bar should be visible, false otherwise.
	 */
	 ...
	 * 
	 * @param isButtonBarVisible
	 *            true if the button bar should be visible, false otherwise.
	 */
	 ...
	 * 
	 * @param isLocationBarVisible
	 *            true if the location bar should be visible, false otherwise.
	 */
	 ...
	 *            the URL or path.
	 * @param parameters
	 *            the parameters (headers and POST data) to send with the
	 *            navigation request.
	 * @return true if the navigation was successful.
	 */
	...
	/**
	 * Invoke the web browser Stop functionality, to stop all current loading
	 * operations.
	 */
	...
	/**
	 * Indicate if Javascript will be allowed to run in pages subsequently
	 * viewed.
	 * 
	...
	/**
	 * Set whether javascript will be allowed to run in pages subsequently. Note
	 * that setting this value does not affect the running of javascript in the
	 * current page.
	 * 
	...
	/**
	 * Execute some javascript, and wait for the result coming from the return
	 * statements.
	 * 
	 * @param javascript
	 *            the javascript to execute which must contain explicit return
	 *            statements.
	...
	public Object executeJavascriptWithResult(String javascript) {
		...
		// "  return null;" +
		// "}");
		Object[] result = executeJavascriptWithCommandResult("[[getScriptResult]]", "try {" + "  " + COMMAND_FUNCTION
				+ "('[[getScriptResult]]', (function() {" + javascript + "})());" + "} catch(exxxxx) {" + "  "
	...
	/**
	 * Create the Javascript function call using the function name and Java
	 * objects as arguments. Note that it does not contain a semi-colon at the
	 * end of the statement, to allow call chaining.
	 * 
	 * @param functionName
	 *            the name of the Javascript funtion.
	 * @param args
	 *            the Java objects (String, number, boolean, or array) which
	 *            will get converted to Javascript arguments.
	 * @return the function call, in the form
	 *         "functionName(convArg1, convArg2, ...)".
	 */
	...
	/**
	 * Convert a Java object to Javascript, to simplify the task of executing
	 * scripts. Conversion adds quotes around Strings (with Java escaping and
	 * Javascript unescaping around), add brackets to arrays, treats arrays of
	 * arrays, and can handle null values.
	 * 
	 * @param o
	 *            the object to convert, which can be a String, number, boolean,
	 *            or array.
	 */
	...
	// private static Object convertJavascriptObjectToJava(String type, String
	// value) {
	// if(type.length() == 0) {
	// return null;
	// }
	...
	/**
	 * Get the loading progress, a value between 0 and 100, where 100 means it
	 * is fully loaded.
	 * 
	 * @return a value between 0 and 100 indicating the current loading
	 *         progress.
	 */
	 ...
	 *            The authentication handler, or null to remove the current one.
	 */
	 ...
	 * 
	 * @param listener
	 *            The web browser listener to add.
	 */
	 ...
	 * 
	 * @param listener
	 *            the web browser listener to remove.
	 */
	public void removeWebBrowserListener(WebBrowserListener listener) {
		listenerList.remove(WebBrowserListener.class, listener);
		NativeWebBrowserListener nativeWebBrowserListener = webBrowserListenerToNativeWebBrowserListenerMap
				.remove(listener);
	...
	public void clearWebBrowserListeners(){
		WebBrowserListener[] webBrowserListeners = getWebBrowserListeners();
		for (WebBrowserListener webBrowserListener : webBrowserListeners) {
	 ...
	 * 
	 * @param areBarsVisible
	 *            true to show all bars, false to hide them all.
	 */
	...
	/**
	 * Set whether this component is able to detect a popup menu gesture to show
	 * its default popup menu.
	 * 
	 * @param isDefaultPopupMenuRegistered
	 *            true if the default popup menu is registered.
	 */
	...
	/**
	 * Register a function to the web browser which can be called from
	 * Javascript.
	 * 
	 * @param function
	 *            the function to add.
	...
	public void registerFunction(WebBrowserFunction function) {
		webBrowserFuncs.add(function);
		nativeWebBrowser.registerFunction(function);
	}

	/**
	 * Unregister a function from the web browser, which cannot be called from
	 * Javascript anymore.
	 * 
	 * @param function
	...
	public void unregisterFunction(WebBrowserFunction function) {
		webBrowserFuncs.remove(function);
		nativeWebBrowser.unregisterFunction(function);
	}

	public void clearFunction() {
		for (WebBrowserFunction webBrowserFunc : webBrowserFuncs) {
			nativeWebBrowser.unregisterFunction(webBrowserFunc);
		}
		webBrowserFuncs.clear();
	}
	...
	/**
	 * Get the version of the browser. This is mainly for troubleshooting and
	 * may even return null if it fails to detect it.
	 * 
	...
	/**
	 * Dispose the native peer but potentially allows confirmation dialog to the
	 * user.
	 * 
	 * @param isConfirmationDialogAllowed
	 *            true if the component is allowed to ask confirmation to the
	...
	/**
	 * Attempt to print the content of the currently loaded page specifying
	 * whether to show the print dialog. This method invokes the Javascript
	 * print facility, or potentially direct native calls if the platforms
	 * allows it. Not all runtimes support printing without showing the dialog
	 * (only IE supports it at the moment) in which case the method would return
	 * false.
	 * 

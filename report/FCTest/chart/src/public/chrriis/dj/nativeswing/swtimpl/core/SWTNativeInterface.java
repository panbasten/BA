 ...
 * zll修改，将SWT进程改为普通方式，去掉了所谓的agent方式
...
//      }
  ...
  static class OutProcess {
    ...
    private static Process createProcess(String localHostAddress, int port, int pid) {
      ...
      //如果是真的Applet模式，需要将SystemProperty.JAVA_IO_TMPDIR.get(), ".djnativeswing/classpath"设置到类路径中
      if(isProxyClassLoaderUsed){
    	  String tempClassPosition = SystemProperty.JAVA_IO_TMPDIR.get() + 
    			  ".djnativeswing" + File.separator + "classpath";
    	  classPathList.clear();
    	  classPathList.add(tempClassPosition);
    	  File classPathFile = new File(tempClassPosition);
    	  for(File child : classPathFile.listFiles()){
    		  if(child.getName().endsWith(".jar")){
    			  classPathList.add(tempClassPosition + File.separator + child.getName());
    		  }
    	  }
      }
...
//      systemPropertiesMap.put("visualvm.display.name", "NativeSwingPeer#" + pid);
      ...
      try {
        ...
        try{
        	...
        	System.out.println("有这个异常说明进程正在运行！");
        }
      } catch(Exception e) {
        ...
        if(p != null) {
          try {        	  
            ...
            break;
        	  ...
        	  System.out.println("Process is not terminated, which means no error");

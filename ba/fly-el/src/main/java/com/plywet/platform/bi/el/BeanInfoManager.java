...
public class BeanInfoManager {
	...
	public static BeanInfoManager getBeanInfoManager(Class pClass) {
		BeanInfoManager ret = (BeanInfoManager) mBeanInfoManagerByClass
				.get(pClass);
		if (ret == null) {
	...
	/**
	 * 
	 * Returns the BeanInfoProperty for the specified property in the given
	 * class, or null if not found.
	 **/
	public static BeanInfoProperty getBeanInfoProperty(Class pClass,
			String pPropertyName) throws ELException {
	...
	/**
	 * 
	 * Returns the BeanInfoIndexedProperty for the specified property in the
	 * given class, or null if not found.
	 **/
	public static BeanInfoIndexedProperty getBeanInfoIndexedProperty(
			Class pClass, String pIndexedPropertyName) throws ELException {
		return getBeanInfoManager(pClass).getIndexedProperty(
		...
		try {
			...
			for (int i = 0; pds != null && i < pds.length; i++) {
				...
				if (pd instanceof IndexedPropertyDescriptor) {
					...
					Method readMethod = getPublicMethod(ipd
							.getIndexedReadMethod());
					Method writeMethod = getPublicMethod(ipd
							.getIndexedWriteMethod());
					BeanInfoIndexedProperty property = new BeanInfoIndexedProperty(
							readMethod, writeMethod, ipd);
				...
				BeanInfoProperty property = new BeanInfoProperty(readMethod,
						writeMethod, pd);
			...
			if (log.isWarnEnabled()) {
				log.warn(MessageUtil.getMessageWithArgs(
						Constants.EXCEPTION_GETTING_BEANINFO, mBeanClass
								.getName()), exc);
			}
		}
	}
	...
	/**
	 * 
	 * Returns the BeanInfoIndexedProperty for the given property name, or null
	 * if not found.
	 **/
	public BeanInfoIndexedProperty getIndexedProperty(
			String pIndexedPropertyName) throws ELException {
		checkInitialized();
		return (BeanInfoIndexedProperty) mIndexedPropertyByName
				.get(pIndexedPropertyName);
	}

	// -------------------------------------
	/**
	 * 
	 * Returns the EventSetDescriptor for the given event set name, or null if
	 * not found.
	 **/
	...
	/**
	 * 
	 * Returns a publicly-accessible version of the given method, by searching
	 * for a public declaring class.
	 **/
	 ...
	 * and arguments as the given method, then that method is returned.
	 * Otherwise the superclass and interfaces are searched recursively.
	 **/
	static Method getPublicMethod(Class pClass, Method pMethod) {
		...
		if (Modifier.isPublic(pClass.getModifiers())) {
			try {
				Method m;
				try {
					m = pClass.getDeclaredMethod(pMethod.getName(), pMethod
							.getParameterTypes());
					...
					m = pClass.getMethod(pMethod.getName(), pMethod
							.getParameterTypes());
				}
